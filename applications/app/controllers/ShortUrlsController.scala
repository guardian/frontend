...
class ShortUrlsController(
  ...
  def redirectShortUrl(shortUrl: String): Action[AnyContent] = Action.async { implicit request =>
    redirectUrl(shortUrl, request.queryString)
  }

  private def redirectUrl(shortUrl: String, queryString: Map[String, Seq[String]])(implicit request: RequestHeader): Future[Result] = {
  ...
  def fetchCampaignAndRedirectShortCode(shortUrl: String, campaignCode: String): Action[AnyContent] = Action.async { implicit request =>
