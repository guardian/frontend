...
case class AudioPage(audio: Video, storyPackage: List[Trail])

object AudioController extends Controller with Logging with ExecutionContexts {
  ...
  private def lookup(path: String)(implicit request: RequestHeader) = {
    ...
    val result = response map { response =>
      ...
      val videoOption = response.content filter { _.isAudio } map { Video(_) }
  ...
  private def renderVideo(model: VideoPage)(implicit request: RequestHeader): Result = {
    val htmlResponse = () => views.html.audio(model)
    val jsonResponse = () => views.html.fragments.audioBody(model)
