...
import model.notifications.{RedisMessageStore, GCMMessage, LatestNotificationsDynamoDbStore, DynamoDbStore}
...
object NotificationsController extends Controller with ExecutionContexts with Logging {
  ...
  def subscribeToTopic() = Action.async { implicit request =>
    form.bindFromRequest.fold(
      ...
      topicInfo => {
        println(s"++ Subscribe to topic ${topicInfo.notificationTopicId}")
        ...
        response.flatMap { res =>
          println("S: " + res.status + " B:" + res.body)
          res.status match {
            case 200 =>
              println("Subscribed")
              Future.successful(Ok)
