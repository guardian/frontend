...
object AccessibleCrosswordRows extends CrosswordGridDataOrdering with CrosswordGridColumnNotation {
  ...
  def getBlankSquaresForRow(
      positionsForRow: List[CrosswordPosition],
      rowIndex: Int,
      columns: Int,
  ): AccessibleCrosswordRow = {
  ...
  def apply(crossword: Crossword): AccessibleCrosswordRows = {
    ...
    val allCrosswordPositionsByRow =
      Grid.fromCrossword(crossword).cells.toList.map { cell => cell._1 }.groupBy { position => position.y }

    val missingColumnsByRow = (0 until rows).toList.map { row =>
