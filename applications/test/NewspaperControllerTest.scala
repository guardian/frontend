...
@DoNotDiscover class NewspaperControllerTest extends FlatSpec with Matchers with ConfiguredTestSuite {
  ...
  val newspaperController = new NewspaperController
  ...
  it should "return 200 to dated page for /theguardian (in the past)" in {
    val result = newspaperController.newspaperForDate("theguardian", "01", "jan", "2015")(TestRequest("/theguardian/2015/jan/01"))
    status(result) should be(200)
  }

  it should "return 200 to dated page for a Sunday date /theobserver (in the past)" in {
    val result = newspaperController.newspaperForDate("theobserver", "08", "nov", "2015")(TestRequest("/theobserver/2015/nov/08"))
    status(result) should be(200)
  }
  ...
  it should "redirect a non Sunday Observer date url to /theobserver" in {
    val result = newspaperController.newspaperForDate("theobserver", "19", "nov", "2015")(TestRequest("/theguardian/2015/nov/19"))
    status(result) should be(302)
  ...
  it should "redirect a Sunday Guardian date url to /theguardian" in {
    val result = newspaperController.newspaperForDate("theguardian", "08", "nov", "2015")(TestRequest("/theguardian/2015/nov/08"))
    status(result) should be(302)
    header("Location", result).head should be("/theguardian")
  }
