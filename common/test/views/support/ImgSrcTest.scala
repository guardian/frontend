...
class ImgSrcTest extends FlatSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "ImgSrc" should "convert the URL of a static image to the resizing endpoint with a /static prefix" in {
    ImageServerSwitch.switchOn()
      Item700.bestSrcFor(image).get should startWith (s"$imageHost/img/static/sys-images/Guardian/Pix/pictures/2013/7/5/1373023097878/b6a5a492-cc18-4f30-9809-88467e07ebfa-460x276.jpeg?width=700&quality=85&auto=format&fit=max&s=")
  }

  it should "convert the URL of a media service to the resizing endpoint with a /media prefix" in {
    ImageServerSwitch.switchOn()
      Item700.bestSrcFor(mediaImage).get should startWith (s"$imageHost/img/media/knly7wcp46fuadowlsnitzpawm/437_0_3819_2291/1000.jpg?width=700&quality=85&auto=format&fit=max&s=")
  }
  ...
  it should "convert the URL of the image if it is a PNG (original image from static.guim.co.uk domain)" in {
    ...
    Item700.bestSrcFor(pngImage).get should startWith (s"$imageHost/img/static/sys-images/Guardian/Pix/contributor/2014/10/30/1414675415419/Jessica-Valenti-R.png?width=700&quality=85&auto=format&fit=max&s=")
  }
  ...
  it should "convert the URL of a jpeg s3 upload to the resizing endpoint with a /uploads prefix" in {
    ImageServerSwitch.switchOn()
    Item700.bestSrcFor(s3UploadJpgImage).get should startWith (s"$imageHost/img/uploads/2016/02/10/Screen_Shot_2016-02-09_at_17.50.09.jpeg?width=700&quality=85&auto=format&fit=max&s=")
  }

  it should "convert the URL of a png s3 upload to the resizing endpoint with a /uploads prefix" in {
    ImageServerSwitch.switchOn()
    Item700.bestSrcFor(s3UploadPNGImage).get should startWith (s"$imageHost/img/uploads/2016/02/04/gu.png?width=700&quality=85&auto=format&fit=max&s=")
  }
