...
class TopMentionsModelTest extends AnyFlatSpec with Matchers {
  ...
  it should "be case insensitive on topic type" in {
    val topics = Seq(
      TestCase("ORG:someEntityValue", Topic(TopMentionsTopicType.Org, "someEntityValue")),
      TestCase("Org:someEntityValue", Topic(TopMentionsTopicType.Org, "someEntityValue")),
      TestCase("Product:someEntityValue", Topic(TopMentionsTopicType.Product, "someEntityValue")),
      TestCase("PRODUCT:someEntityValue", Topic(TopMentionsTopicType.Product, "someEntityValue")),
      TestCase("person:someEntityValue", Topic(TopMentionsTopicType.Person, "someEntityValue")),
    )
