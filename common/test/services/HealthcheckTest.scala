...
abstract class HealthcheckTest(warmupUrl: String) extends FlatSpec with Matchers {
  ...
  // TODO delete after shipping
  "Old Healthchecks" should "pass" in HtmlWithManagement(warmupUrl){ browser =>
    prepareForTest()
  ...
  "Healthchecks" should "pass" in HtmlWithManagement(warmupUrl){ browser =>
    prepareForTest()
    Await.result(WS.url("http://localhost:9000/_healthcheck").get(), 10.seconds).status should be (200)
  }
}
