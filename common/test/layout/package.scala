...
package object layout {
  ...
  val itemLayoutGen: Gen[ItemClasses] = for {
    cls1 <- cardTypeGen
    cls2 <- cardTypeGen
  } yield ItemClasses(cls1, cls2)

  val singleItemGen: Gen[SingleItem] = for {
    layout <- itemLayoutGen
    width <- Gen.choose(1, 4)
  } yield SingleItem(width, layout)

  val rowGen: Gen[Rows] = for {
    layout <- itemLayoutGen
    rows <- Gen.choose(1, 4)
  ...
  val splitColumnGen: Gen[SplitColumn] = for {
    width <- Gen.choose(1, 4)
  ...
  val mpuGen: Gen[MPU] = for {
    width <- Gen.choose(1, 4)
  } yield MPU(width)

  val columnGen: Gen[Column] = Gen.oneOf(singleItemGen, rowGen, splitColumnGen, mpuGen)
