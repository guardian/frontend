...
object RichLinkCleaner extends HtmlCleaner {
  override def clean(document: Document): Document = {
    ...
    val noImagePaths = List("/live/", "/commentisfree/")
    val noStandfirstPaths = List("news/")
    ...
    richLinks
      ...
      .map( richLink => {
        ...
        val showImage = !noImagePaths.exists(href.contains)
        val showStandfirst = !noStandfirstPaths.exists(href.contains)
        val html = views.html.fragments.richLinkDefault(link.text(), href, showImage, showStandfirst).toString()
