...
case class Pagination(currentPage: Int, lastPage: Int, totalContent: Int) {
  ...
  /** Returns the next/prev 3 navigation pages with the current page as close to the center as possible. For first and
    * last 4 pages it shows 4 pages.
    *
    * For 10 pages: Page 1-3: 1 2 3 4 Page 8-10: 7 8 9 10 Page 4-7: 4 5 6
    */
  lazy val pages: Seq[Int] = {
    ...
    /** Make sure that edge cases are handled.
      *
      * If we have only 5 or less pages we want to show whole range. Example:
      *
      * 1 2 3 for 3 pages 1 2 3 4 5 for 5 pages
      *
      * If we have 5+ pages we want to show only 4 pages on the edges Example:
      *
      * 1 2 3 4 ... 10 if we are on the one of the first 3 pages 1 ... 7 8 9 10 if we are on the one of the last 3 pages
  ...
  /** In the case of having just 5 or less pages we want range to be 1-5 In the case of having 5+ pages we want range to
    * be 1-3
    *
