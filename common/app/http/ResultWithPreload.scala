...
trait ResultWithPreload {
  final implicit class RichResult(self: Result) {
    ...
    def withPreload(assets: Seq[PreloadAsset], request: RequestHeader)(implicit context: ApplicationContext): Result = {
      if (assets.nonEmpty) {
        val preloadFiles = assets.map {
          ...
          case cssFile: CssPreloadAsset =>
            s"<${conf.Static(common.Assets.css.projectCss(Some(cssFile.asset.stripSuffix(".css")), request))}>; rel=preload; as=style; nopush"
