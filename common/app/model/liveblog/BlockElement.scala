...
case class EmbedBlockElement(html: Option[String], safe: Option[Boolean], alt: Option[String], caption: Option[String])
    extends BlockElement
case class ContentAtomBlockElement(atomId: String, atomtype: String, role: Option[String], caption: Option[String]) extends BlockElement
...
object BlockElement {
  ...
  def make(element: ApiBlockElement): Option[BlockElement] = {
    ...
    element.`type` match {
      ...
      case Contentatom => element.contentAtomTypeData.map(d => ContentAtomBlockElement(d.atomId, d.atomType, d.role, d.caption))
