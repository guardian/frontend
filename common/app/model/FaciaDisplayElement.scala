...
object FaciaDisplayElement {
  def fromFaciaContentAndCardType(
    ...
    faciaContent.mainVideo match {
      case Some(videoElement) if faciaContent.properties.mediaSelect.showMainVideo =>
        Some(
          InlineVideo(
            videoElement,
      ...
      case _ if faciaContent.properties.mediaSelect.imageSlideshowReplace && itemClasses.canShowSlideshow =>
        InlineSlideshow.fromFaciaContent(faciaContent)
      case _ if faciaContent.properties.mediaSelect.showMainVideo && faciaContent.mainYouTubeMediaAtom.isDefined =>
