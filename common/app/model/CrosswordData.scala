...
case class Entry(
  ...
  lazy val startPosition: String = s"${(position.y + 1).toString}${columnsByLetters(position.x)}"
}
...
case class CrosswordData(
  ...
  val acrossEntries: Seq[Entry] = entries.filter(_.direction == "across")
  val downEntries: Seq[Entry] = entries.filter(_.direction == "down")
}
