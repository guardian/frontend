...
object ImageMedia {
  def make(capiElement: ApiElement, properties: ElementProperties): ImageMedia = ImageMedia(
    allImages = capiElement.assets.filter(_.`type` == "image").map(ImageAsset(_,properties.index)).sortBy(-_.width)
  )
...
object VideoMedia {
  def make(capiElement: ApiElement): VideoMedia = VideoMedia(
    videoAssets = capiElement.assets.filter(_.`type` == "video").map(VideoAsset).sortBy(-_.width)
  )
}
...
object AudioMedia {
  def make(capiElement: ApiElement): AudioMedia = AudioMedia(
    audioAssets = capiElement.assets.filter(_.`type` == "audio").map(AudioAsset)
  )
}
...
object EmbedMedia {
  def make(capiElement: ApiElement): EmbedMedia = EmbedMedia(
    embedAssets = capiElement.assets.filter(_.`type` == "embed").map(EmbedAsset)
  )
}
