...
object TagHistogram {
  def fromTrails(trails: Seq[Trail]): TagHistogram =
    TagHistogram(
      trails.foldLeft(Map.empty[String, Int]) { case (histogram, trail) =>
        trail.tags.tags.map(_.id).toSet.foldLeft(histogram) { case (hist, tagId) =>
  ...
  def fromFaciaContent(faciaContentList: Seq[PressedContent]): TagHistogram =
    TagHistogram(
      faciaContentList.foldLeft(Map.empty[String, Int]) { case (histogram, faciaContent) =>
        faciaContent.frontendTags.map(_.id).toSet.foldLeft(histogram) { case (hist, tagId) =>
          hist + (tagId -> (hist.getOrElse(tagId, 0) + 1))
        }
      },
