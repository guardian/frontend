...
class MatchControllerTest extends FlatSpec with ShouldMatchers {
  ...
  "Match Controller" should "200 when content type is match" in Fake {
    val result = controllers.MatchController.renderMatch("2012", "oct", "20", "derbycounty", "blackburn")(TestRequest())
    status(result) should be(200)
  }

  it should "return JSONP when callback is supplied to match" in Fake {
    val fakeRequest = FakeRequest(GET, "/football/match/2012/oct/20/derbycounty-v-blackburn?callback=foo").withHeaders("host" -> "localhost:9000")
    val result = controllers.MatchController.renderMatch("2012", "oct", "20", "derbycounty-hotspur", "blackburn")(fakeRequest)
    status(result) should be(200)
