...
class CuratedContentAgent(frontJsonFapiLive: FrontJsonFapiLive) extends GuLogging {
  ...
  private val EDITIONS_WITH_CONTAINER = for ((containerId, containerMeta) <- CONTAINERS_WITH_EDITION) yield (containerMeta, containerId)
  ...
  def getTrails(theme: Theme, edition: Edition, isAdFree: Boolean) = if (isAdFree) {
