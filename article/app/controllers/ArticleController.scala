...
case class ArticlePage(article: Article, related: RelatedContent)
...
class ArticleController(
  ...
  private def isSupported(c: ApiContent) =
    c.isArticle || c.isLiveBlog || c.isSudoku
  override def canRender(i: ItemResponse): Boolean =
    i.content.exists(isSupported)
  override def renderItem(
    path: String
  )(implicit request: RequestHeader): Future[Result] =
    mapModel(path, Canonical)(
  ...
  def renderHeadline(path: String): Action[AnyContent] = Action.async {
    implicit request =>
      def responseFromHeadline(headline: Option[String]) = {
        headline
          .map(
            title => Cached(CacheTime.Default)(RevalidatableResult.Ok(title))
          )
          .getOrElse {
  ...
  private def getJson(
    article: ArticlePage
  )(implicit request: RequestHeader): List[(String, Object)] = {
  ...
  private def getGuuiJson(article: ArticlePage, blocks: Blocks)(
    implicit request: RequestHeader
  ): String = {
    val pageType: PageType = PageType(article, request, context)
    DCRDataModel.toJson(
      DotcomponentsDataModel.fromArticle(article, request, blocks, pageType)
    )
  }

  private def render(path: String, article: ArticlePage, blocks: Blocks)(
    implicit request: RequestHeader
  ): Future[Result] = {
    ...
    request.getRequestFormat match {
      case JsonFormat if request.forceDCR =>
        Future.successful(
          common
            .renderJson(getGuuiJson(article, blocks), article)
            .as("application/json")
        )
      case JsonFormat =>
      ...
      case EmailFormat =>
        Future.successful(
          common.renderEmail(ArticleEmailHtmlPage.html(article), article)
        )
  ...
  private def responseToModelOrResult(
    ...
    ModelOrResult(supportedContent, response) match {
      case Left(article: Article) =>
        Left(
          (
            ArticlePage(article, StoryPackages(article.metadata.id, response)),
            blocks
          )
        )
