...
class LiveBlogController(
  ...
  def renderEmail(path: String): Action[AnyContent] = {
    Action.async { implicit request =>
      mapModel(path, ArticleBlocks, maybeTopic = None) {
  ...
  def renderArticle(
    ...
    Action.async { implicit request =>
      ...
      val topMentions = if (filter) None else getTopics(path, topics)
  ...
  def renderJson(
    ...
    Action.async { implicit request: Request[AnyContent] =>
      val filter = shouldFilter(filterKeyEvents)
      val maybeTopic = getTopics(path, topics)
      val range = getRange(lastUpdate, page, maybeTopic)
      val availableTopics = topicService.getAvailableTopics(path)

      mapModel(path, range, filter, maybeTopic) {
        ...
        case (blog: LiveBlogPage, blocks) =>
          getJson(blog, range, isLivePage, filter, blocks.requestedBodyBlocks.getOrElse(Map.empty), maybeTopic)
        case (minute: MinutePage, _) =>
  ...
  private[this] def renderWithRange(
    ...
    mapModel(path, range, filterKeyEvents, maybeTopic) { (page, blocks) =>
      {
  ...
  private[this] def getRange(
    ...
    (lastUpdate.map(ParseBlockId.fromBlockId), page.map(ParseBlockId.fromPageParam), maybeTopic) match {
  ...
  private[this] def getNewBlocks(
    ...
    if (filterKeyEvents) {
      ...
      latestBlocks.filter(block => maybeTopic.get.blocks.contains(block.id))
    } else latestBlocks
  ...
  private[this] def getNewBlocks(
    ...
    if (filterKeyEvents) {
      ...
      blocksAround.filter(block => maybeTopic.get.blocks.contains(block.id))
    } else blocksAround
  }
  ...
  private[this] def renderNewerUpdatesJson(
    ...
    val newBlocks = getNewBlocks(page, lastUpdateBlockId, filterKeyEvents, maybeTopic)
    val newCapiBlocks = getNewBlocks(requestedBodyBlocks, lastUpdateBlockId, filterKeyEvents, maybeTopic)
  ...
  private[this] def mapModel(
    ...
    capiLookup
      ...
      .map(responseToModelOrResult(range, filterKeyEvents, maybeTopic))
  ...
  def getTopics(blogId: String, maybeTopic: Option[String]) = {
    val topics = for {
      selectedTopic <- SelectedTopic.fromString(maybeTopic)
      topics <- topicService.getSelectedTopic(blogId, selectedTopic)
    } yield topics

    topics match {
      case Some(_) => log.info(s"topic was successfully retrieved for ${maybeTopic.get}")
      case None    => if (maybeTopic.isDefined) log.warn(s"topic result couldn't be retrieved for ${maybeTopic.get}")
    }
