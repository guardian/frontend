...
class LiveBlogController(
  ...
  private def checkIfSupported(blog: LiveBlogPage): Boolean = {
    isDeadBlog(blog) && isSupportedTheme(blog) && isNotRecent(blog) && !inDenyList(blog) && isNotAustraliaNewsBlog(blog)
  }
  ...
  private[this] def responseToModelOrResult(
    ...
    val content = supportedContentResult.left.flatMap {
      ...
      case liveBlog: Article if liveBlog.isLiveBlog =>
        createLiveBlogModel(liveBlog, response, range, filterKeyEvents).left
          .map(_ -> blocks)
      case unknown => {
