...
@DoNotDiscover class MembershipAccessTest
    extends FeatureSpec
    with GivenWhenThen
    with Matchers
    with BeforeAndAfter
  ...
  feature("Membership Access") {
    ...
    scenario(
      "Membership access classname should not be set when the article does not require membership access",
      ArticleComponents,
    ) {

      Given("the article does not require membership access")

      goTo("/membership/2015/apr/23/irvine-welsh-trainspotting-jesse-armstrong-peep-show-how-to-write-comedy") {
        browser =>
          import browser._
