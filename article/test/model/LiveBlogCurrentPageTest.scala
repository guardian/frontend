...
class LiveBlogCurrentPageTest extends AnyFlatSpec with Matchers {
  ...
  "firstPage" should "allow 1 block on one page" in {
    ...
    result should be(
      Some(
        LiveBlogCurrentPage(
          currentPage = FirstPage(Seq(fakeBlock(1)), filterKeyEvents = false, maybeTopic = None),
          pagination = None,
          pinnedBlock = None,
        ),
      ),
  ...
  it should "display a page with no blocks when in requestedBodyBlocks key events and summary lists are empty and filter is on" in {
    ...
    should(result, currentPage = FirstPage(List(), true, maybeTopic = None), pagination = None)
  }
  ...
  it should "allow 3 blocks on one page" in {
    ...
    should(result, FirstPage(blocks, filterKeyEvents = false, maybeTopic = None), None)
  }
  ...
  it should "put 4 blocks on two pages (main page)" in {
    ...
    val expectedOldestPage =
      BlockPage(blocks = Nil, blockId = "1", pageNumber = 2, filterKeyEvents = false, maybeTopic = None)
    val expectedOlderPage =
      BlockPage(blocks = Nil, blockId = "2", pageNumber = 2, filterKeyEvents = false, maybeTopic = None)
  ...
  it should "put 5 blocks on two pages (main page)" in {
    ...
    val expectedCurrentPage = FirstPage(blocks = blocks.take(3), filterKeyEvents = false, maybeTopic = None)
    val expectedOldestPage =
      BlockPage(blocks = Nil, blockId = "1", pageNumber = 2, filterKeyEvents = false, maybeTopic = None)
    val expectedOlderPage =
      BlockPage(blocks = Nil, blockId = "2", pageNumber = 2, filterKeyEvents = false, maybeTopic = None)
  ...
  it should "only display key events and summaries" in {
    ...
    val expectedCurrentPage = FirstPage(blocks = keyBlocks.take(3), filterKeyEvents = true, maybeTopic = None)
    val expectedOldestPage =
      BlockPage(blocks = Nil, blockId = "1", pageNumber = 3, filterKeyEvents = true, maybeTopic = None)
    val expectedOlderPage =
      BlockPage(blocks = Nil, blockId = "5", pageNumber = 2, filterKeyEvents = true, maybeTopic = None)
    val expectedPagination = Some(
      N1Pagination(
        newest = None,
        newer = None,
  ...
  "findPageWithBlock" should "put 4 blocks on two pages - older page link" in {
    ...
    val expectedNewestPage = FirstPage(blocks.take(2), filterKeyEvents = false, maybeTopic = None)
    val expectedPagination = Some(
      N1Pagination(
  ...
  it should "put 5 blocks on two pages (block 3 from oldest page)" in {
    ...
    val expectedNewestPage = FirstPage(blocks.take(3), filterKeyEvents = false, maybeTopic = None)
    val expectedPagination = Some(
      N1Pagination(
        newest = Some(expectedNewestPage),
        newer = Some(expectedNewestPage),
  ...
  it should "put 6 blocks on 3 pages (main page)" in {
    ...
    val expectedCurrentPage = FirstPage(blocks = blocks.take(2), filterKeyEvents = false, maybeTopic = None)
    val expectedMiddlePage =
      BlockPage(blocks = Nil, blockId = "4", pageNumber = 2, filterKeyEvents = false, maybeTopic = None)
    val expectedOldestPage =
      BlockPage(blocks = Nil, blockId = "1", pageNumber = 3, filterKeyEvents = false, maybeTopic = None)
    val expectedPagination = Some(
      N1Pagination(
        newest = None,
        newer = None,
  ...
  it should "put 6 blocks on 3 pages (middle page)" in {
    ...
    val expectedFirstPage = FirstPage(blocks = blocks.take(2), filterKeyEvents = false, maybeTopic = None)
    val expectedOlderPage =
      BlockPage(
        blocks = blocks.takeRight(2),
        blockId = "2",
  ...
  it should "display only key events and summaries when the filter is on" in {
    ...
    val expectedFirstPage =
      FirstPage(blocks = keyAndSummaryBlocks.take(2), filterKeyEvents = true, maybeTopic = None)
    val expectedMiddlePage =
      BlockPage(
