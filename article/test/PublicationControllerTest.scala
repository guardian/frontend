...
@DoNotDiscover class PublicationControllerTest
  ...
  "Publication Controller" should "redirect to an /all page when an observer dated book url is requested" in {
    when(bookAgent.getTags("theobserver"))
      .thenReturn(Seq(new TagDefinition("Observer Magazine", "theobserver/magazine", None, false)))
  ...
  it should "redirect to an /all page when an observer dated book section url is requested" in {
    ...
    when(bookSectionAgent.getTags("theobserver"))
      .thenReturn(Seq(new TagDefinition("News", "theobserver/news/uknews", None, false)))
  ...
  it should "200 when an observer article is requested" in {
    ...
    val result = publicationController.publishedOn(
      "theobserver",
      "2014",
      "aug",
      "31",
      "profile-clare-smyth-gordon-ramsay-chef-perfect-ten",
    )(testReq)
  ...
  it should "200 when an observer gallery is requested" in {
    ...
    val result =
      publicationController.publishedOn("theobserver/gallery", "2013", "sep", "14", "the-10-best-fonts")(testReq)
    status(result) should be(OK)
  }
  ...
  it should "redirect to an /all page when a guardian dated newspaper book url is requested" in {
    when(bookAgent.getTags("theguardian"))
      .thenReturn(Seq(new TagDefinition("Main section", "theguardian/mainsection", None, false)))
  ...
  it should "redirect to an /all page when a guardian dated newspaper book section url is requested" in {
    ...
    when(bookSectionAgent.getTags("theguardian"))
      .thenReturn(Seq(new TagDefinition("Comment & features", "theguardian/g2/features", None, false)))
  ...
  it should "200 when a guardian article is requested" in {
    ...
    val result =
      publicationController.publishedOn("theguardian", "2012", "aug", "24", "good-to-meet-you-zubair-limbada")(testReq)
    status(result) should be(OK)
  }
  ...
  it should "200 when a guardian gallery is requested" in {
    ...
    val result =
      publicationController.publishedOn("theguardian/gallery", "2009", "apr", "01", "prince-harry-william-monarchy")(
        testReq,
      )
    status(result) should be(OK)
  }
  ...
  it should "200 when a guardian dated blog article is requested" in {
    ...
    val result = publicationController.publishedOn(
      "theguardian/from-the-archive-blog",
      "2012",
      "feb",
      "17",
      "charlie-chaplin-1952-communist",
    )(testReq)
    status(result) should be(OK)
  }
