                ...
                var formData = $form.data('formData'),
                    submissionMessage = {
                        ...
                        submissionMessage: isSuccess
                            ? formData.customSuccessDesc ||
                                  messages.defaultSuccessDesc
                            : 'Please try again.',
            ...
            ui = {
                updateForm: function(thisRootEl, el, analytics, opts) {
                    var formData = $(thisRootEl).data(),
                        ...
                        formSuccessDesc =
                            (opts && opts.formSuccessDesc) ||
                            formData.formSuccessDesc,
                        removeComforter =
                    ...
                    fastdom.write(function() {
                        if (formDisplayNameNormalText) {
                            $(
                                '.js-email-sub__display-name-normal-text',
                                el
                            ).text(formDisplayNameNormalText);

                            if (formDisplayNameAccentedText) {
                                $(
                                    '.js-email-sub__display-name-accented-text',
                                    el
                                ).text(formDisplayNameAccentedText);
                            }
                        } else if (formTitle) {
                        ...
                        if (formDescription) {
                            $('.js-email-sub__description', el).text(
                                formDescription
                            );
                        }
            ...
            formSubmission = {
                bindSubmit: function($form, analytics) {
                    ...
                    bean.on(
                        $form[0],
                        'submit',
                        this.submitForm($form, url, analytics)
                    );
                },
                        ...
                        if (!state.submitting && validate(emailAddress)) {
                            ...
                            return new Promise(function() {
                                if (formData.customSuccessEventName) {
                                    mediator.emit(
                                        formData.customSuccessEventName
                                    );
                                }
                                googleAnalytics.trackNonClickInteraction(
