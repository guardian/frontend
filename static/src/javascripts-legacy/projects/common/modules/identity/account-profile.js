...
define(
    ...
    function($, bean, bonzo, url, config, avatarApi) {
                ...
                init: function() {
                    ...
                    if (self.accountProfileForms) {
                        ...
                        require.ensure(
                            ...
                            function(require) {
                                require(
                                    'bootstraps/enhanced/membership'
                                ).init();
                            },
                            'membership'
                        );
        ...
        accountProfile.prototype.handleTabsClick = function(event) {
            ...
            if (event.target.nodeName.toLowerCase() === 'a') {
                if (self.unsavedChangesForm) {
                    ...
                    if (
                        !self.unsavedChangesForm.querySelector(
                            self.classes.formError
                        )
                    ) {
                        // Append error message
                        bonzo(self.unsavedChangesForm).prepend(
                            self.genUnsavedError()
                        );
    ...
    */
        ...
        accountProfile.prototype.prependMessage = function(
            message,
            location,
            clazz
        ) {
        ...
        accountProfile.prototype.prependErrorMessage = function(
            message,
            location
        ) {
        ...
        accountProfile.prototype.prependSuccessMessage = function(
            message,
            location
        ) {
    ...
    */
        accountProfile.prototype.genUnsavedError = function() {
            ...
            for (i = 0; i < this.unsavedFields.length; i++) {
                ...
                text = this.accountProfileForms.querySelector(
                    '[for="' + labelId + '"]'
                ).innerHTML;
            ...
            return (
                errorDivStart + errorMessageStart + errorSaveLink + errorDivEnd
            );
        };
