...
define(
    ['bean', 'lib/$', 'common/modules/identity/api', 'lodash/objects/assign'],
    function(bean, $, idApi, assign) {
        function Formstack(el, formstackId, config) {
            ...
            config = assign(
                {
                    ...
                    fsSelectors: {
                        ...
                        sectionHeaderFirst:
                            '.fsSection:first-child .fsSectionHeading',
            ...
            self.dom = function(user) {
                ...
                dom.$form = $(config.fsSelectors.form).addClass(
                    config.idClasses.form
                );
                $('link', el).remove();
            ...
            self.submit = function(event) {
                ...
                setTimeout(function() {
                    ...
                    if (
                        $(config.fsSelectors.formError, dom.$form).length === 0
                    ) {
                        dom.$form[0].submit();
                    }
                }, 100);
