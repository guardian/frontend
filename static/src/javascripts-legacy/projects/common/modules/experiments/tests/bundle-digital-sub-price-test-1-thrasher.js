    ...
    return function() {
        ...
        this.setLink = function(variant) {
            if (this.thrasher()) {
                ...
                if (linkEl && linkEl.getAttribute('href')) {
                    linkEl.setAttribute('href', config.page.membershipUrl + '/bundles?INTCMP=BUNDLE_PRICE_TEST_1M_T_' + config.page.edition.toUpperCase() + '_' + variant.toUpperCase());
                }
            }
        };
        ...
        this.variants = [
            ...
            },
            {
                id: 'B',
                test: function() {
                    self.setup('B');    // Band 'B' - current default +20%
                },
                success: this.completeFunc
            },
            {
                id: 'C',
                test: function() {
                    self.setup('C');    // Band 'C' - current default -20%
                },
                success: this.completeFunc
            }
        ];
