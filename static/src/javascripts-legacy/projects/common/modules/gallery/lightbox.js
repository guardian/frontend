define(
    [
        ...
        'raw-loader!common/views/content/endslate.html',
        'raw-loader!common/views/content/loader.html',
        'raw-loader!common/views/content/share-button.html',
    ...
    function(
        ...
        GalleryLightbox.prototype.generateImgHTML = function(img, i) {
            ...
            return template(blockSharingTpl.replace(/^\s+|\s+$/gm, ''), {
                ...
                shareButtons: map(
                    shareItems,
                    template.bind(null, shareButtonTpl)
                ).join(''),
        ...
        GalleryLightbox.prototype.initSwipe = function() {
            ...
            bean.on(
                this.$swipeContainer[0],
                'touchmove',
                throttle(touchMove, updateTime, { trailing: false })
            );
        ...
        GalleryLightbox.prototype.loadSurroundingImages = function(
            index,
            count
        ) {
        ...
        GalleryLightbox.prototype.translateContent = function(
            ...
            contentEl.style.mozTransform =
                'translate(' + (px + offset) + 'px,0)';
            contentEl.style.msTransform =
                'translate(' + (px + offset) + 'px,0)';
            ...
            image: {
                enter: function() {
                    ...
                    this.translateContent(
                        this.index,
                        0,
                        this.useSwipe && detect.isBreakpoint({ max: 'tablet' })
                            ? 100
                            : 0
                    );
                ...
                events: {
                    ...
                    'toggle-info': function() {
                        ...
                        this.$lightboxEl.toggleClass(
                            'gallery-lightbox--show-info'
                        );
                    },
                    'hide-info': function() {
                        this.pulseButton(this.infoBtn);
                        this.$lightboxEl.removeClass(
                            'gallery-lightbox--show-info'
                        );
                    },
                    'show-info': function() {
                        this.pulseButton(this.infoBtn);
                        this.$lightboxEl.addClass(
                            'gallery-lightbox--show-info'
                        );
                    },
                    resize: function() {
                        this.swipeContainerWidth = this.$swipeContainer.dim().width;
                        this.loadSurroundingImages(
                            this.index,
                            this.images.length
                        ); // regenerate src
                    ...
                    bean.on(
                        ...
                        function(e) {
                            ...
                            var $el = bonzo(e.currentTarget),
                                galleryHref =
                                    $el.attr('href') ||
                                    $el.attr('data-gallery-url'),
                                galleryHrefParts = galleryHref.split('#img-'),
                                parsedGalleryIndex = parseInt(
                                    galleryHrefParts[1],
                                    10
                                ),
