...
define(
    ...
    function(
        ...
        function getHistory() {
            historyCache =
                historyCache || storage.local.get(storageKeyHistory) || [];
            return historyCache;
        }
                        ...
                        if (record[bucket.indexInRecord]) {
                            var visits = record[bucket.indexInRecord]
                                .map(function(day) {
                                    ...
                                    return newAge < summaryPeriodDays &&
                                        newAge >= 0
                                        ? [newAge, day[1]]
                                        : false;
                                })
        ...
        function getTopNavItems() {
            topNavItemsCache =
                topNavItemsCache ||
                $('.js-navigation-header .js-top-navigation a').map(function(
                    item
                ) {
        ...
        function removeFromMegaNav() {
            getMegaNav().each(function(megaNav) {
                fastdom.write(function() {
                    $(
                        '.js-global-navigation__section--history',
                        megaNav
                    ).remove();
                });
            });
