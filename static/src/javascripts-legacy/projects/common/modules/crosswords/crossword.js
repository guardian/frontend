        ...
        var Crossword = React.createClass({
                ...
                mediator.on('window:throttledScroll', function() {
                    ...
                    fastdom.write(function() {
                        ...
                        if (scrollYPastGame >= 0) {
                            var gridOffsetBottom =
                                gridOffset.top + gridOffset.height;

                            if (
                                scrollY >
                                gridOffsetBottom -
                                    stickyClueWrapperOffset.height
                            ) {
                                $stickyClueWrapper
                                    .css('top', 'auto')
                                    .css('bottom', 0);
            ...
            onSelect: function(x, y) {
                ...
                if (cellInFocus && cellInFocus.x === x && cellInFocus.y === y) {
                    ...
                    newDirection = helpers.otherDirection(
                 ...
                 */
                    if (
                        !isStartOfClue(clue.across) &&
                        isStartOfClue(clue.down)
                    ) {
                        newDirection = 'down';
                    } else if (clue.across) {
            ...
            setGridHeight: function() {
                if (!this.$gridWrapper) {
                    this.$gridWrapper = $(
                        React.findDOMNode(this.refs.gridWrapper)
                    );
                }
