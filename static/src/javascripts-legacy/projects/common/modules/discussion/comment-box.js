 ...
 */
        ...
        CommentBox.prototype.showOnboarding = function(e) {
            ...
            if (
                ...
                if (this.options.hasUsername) {
                    this.getElem('onboarding-username').classList.add(
                        'is-hidden'
                    );
                }
            }
        ...
        CommentBox.prototype.postComment = function() {
            ...
            var validEmailCommentSubmission = function() {
                ...
                if (self.errors.length === 0) {
                    if (
                        self.options.newCommenter &&
                        !self.options.hasUsername
                    ) {
 ...
 */
        CommentBox.prototype.fail = function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                this.error(
                    'API_ERROR',
                    this.errorMessages.API_ERROR + xhr.status
                ); // templating would be ideal here
            }
        };

        CommentBox.prototype.onboardingPreviewSuccess = function(
            comment,
            resp
        ) {
            this.getElem('onboarding-preview-body').innerHTML =
                resp.commentBody;
        };
