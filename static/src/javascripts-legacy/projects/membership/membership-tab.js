    ...
    function(bean, $, fetch, config, reportError, formatters, stripe) {
        ...
        function populateUserDetails(userDetails) {
            ...
            if (userDetails.subscription.card) {
                $(CHANGE_TIER_CARD_LAST4).text(
                    userDetails.subscription.card.last4
                );
            } else if (userDetails.subscription.payPalEmail) {
                $(PAYPAL_EMAIL_ADDRESS).text(
                    userDetails.subscription.payPalEmail
                );
            }
            ...
            if (userDetails.subscription.nextPaymentDate) {
                $(PACKAGE_NEXT_PAYMENT_DATE).text(
                    formatters.formatDate(
                        userDetails.subscription.nextPaymentDate
                    )
                );
            ...
            if (userDetails.regNumber) {
                $(DETAILS_MEMBER_NUMBER_CONTAINER).removeClass(
                    IS_HIDDEN_CLASSNAME
                );
