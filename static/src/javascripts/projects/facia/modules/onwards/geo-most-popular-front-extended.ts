...
export class GeoMostPopularFrontExtended extends Component {
  ...
  prerender(): void {
    ...
    if (this.parent) {
       ...
       */
      const mostPopularTabInClone = qwery('.js-tab-2 .most-popular', mostReadClone)[0];

      if (!isInternational && // we don't update the most popular list for international fronts
      latestMostPopularTab && mostPopularTabInClone && mostPopularTabInClone.parentNode) {
        // Replace mostPopularTabInClone with latestMostPopularTab
        mostPopularTabInClone.parentNode.replaceChild(latestMostPopularTab, mostPopularTabInClone);
      }
      ...
      if (latestMostCards && mostCardsInClone && mostCardsInClone.parentNode) {
        // Replace mostCardsInClone with latestMostCards
        mostCardsInClone.parentNode.replaceChild(latestMostCards, mostCardsInClone);
      }

      const latestPopularTrails = qwery('#popular-trails', mostReadClone)[0];
  ...
  ready(): void {
    const isNetworkFront = config.get('page.contentType') === 'Network Front';
