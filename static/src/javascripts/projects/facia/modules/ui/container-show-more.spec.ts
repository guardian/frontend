...
describe('Container Show More', () => {
  ...
  beforeEach(() => {
    $container = bonzo(bonzo.create(`<div>${itemWithId('loldongs')}${itemWithId('corgi')}${itemWithId('geekpie')}</div>`));
  });
  ...
  it('should be able to group elements by id', () => {
    ...
    expect(Object.keys(grouped).filter(i => new Set(['loldongs', 'corgi', 'geekpie']).has(i)).length === 3).toBeTruthy();
  });

  it('should de-duplicate items loaded in', () => {
    const $after = dedupShowMore($container, `<div>${itemWithId('corgi')}${itemWithId('daschund')}</div>`);
