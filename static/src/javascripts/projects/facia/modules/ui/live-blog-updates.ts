...
const maybeAnimateBlocks = (el: Element, container: Element, immediate?: boolean): Promise<boolean> => fastdomPromise.read(() => el.getBoundingClientRect().top).then(vPosition => {
  const isVisible = vPosition > 0 && vPosition < viewportHeightPx;

  if (isVisible) {
    timeoutPromise(immediate ? 0 : animateDelayMs).then(() => fastdomPromise.write(() => el.classList.remove('fc-item__liveblog-blocks__inner--offset')));
...
const applyUpdate = (container: Element, content: Array<Element>): Promise<void> => fastdomPromise.write(() => {
...
const showUpdatesFromLiveBlog = (): Promise<void> => fastdomPromise.read(() => {
