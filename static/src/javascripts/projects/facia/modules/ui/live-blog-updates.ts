...
const renderBlock = (articleId: string, block: Block, index: number): string => {
  ...
  return template(blockTemplate)({
    ...
    text: [block.title, block.body.slice(0, 500)].filter(x => x != null).join('. '),
    index: index + 1
  });
};

const timeoutPromise = (delay: number): Promise<void> => new Promise(resolve => setTimeout(resolve, delay));

const maybeAnimateBlocks = (el: Element, container: Element, immediate?: boolean): Promise<boolean> => fastdomPromise.measure(() => el.getBoundingClientRect().top).then(vPosition => {
  const isVisible = vPosition > 0 && vPosition < viewportHeightPx;

  if (isVisible) {
    timeoutPromise(immediate ? 0 : animateDelayMs).then(() => fastdomPromise.mutate(() => el.classList.remove('fc-item__liveblog-blocks__inner--offset')));
  ...
  maybeAnimateBlocks(el, container).then(didAnimate => {
    if (!didAnimate) {
      const animateOnScroll = debounce(() => {
        maybeAnimateBlocks(el, container, true).then(didAnimateOnScroll => {
...
const applyUpdate = (container: Element, content: Array<Element>): Promise<void> => fastdomPromise.mutate(() => {
  bonzo(container).empty().append(content);
});

const startUpdate = (container: Element, content: Array<Element>, shouldTransitionOut: boolean): Promise<void> => {
  if (shouldTransitionOut) {
    ...
    return timeoutPromise(newUpdateTransitionDelayMs).then(() => applyUpdate(container, content));
  }

  return applyUpdate(container, content);
};

const completeUpdate = (container: Element, content: Array<Element>, shouldTransitionIn: boolean): void => {
  if (shouldTransitionIn) {
    animateBlocks(content[0], container);
  }
};

const isDynamic = (element: Element): boolean => element.classList.contains(dynamicClass);
...
const showBlocks = (articleId: string, targets: Array<Element>, blocks: Array<Block>, oldBlockDate: number): void => {
  const fakeUpdate = isUndefined(oldBlockDate);

  targets.forEach(element => {
    let hasNewBlock = false;

    const wrapperClasses = ['fc-item__liveblog-blocks__inner', 'u-faux-block-link__promote'];

    const blocksHtml = blocks.slice(0, maxBlockCount).map((block, index) => {
    ...
    const el = bonzo.create(`<div class="${wrapperClasses.join(' ')}">${blocksHtml.join('')}</div>`);

    startUpdate(element, el, hasNewBlock).then(() => completeUpdate(element, el, hasNewBlock));
  });
};

const sanitizeBlocks = (blocks: Array<Block>): Array<Block> => blocks.filter(block => block.id && block.publishedDateTime && block.body && block.body.length >= 10);

const showUpdatesFromLiveBlog = (): Promise<void> => fastdomPromise.measure(() => {
  ...
  $(selector).each(element => {
    ...
    if (articleId) {
      const elementsForArticle = elementsById.get(articleId) || [];
