...
describe('Container Toggle', () => {
  ...
  beforeEach(() => {
    container = bonzo.create(`<section class="fc-container js-container--toggle fc-container__will-have-toggle" data-id="${containerId}">` + `<div class="fc-container__header js-container__header">` + `<h2>A container</h2>` + `</div>` + `</section>`);
  ...
  it('should remove "fc-container--will-have-toggle" class from container', done => {
    ...
    fastdom.defer(1, () => {
      expect($container.hasClass('fc-container--will-have-toggle')).toBeFalsy();
      done();
    });
  });
