...
const readDisplayPrefForContainer = (containerId: string): DisplayState => {
  ...
  return prefs && prefs[containerId] ? DISPLAY_STATE.displayed : DISPLAY_STATE.hidden;
};

const updateDisplayPrefForContainer = (containerId: string, state: DisplayState): void => {
  const prefs = userPrefs.get(PREF_NAME, {
    type: 'session'
  }) || {};
...
const dedupShowMore = ($container: bonzo, html: string): bonzo => {
  ...
  $(ITEM_SELECTOR, $html).each(article => {
    ...
    if ($article.attr(ARTICLE_ID_ATTRIBUTE) in seenArticles || (isAdFreeUser() && articleClass && articleClass.includes('paid-content'))) {
      $article.remove();
    }
  });
...
class Button {
  ...
  constructor(el: bonzo, container: bonzo) {
    ...
    this.messages = new Map([[DISPLAY_STATE.hidden, $('.js-button-text', this.$el).text()], [DISPLAY_STATE.displayed, 'Less'], [DISPLAY_STATE.loading, 'Loading&hellip;']]);
  }

  setState(state: DisplayState): void {
    this.$textEl.html(this.messages.get(state));
    this.$el.attr('data-link-name', state === DISPLAY_STATE.displayed ? 'less' : 'more').toggleClass('button--primary', state !== DISPLAY_STATE.displayed).toggleClass('button--tertiary', state === DISPLAY_STATE.displayed).toggleClass(BUTTON_SPINNER_CLASS, state === DISPLAY_STATE.loading);
    this.$container.toggleClass(HIDDEN_CLASS_NAME, state !== DISPLAY_STATE.displayed).toggleClass(VISIBLE_CLASS_NAME, state === DISPLAY_STATE.displayed);
    this.state = state;
  }
  ...
  loadShowMoreForContainer(): void {
    ...
    loadShowMore(config.get('page.pageId'), this.id).then(response => {
      let dedupedShowMore;
  ...
  hideErrorMessage(): void {
    fastdom.mutate(() => {
      if (this.$errorMessage != null) {
        this.$errorMessage.addClass('show-more__error-message--invisible');
      }
    });
  }
  ...
  showErrorMessage(): void {
    ...
    this.$errorMessage = bonzo(bonzo.create('<div class="show-more__error-message">' + 'Sorry, failed to load more stories. Please try again.' + '</div>'));
...
const showMore = (button: Button): void => {
  fastdom.mutate(() => {
     ...
     * DOM nodes.
     */
    button.setState(button.state === DISPLAY_STATE.hidden ? DISPLAY_STATE.displayed : DISPLAY_STATE.hidden);
...
const renderToDom = (button: Button): void => {
  fastdom.mutate(() => {
    button.$container.addClass(HIDDEN_CLASS_NAME).removeClass('js-container--fc-show-more').toggleClass(HIDDEN_CLASS_NAME, button.state === DISPLAY_STATE.hidden);
...
export const init = (): void => {
  fastdom.measure(() => {
    ...
    mediator.on('module:clickstream:click', clickSpec => {
      const clickedButton = buttons.find(button => button.$el[0] === clickSpec.target);
      if (clickedButton && clickedButton.state !== DISPLAY_STATE.loading) {
