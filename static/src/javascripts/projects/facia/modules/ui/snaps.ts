...
const bindIframeMsgReceiverOnce = once(() => {
  bean.on(window, 'message', event => {
    const iframe = snapIframes.find(snapIframe => snapIframe.contentWindow === event.source);
...
const setSnapPoint = (el: HTMLElement, isResize: boolean): void => {
  ...
  fastdom.write(() => {
    breakpoints.map((breakpoint, i, arr) => {
      ...
      return breakpoint;
    }).filter(breakpoint => breakpoint.action).forEach(breakpoint => {
...
const injectIframe = (el: HTMLElement): void => {
  ...
  const containerEl = bonzo.create(`<div style="width: 100%; height: ${height}px; overflow: hidden; -webkit-overflow-scrolling:touch"></div>`)[0];
...
const init = (): void => {
  ...
  const inlinedSnaps = Array.from(document.querySelectorAll('.facia-snap-embed'));
  inlinedSnaps.forEach(initInlinedSnap);

  // Second, init non-inlined embeds.
  const snaps = Array.from(document.querySelectorAll('.js-snappable.js-snap')).filter(el => {
