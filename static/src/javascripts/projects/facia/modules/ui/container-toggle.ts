...
export class ContainerToggle {
  ...
  setState(newState: ToggleState): void {
    ...
    fastdom.write(() => {
      ...
      this.$container[this.state === 'displayed' ? 'removeClass' : 'addClass']('fc-container--rolled-up');
      // data-link-name is inverted, as happens before clickstream
      this.$button.attr('data-link-name', toggleText[this.state === 'displayed' ? 'hidden' : 'displayed']);
  ...
  addToggle(): void {
    ...
    fastdom.write(() => {
      ...
      this.$container.removeClass('js-container--toggle').removeClass('fc-container--will-have-toggle').addClass('fc-container--has-toggle');
    ...
    mediator.on('module:clickstream:click', clickSpec => {
      if (clickSpec.target === this.$button[0]) {
        this.setState(this.state === 'displayed' ? 'hidden' : 'displayed');
        this.updatePref(id);
      }
    });
  }
