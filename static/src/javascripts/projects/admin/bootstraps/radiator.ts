...
const initRadiator = (): void => {
  ...
  window[callback] = deployments => {
    ...
    deployments.response.results.filter(deployment => deployment.projectName.startsWith('dotcom:')).forEach(deploy => {
      const {
        project,
        stage
      } = deploy;
      if (stage && latestDeployments[stage] && !Object.keys(latestDeployments[stage]).includes(project)) {
    ...
    const renderDeployer = (stage, revision, deployer) => {
      ...
      if (!document.getElementById(targetId)) {
        ...
        if (list) {
          ...
          fetchJson(`//${window.location.host}/radiator/commit/${revision}`).then(rev => {
            if (rev.commit) {
    ...
    const renderDeploys = (stage, target) => {
      const sortedDeployments = Object.keys(latestDeployments[stage]).sort((firstDeployment, secondDeployment) => {
  ...
  fetchJson(`//${window.location.host}/ophan/pageviews`).then(data => {
    ...
    // Build Graph
    const builtGraphData = Object.keys(todayData).reduce((graphData, timestamp) => {
    ...
    new window.google.visualization.LineChart(document.getElementById('pageviews')).draw(window.google.visualization.arrayToDataTable(builtGraphData), {
      title: 'Page views',
      backgroundColor: '#fff',
      colors: ['#e6711b'],
      height: 160,
    ...
    const lastOphanEntry = Object.keys(todayData).map(key => todayData[key]).pop().reduce((memo, entry) => entry.count + memo, 0);
    const viewsPerSecond = Math.round(lastOphanEntry / 60);
    const viewsPerSecondElem = document.querySelector('.pageviews-per-second');
