...
const typewrite = (message: string, $target: Element) => new Promise(yay => {
  let i = 0;
...
const timeout = (to: number, $target: Element) => new Promise(yay => {
  const target = Date.now() + to * 1000;
  const loop = () => requestAnimationFrame(() => {
    const diff = target - Date.now();
    $target.innerText = `00:00:${Math.ceil(diff / 1000).toString().padStart(2, '0')}:${(diff % 1000).toString().padStart(3, '0')}`;
    if (diff > 0) {
      loop();
    } else {
...
const writeTextLines = ($repl: Element): Promise<void> => {
  const text = [LineHead('System online'), Line('User authenticated'), Line('... Initiating launch routine'), Line('OK'), Line('Launch routine initiated'), LineHead('Action required'), Line('Confirm launch procedure'), Line('Input launch passphrase now')];
  return text.reduce((promiseChain, currentTask) => promiseChain.then(() => {
    const {
      $wrapper,
      type
    } = currentTask;
...
const awaitForFormInput = ({
  $repl
}): Promise<void> => new Promise(yay => {
...
const start = ($switchboard: HTMLFormElement, $holder: Element) => {
  ...
  setTimeout(() => {
    writeTextLines($repl).then(() => awaitForFormInput({ $repl })).then(() => showSuccessMsg({ $bg, $drama })).then(() => showCountdownMsg({ $drama })).then(() => {
      $holder.remove();
