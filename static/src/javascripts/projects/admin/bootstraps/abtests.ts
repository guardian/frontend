...
import { concurrentTests, epicTests, priorityEpicTest } from "common/modules/experiments/ab-tests";
import { isExpired } from "lib/time-utils";

const renderTests = (tests: ReadonlyArray<ABTest>, active: boolean, elem: Element | null | undefined): Array<any> => {
...
const initABTests = (): void => {
  ...
  renderTests(tests.filter(test => !isExpired(test.expiry)), true, document.querySelector('.abtests-report__data'));

  const expiredTestItems = renderTests(concurrentTests.filter(test => isExpired(test.expiry)), false, document.querySelector('.abtests-expired'));
