...
type ObserverRecord = {
  ...
  registry: Array<{callback: (arg0: number) => void;element: Element;}>;
};

const cache: Map<number, ObserverRecord> = new Map();

const observe = (element: Element, threshold: number, callback: (arg0: number) => void): void => {
  let record = cache.get(threshold);
  if (!record) {
    record = {
      registry: [{ callback, element }],
      observer: new window.IntersectionObserver((entries: window.IntersectionObserverEntry[]) => {
...
const unobserve = (element: Element, threshold: number, callback: (arg0: number) => void): void => {
