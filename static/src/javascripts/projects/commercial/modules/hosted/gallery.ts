...
class HostedGallery {
  ...
  constructor() {
    ...
    this.useSwipe = hasTouchScreen() && (breakpoint === 'mobile' || breakpoint === 'tablet');
  ...
  static ctaIndex(): number | null | undefined {
  ...
  loadSurroundingImages(index: number, count: number) {
    ...
    const setSize = ($image, imageIndex) => {
      if (!that.imageRatios[imageIndex]) {
        that.imageRatios[imageIndex] = $image[0].naturalWidth / $image[0].naturalHeight;
      }
      that.resizeImage.call(that, imageIndex);
    };

    [0, 1, 2].map(i => index + i === 0 ? count - 1 : (index - 1 + i) % count).forEach(function (i) {
  ...
  fadeContent(e: Event) {
    ...
    const scrollHeight = e.target instanceof HTMLElement ? e.target.scrollHeight : 0;
  ...
  image: {
    enter() {
      ...
      if (this.useSwipe) {
        ...
        bonzo(this.$galleryEl).toggleClass('show-cta', this.index === HostedGallery.ctaIndex() + 1);
      }

      const pageName = config.get('page.pageName') || window.location.pathname.substr(window.location.pathname.lastIndexOf('/') + 1);
