...
const shouldAutoplay = (page: Page, switches: {hostedVideoAutoplay: boolean | null | undefined;}): boolean => {
  ...
  return (isSwitchedOn && isUSContent() && isVideoArticle() && flashingElementsAllowed());
};

const sendPercentageCompleteEvents = (atomId: string, youtubePlayer: YouTubePlayer, playerTotalTime: number): void => {
  ...
  Object.keys(playbackEvents).forEach(key => {
    ...
    if (!EVENTSFIRED.includes(key) && youtubePlayer.getCurrentTime() > value) {
...
export const initHostedYoutube = (el: HTMLElement): void => {
  ...
  const youtubeTimer = document.getElementsByClassName('js-youtube-current-time')[0];

  let playTimer;

  initYoutubeEvents(atomId);
  initYoutubePlayer(el, {
    onPlayerStateChange(event: {data: unknown;target: YouTubePlayer;}) {
      ...
      if (event.data === window.YT.PlayerState.ENDED) {
        ...
        youtubeTimer.textContent = minutes + (seconds < 10 ? ':0' : ':') + seconds;
      }
    ...
    onPlayerReady(event: {target: YouTubePlayer;}) {
      if (shouldAutoplay(config.get('page', {}), config.get('switches', {}))) {
