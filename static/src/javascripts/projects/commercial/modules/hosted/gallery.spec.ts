...
describe('Hosted Gallery', () => {
  ...
  it('should open and close the caption on click', () => {
    ...
    expect(classListFor('js-gallery-caption-bar')).toEqual(expect.stringContaining('hosted-gallery--show-caption'));

    button.click();

    expect(classListFor('js-gallery-caption-bar')).not.toEqual(expect.stringContaining('hosted-gallery--show-caption'));
  });
  ...
  it('should open and close the onward journey on click', () => {
    const button: any = document.querySelector('.js-hosted-gallery-oj-close');
    button.click();

    expect(classListFor('js-hosted-gallery-oj')).toEqual(expect.stringContaining('minimise-oj'));

    button.click();

    expect(classListFor('js-hosted-gallery-oj')).not.toEqual(expect.stringContaining('minimise-oj'));
  });
