...
class FabricExpandingV1 {
  ...
  updateBgPosition() {
    ...
    const topCusp = inViewT && viewportHeight * 0.4 - adHeight > adSlotTop ? 'true' : 'false';
    const bottomCusp = inViewB && viewportHeight * 0.5 < adSlotTop ? 'true' : 'false';
    const bottomScroll = bottomCusp === 'true' ? 50 - (viewportHeight * 0.5 - adSlotTop) * -0.2 : 50;
    const topScroll = topCusp === 'true' ? (viewportHeight * 0.4 - adSlotTop - adHeight) * 0.2 : 0;
    ...
    switch (this.params.backgroundImagePType) {
      ...
      case 'fixed':
        ...
        fastdom.mutate(() => {
          $('.ad-exp--expand-scrolling-bg', this.adSlot).css('background-position', `50%${scrollAmount}px`);
        });
        break;
      ...
      case 'parallax':
        ...
        fastdom.mutate(() => {
          ...
          $('.ad-exp--expand-scrolling-bg', this.adSlot).css('background-position', `50%${scrollAmount}%`);
        });
        break;
      case 'none':
        break;
  ...
  listener() {
    if (!this.initialExpandCounter && getViewport().height > this.adSlot.getBoundingClientRect().top + this.openedHeight) {
      ...
      if (!storage.local.get(`gu.commercial.expandable.${itemIdArray[1]}`)) {
        ...
        fastdom.mutate(() => {
          storage.local.set(`gu.commercial.expandable.${itemIdArray[1]}`, true, Date.now() + week);
  ...
  buildVideo(customClass: string) {
    ...
    const viewModel = {
      ...
      className: ['expandable_video', `expandable_video--horiz-pos-${this.params.videoPositionH}`, customClass].join(' '),
  ...
  create() {
    ...
    const showmoreArrow = {
      showArrow: this.params.showMoreType === 'arrow-only' || this.params.showMoreType === 'plus-and-arrow' ? `<button class="ad-exp__open-chevron ad-exp__open">${arrowDown.markup}</button>` : ''
    };
    const showmorePlus = {
      showPlus: this.params.showMoreType === 'plus-only' || this.params.showMoreType === 'plus-and-arrow' ? `<button class="ad-exp__close-button ad-exp__open">${closeCentral.markup}</button>` : ''
    };
    const scrollbgDefaultY = '0%'; // used if no parallax / fixed background scroll support
    const scrollingbg = {
      scrollbg: this.params.backgroundImagePType !== 'none' ? `<div class="ad-exp--expand-scrolling-bg" style="background-image: url(${this.params.backgroundImageP}); background-position: ${this.params.backgroundImagePPosition} ${scrollbgDefaultY}; background-repeat: ${this.params.backgroundImagePRepeat};"></div>` : ''
    };
