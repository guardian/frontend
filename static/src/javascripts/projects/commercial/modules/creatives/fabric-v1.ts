...
const isIE10OrLess = typeof getUserAgent === 'object' && getUserAgent.browser === 'MSIE' && parseInt(getUserAgent.version, 10) <= 10;
...
class FabricV1 {
  ...
  create() {
    ...
    const videoPosition = {
      position: this.params.videoPositionH === 'left' || this.params.videoPositionH === 'right' ? `${this.params.videoPositionH}:${this.params.videoHorizSpace}px;` : ''
    };
    ...
    return fastdom.write(() => {
      ...
      if (this.params.layerTwoAnimation === 'enabled' && this.layer2 && isEnhanced() && !isIE10OrLess) {
        this.layer2.style.backgroundPosition = '';
      }

      if (this.scrollType === 'fixed' && this.scrollingBg && hasBackgroundFixedSupport) {
      ...
      if (this.adSlot.parentNode && this.adSlot.parentNode instanceof HTMLElement && this.adSlot.parentNode.classList.contains('top-banner-ad-container')) {
  ...
  updateBgPosition() {
    if (this.scrollType === 'parallax') {
      const scrollAmount = Math.ceil(this.adSlot.getBoundingClientRect().top * 0.3) + 20;
      fastdom.write(() => {
        if (this.scrollingBg) {
  ...
  layer2Animation() {
    ...
    if (this.params.layerTwoAnimation === 'enabled' && isEnhanced() && !isIE10OrLess) {
      inViewB = getViewport().height > this.adSlot.getBoundingClientRect().top;
      fastdom.write(() => {
        if (this.layer2) {
