...
const FabricExpandableVideoV2 = (adSlot: Element, params: Object) => {
  ...
  const create = () => {
    ...
    const additionalParams = {
      ...
      showArrow: params.showMoreType === 'arrow-only' || params.showMoreType === 'plus-and-arrow' ? `<button class="ad-exp__open-chevron ad-exp__open">${arrowDown.markup}</button>` : '',
      showPlus: params.showMoreType === 'plus-only' || params.showMoreType === 'plus-and-arrow' ? `<button class="ad-exp__close-button ad-exp__open ad-exp__open--${plusIconPosition}">${closeCentral.markup}</button>` : '',
      videoEmbed: params.YoutubeVideoURL !== '' ? `<iframe id="YTPlayer" width="100%" height="${videoHeight}" src="${params.YoutubeVideoURL}?showinfo=0&amp;rel=0&amp;controls=0&amp;fs=0&amp;title=0&amp;byline=0&amp;portrait=0" frameborder="0" class="expandable-video"></iframe>` : ''
    };
    ...
    const open = isOpen => {
      ...
      if (isOpen) {
        ...
        $('.slide-video, .slide-video .ad-exp__layer', adSlot).css('height', openedHeight).addClass('slide-video__expand');
      } else {
        ...
        $('.slide-video, .slide-video .ad-exp__layer', adSlot).css('height', closedHeight).removeClass('slide-video__expand');
      }
    ...
    bean.on(adSlot, 'click', '.video-container__cta, .creative__cta', () => {
      fastdom.mutate(() => {
        open(false);
      });
    });
    ...
    return fastdom.mutate(() => {
      ...
      if (adSlot.parentNode && ((adSlot.parentNode as any) as Element).classList.contains('top-banner-ad-container')) {
        ((adSlot.parentNode as any) as Element).classList.add('top-banner-ad-container--fabric');
      }
      return true;
    });
  };
