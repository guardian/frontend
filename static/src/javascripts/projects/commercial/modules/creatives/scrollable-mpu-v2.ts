...
class ScrollableMpu {
  ...
  updateBgParallax() {
    ...
    fastdom.mutate(() => {
      if (this.$scrollableImage) {
        this.$scrollableImage.addClass('creative--scrollable-mpu-image-parallax').css('background-position', `50% ${scrollAmount}%`);
      }
    });
  }
  ...
  updateBg() {
    if (this.$scrollableMpu) {
      const position = -this.$scrollableMpu[0].getBoundingClientRect().top;
      fastdom.mutate(() => {
        if (this.$scrollableImage) {
  ...
  create(): Promise<boolean | null | undefined> {
    const templateOptions = {
      ...
      backgroundImage: hasScrollEnabled && this.params.backgroundImage ? `<div class="creative--scrollable-mpu-image" style="background-image: url(${this.params.backgroundImage});"></div>` : ''
    };
