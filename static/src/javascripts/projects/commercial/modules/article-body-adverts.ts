...
import { SpacefinderItem } from "common/modules/spacefinder";
...
const insertAdAtPara = (para: Node, name: string, type: string, classes: string | null | undefined, sizes: Sizes | null | undefined): Promise<void> => {
  ...
  return fastdom.write(() => ads.forEach(ad => {
    if (para.parentNode) {
...
const filterNearbyCandidates = (maximumAdHeight: number) => (candidate: SpacefinderItem): boolean => {
...
const getBodySelector = (): string => !config.get('isDotcomRendering', false) ? '.js-article__body' : '.article-body-commercial-selector';
...
const addDesktopInlineAds = (isInline1: boolean): Promise<number> => {
  ...
  const insertAds = (paras: HTMLElement[]): Promise<number> => {
    const slots: Array<Promise<void>> = paras.slice(0, isInline1 ? 1 : paras.length).map((para: Node, i: number) => {
      const inlineId = i + (isInline1 ? 1 : 2);

      return insertAdAtPara(para, `inline${inlineId}`, 'inline', `inline${isInline1 ? '' : ' offset-right'}`, isInline1 ? null : { desktop: [adSizes.halfPage, adSizes.skyscraper] });
    });
...
const addMobileInlineAds = (): Promise<number> => {
  ...
  const insertAds = (paras: HTMLElement[]): Promise<number> => {
    const slots: Array<Promise<void>> = paras.map((para: Node, i: number) => insertAdAtPara(para, i === 0 ? 'top-above-nav' : `inline${i}`, i === 0 ? 'top-above-nav' : 'inline', 'inline'));
...
const attemptToAddInlineMerchAd = (): Promise<boolean> => {
  ...
  return spaceFiller.fillSpace(rules, paras => insertAdAtPara(paras[0], 'im', 'im').then(() => true), {
    waitForImages: true,
    waitForLinks: true,
    waitForInteractives: true
  });
...
const doInit = (): Promise<boolean> => {
  ...
  const im = config.get('page.hasInlineMerchandise') ? attemptToAddInlineMerchAd() : Promise.resolve(false);
  im.then((inlineMerchAdded: boolean) => inlineMerchAdded ? trackAdRender('dfp-ad--im') : Promise.resolve()).then(addInlineAds).then(initCarrot);
