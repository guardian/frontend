...
export const init = (): Promise<boolean> => {
  ...
  return fastdom.measure((): [number, number] => [$mainCol.dim().height, $immersiveEls.offset().top - $mainCol.offset().top]).then(([mainColHeight, immersiveOffset]: [number, number]) => {
    ...
    if (config.get('page.isImmersive') && $immersiveEls.length > 0) {
      return fastdom.mutate(() => {
        ...
        $adSlot[0].setAttribute('data-mobile', getAllowedSizesForImmersive(immersiveOffset));
        return $adSlot[0];
      });
    }
