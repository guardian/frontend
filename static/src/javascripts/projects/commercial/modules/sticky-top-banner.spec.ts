...
describe('Sticky ad banner', () => {
  ...
  it('should add listeners and classes', () => init().then(() => {
  ...
  it('should set the slot height and the header top margin', () => {
    ...
    return init().then(() => _.whenFirstRendered).then(() => resizeStickyBanner(randomHeight)).then(() => {
      if (!header || !stickyBanner) {
        throw Error('missing header or sticky banner element');
      } else {
  ...
  it('should adjust the scroll position', () => {
    ...
    return init().then(() => resizeStickyBanner(randomHeight)).then(() => {
      window.pageYOffset = 0;
  ...
  it('should include height and paddings when setting the slot height', () => {
    ...
    return init().then(() => update(height)).then(() => {
      if (!stickyBanner) {
        throw Error('missing sticky banner element');
      } else {
        expect(stickyBanner.style.height).toBe(`${height + padingTop + paddingBottom}px`);
      }
    });
  });

  it('should reset the banner position and top styles at the top of the page', () => onScroll().then(() => {
    if (!stickyBanner) {
      throw Error('missing header or sticky banner element');
    } else {
  ...
  it('should position the banner absolutely past the header', () => {
    ...
    return init().then(onScroll).then(() => {
      if (!stickyBanner) {
        throw Error('missing header or sticky banner element');
      } else {
