...
describe('Cross-frame messenger: scroll', () => {
  ...
  beforeEach(() => {
    jest.spyOn(global, 'addEventListener').mockImplementation((_, callback) => {
      onScroll = callback;
    });
  ...
  describe('With IntersectionObserver', () => {
    ...
    it('should call respond1 but not respond2 at the top of the page', () => {
      if (onIntersect) {
        onIntersect([{ target: iframe1, intersectionRatio: 0.5 }, { target: iframe2, intersectionRatio: 0 }]);
      }
    ...
    it('should call respond2 but not respond1 at the bottom of the page', () => {
      if (onIntersect) {
        onIntersect([{ target: iframe1, intersectionRatio: 0 }, { target: iframe2, intersectionRatio: 0.5 }]);
      }
      return onScroll().then(() => {
  ...
  describe('Without IntersectionObserver', () => {
    beforeEach(() => {
      reset(false);
      return Promise.all([addScrollListener(iframe1, respond1), addScrollListener(iframe2, respond2)]);
    });
