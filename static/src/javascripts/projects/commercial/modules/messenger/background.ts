...
import { renderStickyAdLabel, renderStickyScrollForMoreLabel } from "commercial/modules/dfp/render-advert-label";
...
const getStylesFromSpec = (specs: AdSpec): SpecStyles => Object.keys(specs).reduce((result, key) => {
  if (key !== 'scrollType') {
...
const setBackground = (specs: AdSpec, adSlot: HTMLElement): Promise<any> => {
  ...
  const maybeBackgroundParent = ((adSlot.getElementsByClassName(backgroundParentClass) as any) as HTMLCollection<HTMLElement>)[0];
  const maybeBackground = maybeBackgroundParent ? ((maybeBackgroundParent.getElementsByClassName(backgroundClass) as any) as HTMLCollection<HTMLElement>)[0] : null;
  const backgroundAlreadyExists = !!(maybeBackgroundParent && maybeBackground);
  ...
  const getBackground = (): Promise<Background> => {
    ...
    return fastdom.mutate(() => {
      if (backgroundParent) {
        ...
        if (config.get('isDotcomRendering', false) && adSlot.firstChild && adSlot.firstChild instanceof HTMLElement) adSlot.firstChild.style.contain = 'layout';
        ...
        if (specs.scrollType === 'interscroller') {
          ...
          if (specs.ctaUrl != null) {
            ...
            adSlot.insertBefore(ctaURLAnchor, adSlot.firstChild);
          }
  ...
  const updateStyles = (backgroundParent: HTMLElement, background: HTMLElement) => {
    ...
    if (config.get('isDotcomRendering', false) && specs.scrollType === 'parallax') background.style.position = 'absolute';
  ...
  const onInterscrollerScroll = (backgroundParent: HTMLElement, background: HTMLElement) => {
    fastdom.measure(() => {
  ...
  const onScroll = (backgroundParent: HTMLElement, background: HTMLElement) => {
    fastdom.measure(() => {
      ...
      const parallaxBackgroundMovement = Math.floor(rect.bottom / (windowHeight + backgroundHeight) * 130);
      ...
      (background.style as any).backgroundPositionY = `${parallaxBackgroundMovement}%`;
    });
  };
    ...
    entries.filter(entry => entry.isIntersecting).forEach(entry => {
      if (!backgroundAlreadyExists) {
        ...
        if (background && background instanceof HTMLElement) {
          addEventListener(window, 'scroll', () => onScroll(backgroundParent, background), {
            passive: true
          });
  ...
  return getBackground().then(({
    backgroundParent,
    background
  }) => updateStyles(backgroundParent, background)).then(({
    backgroundParent,
    background
  }) => {
    if (specs.scrollType === 'interscroller') {
      ...
      addEventListener(window, 'scroll', () => onInterscrollerScroll(backgroundParent, background), {
        passive: true
      });
    } else {
