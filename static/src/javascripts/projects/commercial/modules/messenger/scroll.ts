...
const getDimensions = (id: string): IframeTuple => [id, iframes[id].node.getBoundingClientRect()];
...
const onScroll = (): Promise<any> | null | undefined => {
  if (!taskQueued) {
    ...
    return fastdom.measure(() => {
      ...
      if (useIO) {
        ...
        iframeIds.map(getDimensions).filter(isIframeInViewport, viewport).forEach(data => {
          sendCoordinates(data[0], data[1]);
        });
      }
    });
...
const addScrollListener = (iframe: Element, respond: any): Promise<any> | null | undefined => {
  if (iframeCounter === 0) {
  ...
  fastdom.measure(() => iframe.getBoundingClientRect()).then(domRect => {
