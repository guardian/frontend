...
import { SpacefinderRules, SpacefinderItem } from "common/modules/spacefinder";
...
let SLOTCOUNTER = 0;
let WINDOWHEIGHT;
let firstSlot: HTMLElement | null | undefined;
...
const getSpaceFillerRules = (windowHeight: number, update?: boolean): SpacefinderRules => {
  let prevSlot;
  ...
  const filterSlot = (slot: SpacefinderItem): boolean => {
    if (!prevSlot) {
      prevSlot = slot;
      return !shouldUpdate;
    } if (Math.abs(slot.top - prevSlot.top) > windowHeight * 2) {
      prevSlot = slot;
      return true;
    }
    return false;
...
const fill = (rules: SpacefinderRules): Promise<void> => spaceFiller.fillSpace(rules, insertAds).then(result => {
...
export const init = (): Promise<void> => {
  ...
  fastdom.read(() => {
    ...
    return WINDOWHEIGHT;
  }).then(getSpaceFillerRules).then(fill);
