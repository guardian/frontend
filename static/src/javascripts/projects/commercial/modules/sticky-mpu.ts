...
const stickyCommentsMpu = (adSlot: HTMLElement) => {
  ...
  fastdom.read(() => referenceElement.offsetHeight - 600).then(newHeight => fastdom.write(() => {
...
const stickyMpu = (adSlot: HTMLElement) => {
  ...
  fastdom.read(() => referenceElement.offsetTop + stickyPixelBoundary).then(newHeight => fastdom.write(() => {
    (adSlot.parentNode as any).style.height = `${newHeight}px`;
  })).then(() => {
    if (noSticky) {
