...
const onResize = (specs, _, iframe: HTMLElement | null | undefined) => {
...
const stickyCommentsMpu = (adSlot: HTMLElement) => {
  ...
  const referenceElement: HTMLElement | null | undefined = document.querySelector('.js-comments');

  if (!referenceElement || !adSlot) {
    return;
  }

  fastdom.measure(() => referenceElement.offsetHeight - 600).then(newHeight => fastdom.mutate(() => {
    (adSlot.parentNode as any).style.height = `${newHeight}px`;
  })).then(() => {
    if (noSticky) {
...
const stickyMpu = (adSlot: HTMLElement) => {
  ...
  const referenceElement: HTMLElement | null | undefined = document.querySelector(['.js-article__body:not([style*="display: none;"])', '.js-liveblog-body-content:not([style*="display: none;"])'].join(', '));
  ...
  fastdom.measure(() => referenceElement.offsetTop + stickyPixelBoundary).then(newHeight => fastdom.mutate(() => {
    (adSlot.parentNode as any).style.height = `${newHeight}px`;
  })).then(() => {
    if (noSticky) {
