...
import { HeaderBiddingSlot } from "commercial/modules/header-bidding/types";

const slotKeyMatchesAd = (pbs: HeaderBiddingSlot, ad: Advert): boolean => stripTrailingNumbersAbove1(stripMobileSuffix(ad.id)).endsWith(pbs.key);

const filterByAdvert = (ad: Advert, slots: Array<HeaderBiddingSlot>): Array<HeaderBiddingSlot> => {
  const adUnits = slots.filter(slot => {
    if (slot.key === 'banner') {
      // Special case for interactive banner slots
      // as they are currently incorrectly identified.
      return ((!!ad.id.match(/^dfp-ad--\d+/) && ad.node.classList.contains('ad-slot--banner-ad-desktop')) || slotKeyMatchesAd(slot, ad));
    }
...
const getSlots = (contentType: string): Array<HeaderBiddingSlot> => {
  ...
  const hasExtendedMostPop = isArticle && config.get('switches.extendedMostPopular');
  ...
  const tabletSlots: Array<HeaderBiddingSlot> = [{
    ...
    key: 'mostpop',
    sizes: hasExtendedMostPop ? [[300, 600], [300, 250], [728, 90]] : [[300, 250]]
  }];
  ...
  switch (getBreakpointKey()) {
    case 'M':
      return shouldIncludeMobileSticky() && config.get('switches.mobileStickyPrebid') ? commonSlots.concat([...mobileSlots, mobileStickySlot]) : commonSlots.concat(mobileSlots);
    case 'T':
...
export const getHeaderBiddingAdSlots = (ad: Advert, slotFlatMap?: (arg0: HeaderBiddingSlot) => HeaderBiddingSlot[]): Array<HeaderBiddingSlot> => {
  const effectiveSlotFlatMap = slotFlatMap || (s => [s]); // default to identity
  const adSlots = filterByAdvert(ad, getSlots(config.get('page.contentType', '')));
  return adSlots.map(effectiveSlotFlatMap) // $FlowFixMe
