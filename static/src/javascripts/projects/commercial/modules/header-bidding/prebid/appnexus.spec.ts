...
import { isInUsOrCa as isInUsOrCa_, isInAuOrNz as isInAuOrNz_ } from "common/modules/commercial/geo-utils";
import { _, getAppNexusDirectBidParams, getAppNexusServerSideBidParams } from "./appnexus";
...
jest.mock('common/modules/experiments/ab', () => ({
  isInVariantSynchronous: jest.fn((testId, variantId) => variantId === 'variant')
}));
...
describe('getAppNexusInvCode', () => {
  ...
  test('should return the magic strings for mobile breakpoints', () => {
    ...
    const invCodes = [[[300, 250]], [[300, 600]], [[970, 250]], [[728, 90]]].map(getAppNexusInvCode);

    expect(invCodes).toEqual(['Mmagic300x250', 'Mmagic300x600', 'Mmagic970x250', 'Mmagic728x90']);
  });
  ...
  test('should return the magic strings for other breakpoints', () => {
    ...
    const invCodes = [[[300, 250]], [[300, 600]], [[970, 250]], [[728, 90]]].map(getAppNexusInvCode);
    expect(invCodes).toEqual(['Dmagic300x250', 'Dmagic300x600', 'Dmagic970x250', 'Dmagic728x90']);
  });
...
describe('getAppNexusDirectPlacementId', () => {
  ...
  test('should return the expected values when in AU region and desktop device', () => {
    ...
    expect(prebidSizes.map(size => getAppNexusDirectPlacementId(size))).toEqual(['11016434', '11016434', '11016434', '11016434', '11016434']);
  });

  test('should return the expected values for ROW when on desktop device', () => {
    isInAuOrNz.mockReturnValue(false);
    getBreakpointKey.mockReturnValue('D');
    expect(prebidSizes.map(size => getAppNexusDirectPlacementId(size))).toEqual(['9251752', '9251752', '9926678', '9926678', '9251752']);
  });

  test('should return the expected values for ROW when on tablet device', () => {
    getBreakpointKey.mockReturnValue('T');
    isInAuOrNz.mockReturnValue(false);
    expect(prebidSizes.map(size => getAppNexusDirectPlacementId(size))).toEqual(['4371641', '9251752', '9251752', '4371640', '9251752']);
  });
});
...
describe('getAppNexusPlacementId', () => {
  ...
  test('should return the expected values when on desktop device', () => {
    getBreakpointKey.mockReturnValue('D');
    expect(generateTestIds()).toEqual(['13366606', '13366606', '13366615', '13366615', '13915593']);
  });

  test('should return the expected values when on tablet device', () => {
    getBreakpointKey.mockReturnValue('T');
    expect(generateTestIds()).toEqual(['13366913', '13915593', '13915593', '13366916', '13915593']);
  });

  test('should return the expected values when on mobile device', () => {
    getBreakpointKey.mockReturnValue('M');
    expect(generateTestIds()).toEqual(['13366904', '13915593', '13915593', '13915593', '13915593']);
  });
