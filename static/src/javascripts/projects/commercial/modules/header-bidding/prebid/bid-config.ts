...
const indexExchangeBidders: (arg0: HeaderBiddingSize[]) => PrebidBidder[] = (
    slotSizes
) => {
...
const biddersBeingTested: (arg0: PrebidBidder[]) => PrebidBidder[] = (
    allBidders
) => allBidders.filter((bidder) => pbTestNameMap()[bidder.name]);

const biddersSwitchedOn: (arg0: PrebidBidder[]) => PrebidBidder[] = (
    allBidders
) => {
    const isSwitchedOn: (arg0: PrebidBidder) => boolean = (bidder) =>
        config.get(`switches.${bidder.switchName}`);
    return allBidders.filter((bidder) => isSwitchedOn(bidder));
};

const currentBidders: (arg0: HeaderBiddingSize[]) => PrebidBidder[] = (
    slotSizes
) => {
...
export const bids: (arg0: string, arg1: HeaderBiddingSize[]) => PrebidBid[] = (
    slotId,
    slotSizes
) =>
