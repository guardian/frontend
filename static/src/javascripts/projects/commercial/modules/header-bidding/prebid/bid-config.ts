...
import { buildAppNexusTargeting, buildAppNexusTargetingObject, getPageTargeting } from "common/modules/commercial/build-page-targeting";
...
const isInSafeframeTestVariant = (): boolean => isInVariantSynchronous(commercialPrebidSafeframe, 'variant');

const isArticle = config.get('page.contentType') === 'Article';

const isDesktopAndArticle = getBreakpointKey() === 'D' && isArticle;

const getTrustXAdUnitId = (slotId: string, isDesktopArticle: boolean): string => {
  switch (stripMobileSuffix(slotId)) {
    ...
    default:
      ...
      console.log(`PREBID: Failed to get TrustX ad unit for slot ${slotId}.`);
      return '';
...
const getIndexSiteId = (): string => {
  if (isInSafeframeTestVariant()) {
    ...
    const site = config.get('page.pbIndexSites', []).find(s => s.bp === getBreakpointKey());
...
const getImproveSizeParam = (slotId: string): {w?: number;h?: number;} => {
  ...
  return key && (key.endsWith('mostpop') || key.endsWith('comments') || key.endsWith('inline1') || (key.endsWith('inline') && !isDesktopAndArticle)) ? { w: 300, h: 250 } : {};
};
...
const getTripleLiftInventoryCode = (slotId: string, sizes: HeaderBiddingSize[]): string => {
  if (containsLeaderboard(sizes)) return 'theguardian_topbanner_728x90_prebid';

  if (containsMpu(sizes)) return isArticle ? 'theguardian_article_300x250_prebid' : 'theguardian_sectionfront_300x250_prebid';
...
const appNexusBidder: PrebidBidder = {
  ...
  bidParams: (slotId: string, sizes: HeaderBiddingSize[]): PrebidAppNexusParams => getAppNexusDirectBidParams(sizes)
};
...
const tripleLiftBidder: PrebidBidder = {
  ...
  bidParams: (slotId: string, sizes: HeaderBiddingSize[]): PrebidTripleLiftParams => ({
...
const improveDigitalBidder: PrebidBidder = {
  ...
  bidParams: (slotId: string, sizes: HeaderBiddingSize[]): PrebidImproveParams => ({
...
const xaxisBidder: PrebidBidder = {
  ...
  bidParams: (slotId: string, sizes: HeaderBiddingSize[]): PrebidXaxisParams => ({
...
const indexExchangeBidders: (arg0: HeaderBiddingSize[]) => PrebidBidder[] = slotSizes => {
...
const biddersBeingTested: (arg0: PrebidBidder[]) => PrebidBidder[] = allBidders => allBidders.filter(bidder => pbTestNameMap()[bidder.name]);

const biddersSwitchedOn: (arg0: PrebidBidder[]) => PrebidBidder[] = allBidders => {
  const isSwitchedOn: (arg0: PrebidBidder) => boolean = bidder => config.get(`switches.${bidder.switchName}`);
  return allBidders.filter(bidder => isSwitchedOn(bidder));
};

const currentBidders: (arg0: HeaderBiddingSize[]) => PrebidBidder[] = slotSizes => {
  const otherBidders: PrebidBidder[] = [...(inPbTestOr(shouldIncludeSonobi()) ? [sonobiBidder] : []), ...(inPbTestOr(shouldIncludeTrustX()) ? [trustXBidder] : []), ...(inPbTestOr(shouldIncludeTripleLift()) ? [tripleLiftBidder] : []), ...(inPbTestOr(shouldIncludeAppNexus()) ? [appNexusBidder] : []), ...(inPbTestOr(shouldIncludeImproveDigital()) ? [improveDigitalBidder] : []), ...(inPbTestOr(shouldIncludeXaxis()) ? [xaxisBidder] : []), pubmaticBidder, ...(inPbTestOr(shouldIncludeAdYouLike(slotSizes)) ? [adYouLikeBidder] : []), ...(inPbTestOr(shouldUseOzoneAdaptor()) ? [ozoneClientSideBidder] : []), ...(shouldIncludeOpenx() ? [openxClientSideBidder] : [])];
...
export const bids: (arg0: string, arg1: HeaderBiddingSize[]) => PrebidBid[] = (slotId, slotSizes) => currentBidders(slotSizes).map((bidder: PrebidBidder) => ({
  bidder: bidder.name,
