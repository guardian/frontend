...
import { isInUk as isInUk_, isInUsOrCa as isInUsOrCa_, isInAuOrNz as isInAuOrNz_, isInRow as isInRow_ } from "common/modules/commercial/geo-utils";
import { _, bids } from "./bid-config";
import { PrebidBidder, HeaderBiddingSize } from "../types";

import { containsBillboard as containsBillboard_, containsDmpu as containsDmpu_, containsLeaderboard as containsLeaderboard_, containsLeaderboardOrBillboard as containsLeaderboardOrBillboard_, containsMobileSticky as containsMobileSticky_, containsMpu as containsMpu_, containsMpuOrDmpu as containsMpuOrDmpu_, getBreakpointKey as getBreakpointKey_, shouldIncludeAdYouLike as shouldIncludeAdYouLike_, shouldIncludeAppNexus as shouldIncludeAppNexus_, shouldIncludeImproveDigital as shouldIncludeImproveDigital_, shouldIncludeOpenx as shouldIncludeOpenx_, shouldIncludeTrustX as shouldIncludeTrustX_, shouldIncludeXaxis as shouldIncludeXaxis_, shouldIncludeSonobi as shouldIncludeSonobi_, stripMobileSuffix as stripMobileSuffix_, shouldIncludeTripleLift as shouldIncludeTripleLift_ } from "../utils";
...
const getBidders = () => bids('dfp-ad--top-above-nav', [[728, 90]]).map(bid => bid.bidder);
...
describe('getImprovePlacementId', () => {
  ...
  const generateTestIds = (): Array<number> => {
    const prebidSizes: Array<Array<HeaderBiddingSize>> = [[[300, 250]], [[300, 600]], [[970, 250]], [[728, 90]], [[1, 2]]];
  ...
  test('should return the expected values when geolocated in UK and on desktop device', () => {
    ...
    expect(generateTestIds()).toEqual([1116396, 1116396, 1116397, 1116397, -1]);
  });
  ...
  test('should return the expected values when geolocated in UK and on tablet device', () => {
    ...
    containsLeaderboardOrBillboard.mockReturnValueOnce(true);
    containsLeaderboardOrBillboard.mockReturnValue(false);
    expect(generateTestIds()).toEqual([1116398, 1116398, 1116399, 1116399, -1]);
  });
  ...
  test('should return the expected values when geolocated in ROW region and on desktop device', () => {
    ...
    containsLeaderboardOrBillboard.mockReturnValueOnce(true);
    containsLeaderboardOrBillboard.mockReturnValue(false);
    expect(generateTestIds()).toEqual([1116420, 1116420, 1116421, 1116421, -1]);
  });
  ...
  test('should return the expected values when not geolocated in ROW region and on tablet device', () => {
    ...
    containsLeaderboardOrBillboard.mockReturnValueOnce(true);
    containsLeaderboardOrBillboard.mockReturnValue(false);
    expect(generateTestIds()).toEqual([1116422, 1116422, 1116423, 1116423, -1]);
  });
  ...
  test('should use test placement ID when participating in CommercialPrebidSafeframe test in desktop MPU', () => {
    isInVariantSynchronous.mockImplementationOnce((testId, variantId) => variantId === 'variant');
    getBreakpointKey.mockReturnValue('D');
...
describe('indexExchangeBidders', () => {
  beforeEach(() => {
    resetConfig();
    getBreakpointKey.mockReturnValue('D');
    config.set('page.pbIndexSites', [{ bp: 'D', id: 123456 }, { bp: 'M', id: 234567 }, { bp: 'T', id: 345678 }]);
  });
  ...
  test('should return an IX bidder for every size that the slot can take', () => {
    ...
    expect(bidders).toEqual([expect.objectContaining({
      name: 'ix',
...
describe('getIndexSiteId', () => {
  ...
  test('should find the correct ID for the breakpoint', () => {
    ...
    expect(results).toEqual(['234567', '123456', '234567', '345678', '123456']);
  });
...
describe('bids', () => {
  ...
  test('should include ix bidder for each size that slot can take', () => {
    const rightSlotBidders = () => bids('dfp-right', [[300, 600], [300, 250]]).map(bid => bid.bidder);
  ...
  test('should only include multiple bidders being tested, even when their switches are off', () => {
    ...
    isInVariantSynchronous.mockImplementation((testId, variantId) => variantId === 'variant');
    config.set('switches.prebidXaxis', false);
...
describe('triplelift adapter', () => {
  ...
  test('should return correct triplelift adapter params for leaderboard', () => {
    ...
    const tripleLiftBids = bids('dfp-ad--top-above-nav', [[728, 90]])[1].params;
  ...
  test('should return correct triplelift adapter params for mobile sticky', () => {
    ...
    const tripleLiftBids = bids('dfp-ad--top-above-nav', [[320, 50]])[1].params;
    expect(tripleLiftBids).toEqual({
