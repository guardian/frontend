...
import { getLargestSize, containsLeaderboard, containsLeaderboardOrBillboard, containsMpu, containsMpuOrDmpu, getBreakpointKey } from "../utils";

import { PrebidAppNexusParams, HeaderBiddingSize } from "../types";

const getAppNexusInvCode = (sizes: Array<HeaderBiddingSize>): string | null | undefined => {
  ...
  const sectionName = config.get('page.section', '') || config.get('page.sectionName', '').replace(/ /g, '-');
...
export const getAppNexusDirectPlacementId = (sizes: HeaderBiddingSize[]): string => {
  if (isInAuOrNz()) {
    return '11016434';
  }
...
export const getAppNexusDirectBidParams = (sizes: HeaderBiddingSize[]): PrebidAppNexusParams => {
