...
import { getBreakpoint as getBreakpoint_, isBreakpoint as isBreakpoint_ } from "lib/detect";
import config from "lib/config";
import { _ } from "common/modules/commercial/geo-utils";
import { getLargestSize, getBreakpointKey, shouldIncludeAdYouLike, shouldIncludeAppNexus, shouldIncludeImproveDigital, shouldIncludeOpenx, shouldIncludeSonobi, shouldIncludeTrustX, shouldIncludeXaxis, stripMobileSuffix, stripTrailingNumbersAbove1, stripDfpAdPrefixFrom, removeFalseyValues, shouldIncludeMobileSticky } from "./utils";
...
describe('Utils', () => {
  ...
  test('stripPrefix correctly strips valid cases', () => {
    const validStrips: Array<Array<string>> = [['dfp-ad--slot', 'slot'], ['slot', 'slot'], ['dfp-ad--', '']];
  ...
  // $FlowFixMe
  test.each([['AU', 'on', true], ['AU', 'off', true], ['NZ', 'on', true], ['NZ', 'off', true], ['GB', 'on', true], ['GB', 'off', false], ['US', 'on', false], ['CA', 'on', false], ['FK', 'on', true], ['GI', 'on', true], ['GG', 'on', true], ['IM', 'on', true], ['JE', 'on', true], ['SH', 'on', true], ['FK', 'off', false], ['GI', 'off', false], ['GG', 'off', false], ['IM', 'off', false], ['JE', 'off', false], ['SH', 'off', false]])(`In %s, if switch is %s, shouldIncludeAppNexus should return %s`, (region, switchState, expected) => {
  ...
  test('shouldIncludeXaxis should be false if geolocation is not GB', () => {
    config.set('page.isDev', true);
    const testGeos = ['FK', 'GI', 'GG', 'IM', 'JE', 'SH', 'AU', 'US', 'CA', 'NZ'];
    for (let i = 0; i < testGeos.length; i += 1) {
      getSync.mockReturnValue(testGeos[i]);
  ...
  test('stripMobileSuffix', () => {
    expect(stripMobileSuffix('top-above-nav--mobile')).toBe('top-above-nav');
