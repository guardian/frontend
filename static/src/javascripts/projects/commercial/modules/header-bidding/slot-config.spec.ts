...
import { getBreakpointKey as getBreakpointKey_, shouldIncludeMobileSticky as shouldIncludeMobileSticky_ } from "./utils";
...
describe('getPrebidAdSlots', () => {
  ...
  test('should return the correct interactive banner slot at breakpoint D', () => {
    ...
    dfpAdvert.node.setAttribute('class', 'js-ad-slot ad-slot ad-slot--banner-ad ad-slot--banner-ad-desktop ad-slot--rendered');
  ...
  test('should return the correct mobile-sticky slot at breakpoint M', () => {
    ...
    config.set('switches.mobileStickyPrebid', true);
    shouldIncludeMobileSticky.mockReturnValue(true);
    expect(getHeaderBiddingAdSlots(buildAdvert('dfp-ad-mobile-sticky'))).toEqual([{
      key: 'mobile-sticky',
      sizes: [[320, 50]]
    }]);
  });
