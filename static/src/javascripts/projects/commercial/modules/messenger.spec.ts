...
describe('Cross-frame messenger', () => {
  ...
  it('should not respond when sending malformed JSON', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: mockOrigin, data: '{', source: '' })).then(() => {
  ...
  it('should throw when the listener fails', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: mockOrigin, data: '', source: 'source' })).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: {
          code: 500,
  ...
  it('should respond with the listeners cumulative result', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: mockOrigin, data: '', source: 'sorcery' })).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: null,
        id: payload.id,
        result: 4
  ...
  it('should respond to Rubicon messages with no IDs', done => {
    ...
    onMessage({ origin: mockOrigin, data: '', source: 'saucy' }).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: null,
