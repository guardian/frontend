...
import { register as register_, unregister as unregister_, _ as testExports } from "commercial/modules/messenger";
...
describe('Cross-frame messenger', () => {
  ...
  it('should only respond when origin is whitelisted', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: 'http://google.com', source: '' })).then(() => {
      expect(postMessage).not.toHaveBeenCalled();
      return onMessage({ origin: dfpOrigin, source: '' });
    }).then(() => {
      expect(postMessage).toHaveBeenCalled();
    }).then(done).catch(done.fail).then(() => {
      unregister('abc', noop);
    });
  });
  ...
  it('should not respond when sending malformed JSON', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: dfpOrigin, data: '{', source: '' })).then(() => {
      expect(postMessage).not.toHaveBeenCalled();
    }).then(done).catch(done.fail);
  });

  it('should not respond when sending incomplete payload', done => {
    const payloads = [{ type: 'missing data' }, { value: 'missing type' }, { type: 'unregistered', value: 'type' }];
  ...
  it('should throw when the listener fails', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: dfpOrigin, data: '', source: 'source' })).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: {
          code: 500,
  ...
  it('should respond with the listeners cumulative result', done => {
    ...
    Promise.resolve().then(() => onMessage({ origin: dfpOrigin, data: '', source: 'sorcery' })).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: null,
        id: payload.id,
        result: 4
  ...
  it('should respond to Rubicon messages with no IDs', done => {
    ...
    onMessage({ origin: dfpOrigin, data: '', source: 'saucy' }).then(() => {
      expect(postMessage).toHaveBeenCalledWith({
        error: null,
