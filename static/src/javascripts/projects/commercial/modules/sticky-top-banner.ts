...
let topSlot: HTMLElement | null | undefined;
let topSlotHeight: number;
let topSlotStyles: CSSStyleDeclaration;
let stickyBanner: HTMLElement | null | undefined;
let scrollY: number;
...
// though.
const setupAnimation = (): Promise<any> => fastdom.mutate(() => {
  if (stickyBanner && header) {
...
const update = (newHeight: number): Promise<any> => fastdom.measure(() => {
...
const onResize = (specs: any, _: any, iframe: Element | null | undefined): void => {
...
const getAdvertSizeByIndex = (advert: Advert | null | undefined, index: number): number | null | undefined => {
...
const onFirstRender = (): void => {
  ...
  _.whenFirstRendered = trackAdRender(topSlotId).then(isRendered => {
    if (isRendered) {
      ...
      return fastdom.measure(() => (topSlot && topSlot.getBoundingClientRect().height) || 0).then(resizeStickyBanner);
    }
  });
  /* eslint-enable no-use-before-define */
};

const initState = (): Promise<any> => fastdom.measure(() => {
  if (header) {
    headerHeight = header.getBoundingClientRect().height;
  }

  return (topSlot && topSlot.getBoundingClientRect().height) || 0;
}).then(currentHeight => Promise.all([resizeStickyBanner(currentHeight), onScroll()]));
...
const init = (): Promise<void> => {
  ...
  if (topSlot && isBreakpoint({
    min: 'desktop'
  })) {
    header = document.getElementById('header');
    stickyBanner = (topSlot.parentNode as any);
