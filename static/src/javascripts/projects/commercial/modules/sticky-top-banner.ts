...
// though.
const setupAnimation = (): Promise<any> => fastdom.write(() => {
  if (stickyBanner && header) {
...
const update = (newHeight: number): Promise<any> => fastdom.read(() => {
...
const onFirstRender = (): void => {
  ...
  _.whenFirstRendered = trackAdRender(topSlotId).then(isRendered => {
    if (isRendered) {
      ...
      return fastdom.read(() => (topSlot && topSlot.getBoundingClientRect().height) || 0).then(resizeStickyBanner);
    }
  });
  /* eslint-enable no-use-before-define */
};

const initState = (): Promise<any> => fastdom.read(() => {
  if (header) {
