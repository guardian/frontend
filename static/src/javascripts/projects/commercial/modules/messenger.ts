...
const ALLOWED_HOSTS = [dfpOrigin, `${window.location.protocol}//${window.location.host}`];
const LISTENERS = {};
...
const isProgrammaticMessage = (payload: ProgrammaticMessage | StandardMessage): boolean => payload.type === 'set-ad-height' && ('id' in payload.value || 'slotId' in payload.value) && 'height' in payload.value;
...
const getIframe = (data: StandardMessage): HTMLElement | null | undefined => {
  if (data.slotId) {
...
// event type
const isValidPayload = (payload: StandardMessage): boolean => 'type' in payload && 'value' in payload && 'id' in payload && payload.type in LISTENERS && /^[a-f0-9]{8}-([a-f0-9]{4}-){3}[a-f0-9]{12}$/.test(payload.id);
...
const onMessage = (event: Event): void => {
  ...
  if (Array.isArray(LISTENERS[data.type]) && LISTENERS[data.type].length) {
    ...
    // Hat tip @piuccio
    const promise = LISTENERS[data.type].slice() // We offer, but don't impose, the possibility that a listener returns
    ...
    .reduce((func, listener) => func.then(ret => {
...
export type RegisterListeners = (type: string, callback: () => (Promise<any> | null | undefined) | (Array<any> | null | undefined), options: Object | null | undefined) => void;
...
export const unregister = (type: string, callback: () => Promise<any> | null | undefined, options: Object = {}): void => {
