...
describe('createCommentSlots', () => {
  ...
  it('should return an ad slot with the correct sizes', () => {
    ...
    expect(commentMpu.getAttribute('data-desktop')).toBe('1,1|2,2|300,250|300,274|620,1|620,350|550,310|fluid');
    expect(commentMpu.getAttribute('data-mobile')).toBe('1,1|2,2|300,197|300,250|300,274|fluid');
    expect(commentDmpu.getAttribute('data-desktop')).toBe('1,1|2,2|300,250|300,274|620,1|620,350|550,310|fluid|300,600|160,600');
    expect(commentDmpu.getAttribute('data-mobile')).toBe('1,1|2,2|300,197|300,250|300,274|fluid');
  });
...
describe('initCommentAdverts', () => {
  ...
  it('should otherwise set the EventListener that can insert the slot', done => {
    ...
    initCommentAdverts().then(result => {
      ...
      expect(spyOn.mock.calls[0]).toEqual(expect.arrayContaining(['discussion:comments:get-more-replies']));
      done();
    });
  });
