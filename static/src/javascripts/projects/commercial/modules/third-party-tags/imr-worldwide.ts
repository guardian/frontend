...
const onLoad = () => {
  ...
  const subBrandApId = guMetadata[sectionFromMeta] || guMetadata['brand-only'];

  const sectionRef = sectionFromMeta in guMetadata ? sectionFromMeta : 'The Guardian - brand only';
