...
const findAnchor = (): Promise<HTMLElement | null> => {
  const selector = !(config.get('page.seriesId') || config.get('page.blogIds')) ? '.js-related, .js-outbrain-anchor' : '.js-outbrain-anchor';
...
const renderWidget = (widgetType: string, init: any): Promise<void> => {
  ...
  return findAnchor().then(anchorNode => fastdom.write(() => {
