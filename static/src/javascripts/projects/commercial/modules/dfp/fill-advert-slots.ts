...
const fillAdvertSlots = (): Promise<void> => {
  ...
  return Promise.all(dependencies).then(() => {
    ...
    const isDCRMobile = config.get('isDotcomRendering', false) && getBreakpoint() === 'mobile';
    // Get all ad slots
    const adverts = qwery(dfpEnv.adSlotSelector).filter(adSlot => !(adSlot.id in dfpEnv.advertIds)) // TODO: find cleaner workaround
    ...
    .filter(adSlot => !(isDCRMobile && adSlot.id === 'dfp-ad--top-above-nav')).map(adSlot => new Advert(adSlot));
