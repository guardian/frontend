...
import { Slot } from "commercial/types";

const adUnit = once(() => {
  const urlVars = getUrlVars();
  return urlVars['ad-unit'] ? `/${config.get('page.dfpAccountId')}/${urlVars['ad-unit']}` : config.get('page.adUnit');
});
...
const getSizeOpts = (sizesByBreakpoint: Object): Object => {
  ...
  const sizes = uniqBy(flatten(sizeMapping.map(size => size[1])), size => `${size[0]}-${size[1]}`);
...
const isEligibleForOutstream = (slotTarget: string | null | undefined): boolean => typeof slotTarget === 'string' && (slotTarget === 'inline1' || slotTarget === 'top-above-nav');
...
const defineSlot = (adSlotNode: Element, sizes: Object): Object => {
  ...
  if (config.get('switches.iasAdTargeting', false)) {
    ...
    const iasPETSlots = [{
      adSlotId: id,
      size: slot.getSizes().filter(size => size !== 'fluid').map(size => [size.getWidth(), size.getHeight()]),
    ...
    const iasDataCallback = targetingJSON => {
      ...
      */
      ...
      Object.keys(targeting.brandSafety).forEach(key => window.googletag.pubads().setTargeting(key, targeting.brandSafety[key]));
      if (targeting.fr) {
      ...
      Object.keys(targeting.slots[id]).filter(x => !ignoredKeys.includes(x)).forEach(key => slot.setTargeting(key, targeting.slots[id][key]));
    ...
    const iasTimeout = () => new Promise(resolve => {
  ...
  const fabricKeyValues = new Map([['top-above-nav', 'fabric1'], ['merchandising-high', 'fabric2'], ['merchandising', 'fabric3']]);
