...
const renderCreativeTemplate = (adSlot: Element, iFrame: HTMLIFrameElement): Promise<boolean> => {
  const fetchCreativeConfig = (): string | null | undefined => {
    try {
      const breakoutScript = iFrame.contentDocument.body ? iFrame.contentDocument.body.querySelector('.breakout__script[type="application/json"]') : null;
  ...
  const mergeViewabilityTracker = (json: Object): Object => {
    ...
    const viewabilityTracker = viewabilityTrackerDiv ? viewabilityTrackerDiv.childNodes[0].nodeValue.trim() : null;
  ...
  const renderCreative = (config: Object): Promise<boolean> => new Promise(resolve => {
    const Creative = creativeLookup[config.name];
    resolve(new Creative(adSlot, config.params, config.opts).create());
  });

  const hideIframe = (): Promise<any> => fastdom.mutate(() => {
    iFrame.style.display = 'none';
  });

  const creativeConfig: string | null | undefined = fetchCreativeConfig();

  if (creativeConfig) {
    return hideIframe().then(() => JSON.parse(creativeConfig)).then(mergeViewabilityTracker).then(renderCreative).catch(err => {
      reportError(Error(`Failed to get creative JSON ${err}`), { feature: 'commercial' }, false);
 ...
 */
const applyCreativeTemplate = (adSlot: Element): Promise<boolean> => getAdvertIframe(adSlot).then((iframe: HTMLIFrameElement) => renderCreativeTemplate(adSlot, iframe));
