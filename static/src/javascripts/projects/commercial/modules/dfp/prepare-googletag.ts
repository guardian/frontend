...
initMessenger(type, getStyles, initGetPageTargeting, initGetPageUrl, resize, hide, scroll, viewport, sendClick, background, disableRefresh);
...
const removeAdSlots = (): Promise<void> => {
  ...
  return fastdom.mutate(() => adSlots.forEach((adSlot: Element) => adSlot.remove()));
};

const setPublisherProvidedId = (): void => {
  const user: Object | null | undefined = getUserFromCookie();
  if (user) {
...
export const init = (): Promise<void> => {
  const setupAdvertising = (): Promise<void> => {
    ...
    window.googletag.cmd.push(setDfpListeners, setPageTargeting, setPublisherProvidedId, refreshOnResize, () => {
      fillAdvertSlots();
    });
    ...
    onConsentChange(state => {
      ...
      if (state.ccpa) {
        ...
        if (state.tcfv2) {
          // TCFv2 mode
          npaFlag = Object.keys(state.tcfv2.consents).length === 0 || Object.values(state.tcfv2.consents).includes(false);
        ...
        window.googletag.cmd.push(() => {
          window.googletag.pubads().setRequestNonPersonalizedAds(npaFlag ? 1 : 0);
        });
      }
      ...
      if (canRun) {
        loadScript(config.get('libs.googletag', '//www.googletagservices.com/tag/js/gpt.js'), { async: false });
      }
    });
