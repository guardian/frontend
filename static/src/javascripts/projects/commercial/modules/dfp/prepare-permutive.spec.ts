...
describe('Generating Permutive payload utils', () => {
  describe('isEmpty', () => {
    it('returns true for empty values', () => {
      ...
      emptyValues.forEach(emptyValue => expect(_.isEmpty(emptyValue)).toBe(true));
    });

    it('returns false for non empty values', () => {
      const values = ['word', -1, 0, 42, 2.35, ['word'], { word: true }, true, false];
  ...
  describe('generatePayload', () => {
    it('safely removes invalid authors or keywords', () => {
      ...
      invalidValues.forEach(invalid => expect(_.generatePayload({
    ...
    it('removes invalid date', () => {
      ...
      invalidDates.forEach(invalid => {
        expect(_.generatePayload({ page: { webPublicationDate: invalid } })).toEqual(expected);
      });
    });
  ...
  describe('runPermutive', () => {
    ...
    it("calls permutive's identify method, passing the ophan browser ID", () => {
      ...
      expect(mockPermutive.identify).toHaveBeenCalledWith([{ tag: 'ophan', id: bwid }]);
      expect(logger).not.toHaveBeenCalled();
    });
    it("calls permutive's identify before it calls addon, if the browser ID is present", () => {
      ...
      _.runPermutive(config, mockPermutive, logger);
      const [identifyCallOrder // $FlowFixMe Flow types for jest are missing invocationCallOrder
      ] = mockPermutive.identify.mock.invocationCallOrder;
      const [addonCallOrder // $FlowFixMe Flow types for jest are missing invocationCallOrder
