...
import { SlotVisibilityChangedEvent } from "commercial/types";
  ...
  if (storage.session.isAvailable()) {
    ...
    if (advert && advert.maxViewPercentage < event.inViewPercentage) {
      ...
      if (advert.maxViewPercentage <= 90 && event.inViewPercentage > 90) {
        const highVisibilitySlots = storage.session.get('gu.commercial.slotVisibility') || 0;
        storage.session.set('gu.commercial.slotVisibility', highVisibilitySlots + 1);
      }
