...
const AusWithConsentMock = (callback): void => callback({
  aus: { personalisedAdvertising: true }
});

const AusWithoutConsentMock = (callback): void => callback({
  aus: { personalisedAdvertising: true }
});
...
describe('init', () => {
  ...
  it('should initialise redplanet when all conditions are true with right params', async () => {
    ...
    expect(window.launchpad.mock.calls).toEqual([expectedNewTrackerCall, expectedTrackUnstructEventCall]);
  });
  ...
  it('should throw an error when on CCPA mode', async () => {
    ...
    getConsentFor.mockReturnValue(true);
    expect((await init)).toThrow(`Error running Redplanet without AUS consent. It should only run in Australia on AUS mode`);
  });
