...
import { SlotRenderEndedEvent } from "commercial/types";
 ...
 *
 */
const addClassIfHasClass = (newClassNames: Array<string>) => function hasClass(classNames) {
  return function onAdvertRendered(_, advert) {
    if (classNames.some(className => advert.node.classList.contains(className))) {
      return fastdom.mutate(() => {
        ...
        if (config.get('isDotcomRendering', false) && newClassNames.includes('ad-slot--fluid')) {
          advert.node.style.minHeight = '250px';
          advert.node.style.lineHeight = '10px';
          advert.node.style.padding = '0';
          advert.node.style.margin = '0';
          if (!newClassNames.includes('ad-slot--im') && !newClassNames.includes('ad-slot--carrot') && !newClassNames.includes('ad-slot--offset-right')) advert.node.style.width = '100%';
        }
      });
    }
...
const removeStyleFromAdIframe = (advert: Advert, style: string) => {
  const adIframe: HTMLElement | null | undefined = advert.node.querySelector('iframe');
...
const sizeCallbacks: {
  [key: string]: (arg0: any, arg1: any) => Promise<void>;
} = {};
 ...
 */
sizeCallbacks[adSizes.fluid] = (renderSlotEvent: any, advert: Advert) => addFluid(['ad-slot'])(renderSlotEvent, advert).then(() => removeStyleFromAdIframe(advert, 'vertical-align'));

/**
 * Trigger sticky scrolling for MPUs in the right-hand article column
 */
sizeCallbacks[adSizes.mpu] = (_, advert) => fastdom.measure(() => {
 ...
 */
sizeCallbacks[adSizes.halfPage] = (_, advert) => fastdom.measure(() => {
  if (advert.node.classList.contains('ad-slot--right')) {
    stickyMpu(advert.node);
  }
  if (advert.node.classList.contains('ad-slot--comments')) {
...
sizeCallbacks[adSizes.skyscraper] = (_, advert) => fastdom.measure(() => {
  if (advert.node.classList.contains('ad-slot--right')) {
    stickyMpu(advert.node);
  }
  if (advert.node.classList.contains('ad-slot--comments')) {
  ...
  return fastdom.mutate(() => advert.updateExtraSlotClasses('ad-slot--sky'));
});

sizeCallbacks[adSizes.video] = (_, advert) => fastdom.mutate(() => {
  advert.updateExtraSlotClasses('u-h');
});

sizeCallbacks[adSizes.outstreamDesktop] = (_, advert) => fastdom.mutate(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.outstreamGoogleDesktop] = (_, advert) => fastdom.mutate(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.outstreamMobile] = (_, advert) => fastdom.mutate(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.googleCard] = (_, advert) => fastdom.mutate(() => {
 ...
 */
sizeCallbacks[adSizes.portrait] = () => // remove geo most popular
geoMostPopular.whenRendered.then(popular => fastdom.mutate(() => {
 ...
 */
export const renderAdvert = (advert: Advert, slotRenderEndedEvent: SlotRenderEndedEvent): Promise<boolean> => {
  ...
  return applyCreativeTemplate(advert.node).then(isRendered => {
    const callSizeCallback = () => {
      if (advert.size) {
        ...
        return Promise.resolve(sizeCallbacks[size] ? sizeCallbacks[size](slotRenderEndedEvent, advert) : fastdom.mutate(() => {
    ...
    const addRenderedClass = () => isRendered ? fastdom.mutate(() => {
      advert.node.classList.add('ad-slot--rendered');
    }) : Promise.resolve();

    return callSizeCallback().then(() => renderAdvertLabel(advert.node)).then(addRenderedClass).then(() => isRendered);
  }).catch(err => {
    reportError(err, {
      feature: 'commercial'
    }, false);
