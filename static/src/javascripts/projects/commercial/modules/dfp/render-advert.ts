 ...
 */
sizeCallbacks[adSizes.mpu] = (_, advert) => fastdom.read(() => {
 ...
 */
sizeCallbacks[adSizes.halfPage] = (_, advert) => fastdom.read(() => {
  if (advert.node.classList.contains('ad-slot--right')) {
    stickyMpu(advert.node);
  }
  if (advert.node.classList.contains('ad-slot--comments')) {
...
sizeCallbacks[adSizes.skyscraper] = (_, advert) => fastdom.read(() => {
  if (advert.node.classList.contains('ad-slot--right')) {
    stickyMpu(advert.node);
  }
  if (advert.node.classList.contains('ad-slot--comments')) {
...
sizeCallbacks[adSizes.video] = (_, advert) => fastdom.write(() => {
  advert.updateExtraSlotClasses('u-h');
});

sizeCallbacks[adSizes.outstreamDesktop] = (_, advert) => fastdom.write(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.outstreamGoogleDesktop] = (_, advert) => fastdom.write(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.outstreamMobile] = (_, advert) => fastdom.write(() => {
  advert.updateExtraSlotClasses('ad-slot--outstream');
});

sizeCallbacks[adSizes.googleCard] = (_, advert) => fastdom.write(() => {
...
geoMostPopular.whenRendered.then(popular => fastdom.write(() => {
  ...
  return applyCreativeTemplate(advert.node).then(isRendered => {
    const callSizeCallback = () => {
      if (advert.size) {
        ...
        return Promise.resolve(sizeCallbacks[size] ? sizeCallbacks[size](slotRenderEndedEvent, advert) : fastdom.write(() => {
    ...
    const addRenderedClass = () => isRendered ? fastdom.write(() => {
