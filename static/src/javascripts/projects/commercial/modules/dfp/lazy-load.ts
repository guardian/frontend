...
const onIntersect = (entries: Array<IntersectionObserverEntry>, observer: IntersectionObserver): void => {
  ...
  dfpEnv.advertsToLoad = dfpEnv.advertsToLoad.filter(advert => advertIds.indexOf(advert.id) < 0);
};

const getObserver = once(() => Promise.resolve(new window.IntersectionObserver(onIntersect, {
