...
const {
  fillAdvertSlots: actualFillAdvertSlots
} = jest.requireActual('commercial/modules/dfp/fill-advert-slots');
...
describe('DFP', () => {
  ...
  beforeEach(() => {
    ...
    $style = $.create('<style type="text/css"></style>').html(`body:after{ content: "wide"}`).appendTo('head');
    const pubAds = {
      listeners: [],
  ...
  it('should get the slots', () => new Promise(resolve => {
  ...
  it('should set listeners', () => prepareGoogletag().then(() => {
    expect(window.googletag.pubads().addEventListener).toHaveBeenCalledWith('slotRenderEnded', expect.anything());
  }));

  it('should define slots', () => new Promise(resolve => {
    fillAdvertSlots.mockImplementation(() => {
      actualFillAdvertSlots().then(resolve);
    });
    ...
    [['dfp-ad-html-slot', [[300, 50]], [[[0, 0], [[300, 50]]]], 'html-slot'], ['dfp-ad-script-slot', [[300, 50], [320, 50]], [[[0, 0], [[300, 50], [320, 50]]]], 'script-slot'], ['dfp-ad-already-labelled', [[728, 90], [300, 50], [320, 50]], [[[740, 0], [[728, 90]]], [[0, 0], [[300, 50], [320, 50]]]], 'already-labelled'], ['dfp-ad-dont-label', [[728, 90], [900, 250], [970, 250], [300, 50], [320, 50]], [[[980, 0], [[728, 90], [900, 250], [970, 250]]], [[740, 0], [[728, 90]]], [[0, 0], [[300, 50], [320, 50]]]], 'dont-label']].forEach(data => {
  ...
  describe('keyword targeting', () => {
    it('should send page level keywords', () => {
      onConsentChange.mockImplementation(callback => callback(tcfv2WithConsent));
  ...
  describe('NPA flag is set correctly', () => {
    it('when full TCF consent was given', () => {
      ...
      getConsentFor.mockReturnValue(true);
      prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(0);
      });
    });
    it('when no TCF consent preferences were specified', () => {
      onConsentChange.mockImplementation(callback => callback(tcfv2NullConsent));
      getConsentFor.mockReturnValue(true);
      prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(0);
      });
    ...
    it('when full TCF consent was denied', () => {
      onConsentChange.mockImplementation(callback => callback(tcfv2WithoutConsent));
      getConsentFor.mockReturnValue(false);
      prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(1);
      });
    });
    it('when only partial TCF consent was given', () => {
      onConsentChange.mockImplementation(callback => callback(tcfv2MixedConsent));
      getConsentFor.mockReturnValue(false);
      prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(1);
      });
  ...
  describe('NPA flag in AUS', () => {
    it('when AUS has not retracted advertising consent', () => {
      onConsentChange.mockImplementation(callback => callback(ausNotRejected));
      getConsentFor.mockReturnValue(true);
      return prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(0);
      });
    ...
    it('when AUS has retracted advertising consent', () => {
      onConsentChange.mockImplementation(callback => callback(ausRejected));
      getConsentFor.mockReturnValue(false);
      return prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setRequestNonPersonalizedAds).toHaveBeenCalledWith(1);
      });
  ...
  describe('restrictDataProcessing flag is set correctly', () => {
    it('when CCPA consent was given', () => {
      onConsentChange.mockImplementation(callback => callback(ccpaWithConsent));
      getConsentFor.mockReturnValue(true);
      prepareGoogletag().then(() => {
    ...
    it('when CCPA consent was denied', () => {
      onConsentChange.mockImplementation(callback => callback(ccpaWithoutConsent));
      getConsentFor.mockReturnValue(false);
      prepareGoogletag().then(() => {
        expect(window.googletag.pubads().setPrivacySettings).toHaveBeenCalledWith({
