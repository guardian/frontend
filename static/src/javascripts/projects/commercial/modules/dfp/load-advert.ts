...
export const loadAdvert = (advert: Advert): void => {
  advert.whenSlotReady.catch(() => {// The display needs to be called, even in the event of an error.
  }).then(() => {
    markTime(`Commercial: Slot Ready: ${advert.id}`);
    advert.startLoading();
    return Promise.all([prebid.requestBids(advert), a9.requestBids(advert)]);
  }).then(() => {
...
export const refreshAdvert = (advert: Advert): void => {
  ...
  advert.whenSlotReady.then(() => {
    const prebidPromise = prebid.requestBids(advert, prebidSlot => forcedSlotSize(advert, prebidSlot));

    const a9Promise = a9.requestBids(advert, a9Slot => forcedSlotSize(advert, a9Slot));
