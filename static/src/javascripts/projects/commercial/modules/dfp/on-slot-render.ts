...
const reportEmptyResponse = (adSlotId: string, event: SlotRenderEndedEvent): void => {
  ...
  if (Math.random() < 0.0001) {
    ...
    const adTargetingKValues = adTargetingKeys.includes('k') ? event.slot.getTargeting('k') : [];
...
const outstreamSizes = [adSizes.outstreamDesktop.toString(), adSizes.outstreamMobile.toString(), adSizes.outstreamGoogleDesktop.toString()];
...
export const onSlotRender = (event: SlotRenderEndedEvent): void => {
  ...
  if (event.isEmpty) {
    ...
    const isNonRefreshableLineItem = event.lineItemId && config.get('page.dfpNonRefreshableLineItemIds', []).includes(event.lineItemId);

    advert.shouldRefresh = isNotFluid && !isOutstream && !config.get('page.hasPageSkin') && !isNonRefreshableLineItem;
