...
type Field = {
  ...
  decoder?: (input: string, output: {}, startPosition?: number) => {fieldValue?: unknown;newPosition?: number;};
...
const padLeft = (string: string, padding: number): string => repeat(Math.max(0, padding)).toString() + string;

const padRight = (string: string, padding: number): string => string + repeat(Math.max(0, padding));

const encodeIntToBits = (number: unknown, numBits: number | null | undefined): string => {
  let bitString = '';
...
const encodeBoolToBits = (value: unknown): string => encodeIntToBits(value === true ? 1 : 0, 1);

const encodeDateToBits = (date: unknown, numBits: number): string => {
...
const decodeBitsToInt = (bitString: string, start: number, length: number): number => {
  const str = bitString.substr(start, length);
  return parseInt(str, 2);
};

const decodeBitsToDate = (bitString: string, start: number, length: number): Date => new Date(decodeBitsToInt(bitString, start, length) * 100);
...
const decode6BitCharacters = (bitString: string, start: number, length: number): string => {
  let decoded = '';
...
const encodeFields = ({
  input,
  fields
}: {input: {};fields: Array<Field>;}) => fields.reduce((acc, field) => {
...
const encodeField = ({
  ...
  const inputValue: unknown = input[name];
  const fieldValue: unknown = inputValue === null || inputValue === undefined ? '' : inputValue;
  switch (type) {
    case 'int':
...
const decodeField = ({
  ...
  startPosition,
  field
}): {fieldValue?: unknown;newPosition?: number;} => {
  const {
    type,
    numBits,
    decoder,
 ...
 */
const encodeCookieValue = (data: {cookieVersion: number;}, definitionMap: {}): string | null | undefined => {
...
const decodeCookieBitValue = (bitString: string, definitionMap: DefinitionMap) => {
...
const decodeCookieValue = (cookieValue: string, definitionMap: {}) => {
  ...
  const unsafe = cookieValue.replace(/-/g, '+').replace(/_/g, '/') + '=='.substring(0, 3 * cookieValue.length % 4);
