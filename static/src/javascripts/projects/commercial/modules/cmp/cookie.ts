...
import { padRight, encodeCookieValue, decodeCookieValue } from "commercial/modules/cmp/cookieutils";

import { VendorConsentData, Range, ShortVendorList } from "./types";

const encodeVendorCookieValue = (data: VendorConsentData): string | null | undefined => {
  if (data && data.cookieVersion) {
    return encodeCookieValue(data, vendorVersionMap);
  }
};

const decodeVendorCookieValue = (cookieValue: string) => decodeCookieValue(cookieValue, vendorVersionMap);

const encodeVendorIdsToBits = (maxVendorId: number, selectedVendorIds: Array<number> = []): string => {
  let vendorString = '';
...
const encodePurposeIdsToBits = (purposeIDs: Array<number>, selectedPurposeIds: Array<number>): string => {
...
const convertVendorsToRanges = (maxVendorId: number, selectedIds: Array<number> = []): Array<Range> => {
  let range = [];
  const ranges = [];
  for (let id = 1; id <= maxVendorId; id += 1) {
...
const encodeVendorConsentData = (data: VendorConsentData, shortVendorList?: ShortVendorList): string => {
  const {
    selectedPurposeIds = [],
    selectedVendorIds = [],
    maxVendorId
  ...
  const noRangesData: string | null | undefined = encodeVendorCookieValue({
    ...data,
    maxVendorId,
  ...
  const rangesData: string | null | undefined = encodeVendorCookieValue({
    ...data,
    maxVendorId,
    purposeIdBitString: encodePurposeIdsToBits(purposeIDs, selectedPurposeIds),
    isRange: true,
...
const decodeVendorConsentData = (cookieValue: string): VendorConsentData => {
  ...
  if (isRange) {
    const idMap = vendorRangeList.reduce( // eslint-disable-next-line no-shadow
    (acc, {
      isRange,
      startVendorId,
      endVendorId
...
const readVendorConsentCookie = (): VendorConsentData | false => {
  const cookieVal: string | null | undefined = getCookie(VENDOR_CONSENT_COOKIE_NAME);
  if (cookieVal) {
...
const writeVendorConsentCookie = (vendorConsentData: VendorConsentData): Promise<void> => {
  log.debug(`Attempting to write consent data to cookie`, vendorConsentData);
  return Promise.resolve(addCookie(VENDOR_CONSENT_COOKIE_NAME, encodeVendorConsentData(vendorConsentData), VENDOR_CONSENT_COOKIE_MAX_AGE, true));
};

export { encodeVendorConsentData, decodeVendorConsentData, readVendorConsentCookie, writeVendorConsentCookie };
