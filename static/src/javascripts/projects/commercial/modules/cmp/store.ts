...
const generateVendorConsentResponse = (vendorConsentData: VendorConsentData, shortVendorList: ShortVendorList, vendorIds: Array<number> | null | undefined): ConsentSelection => {
  const {
    maxVendorId = 0,
    selectedVendorIds = [],
    selectedPurposeIds = []
...
const generateVendorData = (canPersonalise: boolean, shortVendorList: ShortVendorList): SelectedData => {
  const allVendors = Object.keys(shortVendorList.purposesByVID).map(s => parseInt(s, 10));
...
*/
const generateConsentData = (cmpId: number, cmpVersion: number, cookieVersion: number, vendorListVersion: number): ConsentData => {
...
const getVendorConsentData = (cmpId: number, cmpVersion: number, cookieVersion: number, canPersonalise: boolean | null, shortVendorList: ShortVendorList): VendorConsentData | null | undefined => {
  const cookieVal = readVendorConsentCookie();
  if (cookieVal) return { ...cookieVal };

  if (typeof canPersonalise === 'boolean') {
    const consentData = generateConsentData(cmpId, cmpVersion, cookieVersion, shortVendorList.version);
...
export class CmpStore {
  ...
  constructor(cmpId: number, cmpVersion: number, cookieVersion: number, canPersonalise: boolean | null, shortVendorList: ShortVendorList) {
    ...
    this.allowedVendorIds = Object.keys(shortVendorList.purposesByVID).map(s => parseInt(s, 10));
    this.consentData = generateConsentData(cmpId, cmpVersion, cookieVersion, shortVendorList.version);
    this.vendorConsentData = getVendorConsentData(cmpId, cmpVersion, cookieVersion, canPersonalise, shortVendorList);
  }

  getVendorConsentsObject = (vendorIds: Array<number> | null | undefined): VendorConsentResponse | null | undefined => {
    if (this.vendorConsentData) {
      const consentDataResponse = generateVendorConsentResponse(this.vendorConsentData, this.shortVendorList, vendorIds);
      log.info('GetVendorConsentsObject: Successfully generated response');
      return {
        ...this.consentData,
        ...consentDataResponse
      };
