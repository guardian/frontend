...
const createCommentSlots = (canBeDmpu: boolean): Array<HTMLDivElement | HTMLSpanElement> => {
...
const insertCommentAd = ($commentMainColumn: bonzo, $adSlotContainer: bonzo, canBeDmpu: boolean): Promise<void> => {
  ...
  return fastdom.write(() => {
    ...
    if (!config.get('page.isLiveBlog') && !config.get('page.isMinuteArticle')) {
...
const maybeUpgradeSlot = (ad: Advert, $adSlot: bonzo): Advert => {
  if (!containsDMPU(ad)) {
    ...
    fastdom.write(() => {
      $adSlot[0].setAttribute('data-desktop', '1,1|2,2|300,250|300,274|fluid|300,600|160,600');
    });
  }
  return ad;
};
...
export const initCommentAdverts = (): Promise<boolean> => {
  ...
  mediator.once('modules:comments:renderComments:rendered', (): void => {
    ...
    fastdom.read(() => $commentMainColumn.dim().height).then((mainColHeight: number) => {
