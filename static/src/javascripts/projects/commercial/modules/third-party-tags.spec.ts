...
import { getConsentFor as getConsentFor_, onConsentChange as onConsentChange_ } from "@guardian/consent-management-platform";
...
const tcfv2AllConsentMock = (callback): void => callback({
  tcfv2: {
    consents: {
      '1': true,
      '2': true,
...
describe('third party tags', () => {
  ...
  describe('insertScripts', () => {
    ...
    it('should add scripts to the document when TCFv2 consent has been given', () => {
      ...
      insertScripts([fakeThirdPartyAdvertisingTag], [fakeThirdPartyPerformanceTag]);
    ...
    it('should add scripts to the document when CCPA consent has been given', () => {
      onConsentChange.mockImplementation(callback => callback({ ccpa: { doNotSell: false } }));
      getConsentFor.mockReturnValue(true);
      insertScripts([fakeThirdPartyAdvertisingTag], [fakeThirdPartyPerformanceTag]);
      expect(document.scripts.length).toBe(3);
    });
    it('should only add performance scripts to the document when CCPA consent has not been given', () => {
      onConsentChange.mockImplementation(callback => callback({ ccpa: { doNotSell: true } }));
      getConsentFor.mockReturnValue(false);
      insertScripts([fakeThirdPartyAdvertisingTag], [fakeThirdPartyPerformanceTag]);
      expect(document.scripts.length).toBe(2);
    });
    ...
    it('should only add consented custom vendors to the document for TCFv2', () => {
      ...
      insertScripts([fakeThirdPartyAdvertisingTag, fakeThirdPartyAdvertisingTag2], []);
      expect(document.scripts.length).toBe(2);
    });
