...
import type {
    ...
    PrebidSonobiParams,
    PrebidSize,
    PrebidBid } from 'commercial/modules/prebid/types';
...
function filterConfigEntries(bidder: string, slotSizes: PrebidSize[]): PrebidAdSlotCriteria[] {
    ...
    bidConfigEntries = bidConfigEntries.filter(bid => {
        return slotSizes.find(size => isEqualAdSize(size, bid.size));
    });
...
function getMatchingBids(availableSizes: PrebidSize[], matchedSizes: PrebidSize[], slotId: string): PrebidBid[] {
    let bids: PrebidBid[] = [];
    PrebidTestService.bidders.forEach( (bidder: PrebidBidder) => {
        const matchingConfigEntries: PrebidAdSlotCriteria[] = filterConfigEntries(bidder.name, availableSizes);
