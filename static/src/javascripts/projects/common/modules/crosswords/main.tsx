...
const initCrosswords = (): void => {
  fastdom.measure(() => document.getElementsByClassName('js-crossword')).then(elements => {
    Array.from(elements).forEach(element => {
      const data = element.getAttribute('data-crossword-data');

      if (!data) {
        throw new Error('JavaScript crossword without associated data in data-crossword-data');
      }

      const crosswordComponent = render(<Crossword data={JSON.parse(data)} />, element);

      const entryId = window.location.hash.replace('#', '');
      const entry = crosswordComponent.props.data.entries.find(val => val.id === entryId);
      ...
      bean.on(element, 'click', $('.crossword__clue'), e => {
        ...
        const newEntry = crosswordComponent.props.data.entries.find(val => val.id === newEntryId);
