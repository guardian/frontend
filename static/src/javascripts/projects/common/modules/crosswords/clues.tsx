...
class Clue extends Component<any, any> {
  ...
  render() {
    return <li>
                <a href={`#${this.props.id}`} onClick={this.onClick.bind(this)} className={classNames({
...
class Clues extends Component<any, any> {
  ...
  componentDidMount() {
    this.$cluesNode = (findDOMNode(this.clues) as any);
  ...
  componentDidUpdate(prev: Object) {
    if (isBreakpoint({
      min: 'tablet',
      max: 'leftCol'
    }) && (this.props.focussed && (!prev.focussed || prev.focussed.id !== this.props.focussed.id))) {
      fastdom.measure(() => {
  ...
  scrollIntoView(clue: Object) {
    const buffer = 100;
    const node: HTMLElement = (findDOMNode(this[clue.id]) as any);
    const visible = node.offsetTop - buffer > this.$cluesNode.scrollTop && node.offsetTop + buffer < this.$cluesNode.scrollTop + this.$cluesNode.clientHeight;
  ...
  render() {
    ...
    const cluesByDirection = direction => this.props.clues.filter(clue => clue.entry.direction === direction).map(clue => <Clue ref={clueRef => {
      this[clue.entry.id] = clueRef;
    }} id={clue.entry.id} key={clue.entry.id} number={clue.entry.number} humanNumber={clue.entry.humanNumber} clue={clue.entry.clue} hasAnswered={clue.hasAnswered} isSelected={clue.isSelected} setReturnPosition={() => {
      this.props.setReturnPosition(window.scrollY);
    }} />);

    return <div className={`crossword__clues--wrapper ${this.state.showGradient ? '' : 'hide-gradient'}`}>
