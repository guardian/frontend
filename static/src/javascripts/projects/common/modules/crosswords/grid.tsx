...
export type GridProps = {
  ...
  crossword: Crossword;
  focussedCell: Position | null | undefined;
};

// Position at end of previous cell
const createWordSeparator = (x: number, y: number, direction: Direction): React$Node | null | undefined => {
  ...
  if (direction === 'across') {
    const width = 1;
    return <rect x={left - borderWidth - width} y={top} key={['sep', direction, x, y].join('_')} width={width} height={constants.cellSize} />;
  } else if (direction === 'down') {
    const height = 1;
    return <rect x={left} y={top - borderWidth - height} key={['sep', direction, x, y].join('_')} width={constants.cellSize} height={height} />;
  }
};

// Position in-between this and previous cells
const createHyphenSeparator = (x: number, y: number, direction: Direction): React$Node | null | undefined => {
  const top = gridSize(y);
  const left = gridSize(x);
  const borderWidth = 1;
  let width;
  ...
  if (direction === 'across') {
    ...
    height = 1;
    return <rect x={left - borderWidth / 2 - width / 2} y={top + constants.cellSize / 2 + height / 2} key={['sep', direction, x, y].join('_')} width={width} height={height} />;
  } else if (direction === 'down') {
    width = 1;
    height = constants.cellSize / 4;
    return <rect x={left + constants.cellSize / 2 + width / 2} y={top - borderWidth / 2 - height / 2} key={['sep', direction, x, y].join('_')} width={width} height={height} />;
  }
};

const createSeparator = (x: number, y: number, separatorDescription: SeparatorDescription | null | undefined): React$Node | null | undefined => {
...
export const Grid = (props: GridProps): React$Node => {
  const getSeparators = (x: number, y: number): SeparatorDescription | null | undefined => props.separators[clueMapKey(x, y)];
  ...
  range(props.rows).forEach(y => range(props.columns).forEach(x => {
    ...
    if (cellProps.isEditable) {
      ...
      cells.push(<GridCell {...Object.assign({}, cellProps, {
        handleSelect,
        x,
        y,
        key: `cell_${x}_${y}`,
        isHighlighted,
        isFocussed: props.focussedCell && x === props.focussedCell.x && y === props.focussedCell.y
      }, this)} />);
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} className={classNames({
