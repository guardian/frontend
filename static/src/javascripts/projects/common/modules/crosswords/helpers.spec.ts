...
import { isAcross, otherDirection, buildGrid, clueMapKey, buildClueMap, cellsForEntry, cellsForClue, entryHasCell, getAnagramClueData, clueIsInGroup, getGroupEntriesForClue, getNumbersForGroupedEntries, getClueForGroupedEntries, getAllSeparatorsForGroup, getTtotalLengthOfGroup, cluesAreInGroup, checkClueHasBeenAnswered, getClearableCellsForClue } from "common/modules/crosswords/helpers";
...
describe('Helpers', () => {
  describe('isAcross', () => {
    ...
    describe('buildGrid', () => {
      ...
      it('should set entries to not editable by default', () => {
        ...
        expect(grid.every(column => column.every(({
          isEditable
        }) => isEditable === false))).toBe(true);
      });
      ...
      it('should set values from the state', () => {
        const grid = buildGrid(5, 5, [entryFixture], [['', '', '', '', ''], ['', '', '', '', ''], ['', '', '', 'W', ''], ['', '', '', '', ''], ['', '', '', '', '']]);
  ...
  describe('cellsForClue', () => {
    ...
    it('should return all cells for a grouped entry', () => {
      ...
      const expectedCells = [{ x: 3, y: 0 }, { x: 3, y: 1 }, { x: 3, y: 2 }, { x: 3, y: 3 }, { x: 3, y: 4 }, { x: 3, y: 5 }, { x: 3, y: 6 }, { x: 3, y: 7 }, { x: 5, y: 3 }, { x: 6, y: 3 }, { x: 7, y: 3 }, { x: 8, y: 3 }, { x: 9, y: 3 }, { x: 10, y: 3 }, { x: 11, y: 3 }, { x: 12, y: 3 }, { x: 13, y: 3 }, { x: 13, y: 11 }, { x: 13, y: 12 }, { x: 13, y: 13 }, { x: 13, y: 14 }, { x: 1, y: 11 }, { x: 2, y: 11 }, { x: 3, y: 11 }, { x: 4, y: 11 }, { x: 5, y: 11 }, { x: 6, y: 11 }, { x: 7, y: 11 }, { x: 8, y: 11 }, { x: 9, y: 11 }];
  ...
  describe('checkClueHasBeenAnswered', () => {
    const gridFixture = [['R', 'I', 'V', 'E', 'R', ''].map(stubCellWithValue), ['', '', '', '', '', ''].map(stubCellWithValue), ['', '', '', '', '', ''].map(stubCellWithValue), ['', '', '', '', '', ''].map(stubCellWithValue), ['', 'T', '2', '0', '', ''].map(stubCellWithValue), ['', '', '', '', '', ''].map(stubCellWithValue)];
  ...
  describe('getClearableCellsForEntry', () => {
    ...
    const gridFixture = [['R', 'I', 'V', 'E', 'R', '', 'C', '', 'L', 'U', 'S', 'T', 'R', 'E'].map(stubCellWithValue), ['', '', '', '', '', '', 'L', '', 'I', '', '', '', '', ''].map(stubCellWithValue), ['S', 'A', 'N', 'T', 'A', '', 'A', '', 'G', 'I', 'S', 'T', '', ''].map(stubCellWithValue), ['', '', '', '', '', '', 'U', '', 'H', '', 'O', '', '', ''].map(stubCellWithValue), ['', '', '', '', '', '', 'S', '', 'T', '', 'u', '', '', ''].map(stubCellWithValue), ['', 'N', '', '', '', '', 'E', '', 'S', '', 'T', '', '', ''].map(stubCellWithValue), ['', 'O', '', 'F', '', '', '', '', '', '', 'H', '', '', ''].map(stubCellWithValue), ['', 'R', 'A', 'I', 'L', 'R', 'O', 'A', 'D', '', 'E', '', '', ''].map(stubCellWithValue), ['', 'T', '', 'G', '', '', '', '', '', 'C', 'R', 'O', 'S', 'S'].map(stubCellWithValue), ['', 'H', '', 'H', '', '', '', '', '', '', 'N', '', '', ''].map(stubCellWithValue), ['', 'E', '', 'T', '', '', '', '', '', '', '', '', '', ''].map(stubCellWithValue), ['', 'R', '', 'S', 'O', 'U', 'T', 'H', 'E', 'R', 'N', '', '', ''].map(stubCellWithValue), ['', 'N', '', '', '', '', '', '', '', '', '', '', '', ''].map(stubCellWithValue)];
    ...
    const theEntriesFixture = [oneDownFixture, twoDownFixture, threeDownFixture, fourDownFixture, fiveDownFixture, sixDownFixture, sevenDownFixture, oneAcrossFixture, twoAcrossFixture, threeAcrossFixture, fourAcrossFixture, fiveAcrossFixture];
    ...
    it('should return all cells for an ungrouped clue which does not intersect other answered clues', () => {
      ...
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, oneDownFixture)).toEqual(expectedCells);
    });
    ...
    it('should return all correct cells for an ungrouped clue which intersects an unanswered clue', () => {
      ...
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, fourAcrossFixture)).toEqual(expectedCells);
    });

    // SANTACLAUSE
    it('should return all cells for a grouped clue which does not intersect other answered clues', () => {
      const expectedCells = [{ x: 2, y: 0 }, { x: 2, y: 1 }, { x: 2, y: 2 }, { x: 2, y: 3 }, { x: 2, y: 4 }, { x: 0, y: 6 }, { x: 1, y: 6 }, { x: 2, y: 6 }, { x: 3, y: 6 }, { x: 4, y: 6 }, { x: 5, y: 6 }];
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, twoDownFixture)).toEqual(expectedCells);
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, oneAcrossFixture)).toEqual(expectedCells);
    });
    ...
    it('should return all cells for a grouped clue which intersects other completed clues', () => {
      const expectedCells = [{ x: 5, y: 1 }, { x: 6, y: 1 }, { x: 8, y: 1 }, { x: 9, y: 1 }, { x: 10, y: 1 }, { x: 11, y: 1 }, { x: 12, y: 1 }, { x: 1, y: 8 }, { x: 3, y: 8 }, { x: 4, y: 8 }, { x: 5, y: 8 }];
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, twoAcrossFixture)).toEqual(expectedCells);
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, threeAcrossFixture)).toEqual(expectedCells);
    });
    ...
    it('should return the correct cells where entries in a grouped clue intersect each other', () => {
      const expectedCells = [{ x: 3, y: 10 }, { x: 4, y: 10 }, { x: 5, y: 10 }, { x: 6, y: 10 }, { x: 7, y: 10 }, { x: 8, y: 10 }, { x: 9, y: 10 }, { x: 8, y: 9 }, { x: 8, y: 11 }, { x: 8, y: 12 }, { x: 8, y: 13 }];
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, fiveAcrossFixture)).toEqual(expectedCells);
      expect(getClearableCellsForClue(gridFixture, clueMapFixture, theEntriesFixture, sevenDownFixture)).toEqual(expectedCells);
    });
  });
});