...
import { buildClueMap, buildGrid, otherDirection, entryHasCell, cluesFor, mapGrid, getClearableCellsForClue, getLastCellInClue, getPreviousClueInGroup, isFirstCellInClue, getNextClueInGroup, isLastCellInClue, gridSize, checkClueHasBeenAnswered, buildSeparatorMap, cellsForEntry } from "common/modules/crosswords/helpers";
import { keycodes } from "common/modules/crosswords/keycodes";
import { saveGridState, loadGridState } from "common/modules/crosswords/persistence";
import { classNames } from "common/modules/crosswords/classNames";
import { GridProps } from "common/modules/crosswords/grid";

type CrosswordState = {
  grid: Array<Array<Cell>>;
  cellInFocus?: Position | null | undefined;
  directionOfEntry?: Direction | null | undefined;
  showAnagramHelper?: boolean;
};

class Crossword extends Component<any, CrosswordState> {
  ...
  constructor(props: Object) {
    ...
    this.state = {
      grid: buildGrid(dimensions.rows, dimensions.cols, this.props.data.entries, loadGridState(this.props.data.id)),
      cellInFocus: null,
  ...
  componentDidMount(): void {
    ...
    mediator.on('window:orientationchange', debounce(this.setGridHeight.bind(this), 200));
    this.setGridHeight();
  ...
  onSelect(x: number, y: number): void {
    ...
    const isInsideFocussedClue = (): boolean => focussedClue ? entryHasCell(focussedClue, x, y) : false;
    ...
    if (cellInFocus && cellInFocus.x === x && cellInFocus.y === y && this.state.directionOfEntry) {
      ...
      const isStartOfClue = (sourceClue): boolean => !!sourceClue && sourceClue.position.x === x && sourceClue.position.y === y;
  ...
  onClearSingle(): void {
    ...
    if (clueInFocus) {
      ...
      const cellsInFocus = getClearableCellsForClue(this.state.grid, this.clueMap, this.props.data.entries, clueInFocus);
  ...
  clueMap: any;
  $gridWrapper: any;
  gridHeightIsSet: boolean | null | undefined;
  returnPosition: number | null | undefined;
  ...
  focusHiddenInput(x: number, y: number): void {
    const wrapper: HTMLElement = (findDOMNode(this.hiddenInputComponent.wrapper) as any);
    ...
    const hiddenInputNode: HTMLElement = (findDOMNode(this.hiddenInputComponent.input) as any);
  ...
  clueInFocus(): Object | null | undefined {
    if (this.state.cellInFocus) {
  ...
  clueIsInFocusGroup(clue: Object): boolean {
    if (this.state.cellInFocus) {
      ...
      if (this.state.directionOfEntry && cluesForCell[this.state.directionOfEntry]) {
        return cluesForCell[this.state.directionOfEntry].group.includes(clue.id);
      }
    }
    return false;
  }
  ...
  check(entry: Object): void {
    ...
    if (entry.solution) {
      const badCells = zip(cells, entry.solution.split('')).filter(cellAndSolution => {
  ...
  isHighlighted(x: number, y: number): boolean {
    ...
    return focused ? focused.group.some(id => {
  ...
  render(): React$Node {
    ...
    const anagramHelper = this.state.showAnagramHelper && <AnagramHelper crossword={this} focussedEntry={focused} entries={this.props.data.entries} grid={this.state.grid} close={this.onToggleAnagramHelper} />;
    ...
    return <div className={`crossword__container crossword__container--${this.props.data.crosswordType} crossword__container--react`} data-link-name="Crosswords">
        ...
        this.game = game;
          ...
          this.stickyClueWrapper = stickyClueWrapper;
            ...
            'is-hidden': !focused
                            ...
                            {focused && <div className="crossword__sticky-clue__inner">
          ...
          this.gridWrapper = gridWrapper;
                        ...
                        {Grid(gridProps)}
                        <HiddenInput crossword={this} value={this.hiddenInputValue()} ref={hiddenInputComponent => {
            this.hiddenInputComponent = hiddenInputComponent;
                ...
                <Clues clues={this.cluesData()} focussed={focused} setReturnPosition={this.setReturnPosition.bind(this)} />
            </div>;
  }
}
