...
const getAllSeparatorsForGroup = (clues: Clue[]): SeparatorLocations => {
    const k = {};
...
const getClueForGroupedEntries = (clueGroup: Clue[]): string =>
    clueGroup[0].clue;

const getNumbersForGroupedEntries = (clueGroup: Clue[]): number | string =>
    clueGroup[0].humanNumber;

const getTtotalLengthOfGroup = (clueGroup: Clue[]): number =>
    clueGroup.reduce((total, clue) => total + clue.length, 0);

const getAnagramClueData = (entries: Clue[], clue: Clue): Clue | GroupClue => {
...
const cluesAreInGroup = (clue: Clue, otherClue: Clue): boolean =>
    otherClue.group.includes(clue.id);

const cellsForEntry = (entry: Clue): Position[] =>
    isAcross(entry)
...
const cellsForClue = (entries: Clue[], clue: Clue): Position[] => {
    if (clueIsInGroup(clue)) {
...
const cluesFor = (clueMap: ClueMap, x: number, y: number): CluesIntersect =>
...
const buildClueMap = (clues: Clue[]): ClueMap => {
    const map = {};
...
const buildSeparatorMap = (clues: Clue[]): SeparatorMap => {
