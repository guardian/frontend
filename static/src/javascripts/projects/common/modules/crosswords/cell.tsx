...
class Cell extends Component<any, any> {
  ...
  render() {
    ...
    if (this.props.number !== undefined) {
      cellNumber = <text x={left + 1} y={top + constants.numberSize} key="number" className="crossword__cell-number">
                    {this.props.number}
                </text>;
    }
    ...
    if (this.props.value !== undefined) {
      cellValue = <text x={left + constants.cellSize * 0.5} y={top + constants.cellSize * 0.675} key="entry" className={classNames({
    ...
    return <g onClick={this.onClick.bind(this)}>
                <rect x={left} y={top} width={constants.cellSize} height={constants.cellSize} className={classNames({
