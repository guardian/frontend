...
import { getEngagementBannerTestsFromGoogleDoc } from "common/modules/commercial/contributions-utilities";
...
const appendOverlay = (): Promise<void> => {
  ...
  return Promise.all([getEngagementBannerTestsFromGoogleDoc()]).then(([asyncEpicTests, asyncBannerTests]) => {
    const data = {
      testGroups: [{
        name: 'Epic',
