...
class TailorSurvey {
    ...
    constructor() {
        ...
        const getSurveyIdsNotToShow = () => {
            ...
            const values = currentCookieValues
                ? currentCookieValues.split(',')
                : [];
            ...
            return tailor.getSuggestedSurvey(queryParams).then(suggestion => {
                if (suggestion) {
                    ...
                    const survey = bonzo.create(
                        template(tailorSurveyHtml, json)
                    );

                    return inArticleWriter(
                        survey,
                        suggestion.data.survey.surveyId
                    );
                }
                Promise.resolve();
            });
        ...
        const handleSurveyResponse = surveyId => {
            ...
            surveyQuestions.forEach(question => {
                bean.on(question, 'click', event => {
                    if (
                        event.target.attributes.getNamedItem('data-link-name')
                    ) {
