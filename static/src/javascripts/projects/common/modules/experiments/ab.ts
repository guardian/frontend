...
import { concurrentTests, engagementBannerTests, epicTests as hardcodedEpicTests } from "common/modules/experiments/ab-tests";
import { getEngagementBannerTestsFromGoogleDoc, getConfiguredLiveblogEpicTests } from "common/modules/commercial/contributions-utilities";

export const getLiveblogEpicTest = memoize((): Promise<Runnable<ABTest> | null | undefined> => {
...
export const getAsyncTestsToRun = (): Promise<ReadonlyArray<Runnable<ABTest>>> => Promise.all([getLiveblogEpicTest(), getEngagementBannerTestToRun()]).then(tests => tests.filter(Boolean));
