...
export const getLiveblogEpicTest = memoize(
    (): Promise<Runnable<ABTest> | null | undefined> => {
        if (config.get('page').contentType === 'LiveBlog') {
            return getConfiguredLiveblogEpicTests().then(
                (configuredEpicTests) => {
                    configuredEpicTests.forEach((test) =>
                        config.set(`switches.ab${test.id}`, true)
                    );

                    return firstRunnableTest<ABTest>([
                        ...hardcodedEpicTests,
                        ...configuredEpicTests,
                    ]);
                }
...
export const getEngagementBannerTestToRun = memoize(
    (): Promise<Runnable<AcquisitionsABTest> | null | undefined> => {
