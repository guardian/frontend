...
import { getParticipationsFromLocalStorage, setParticipationsInLocalStorage } from "common/modules/experiments/ab-local-storage";
import { getForcedParticipationsFromUrl } from "common/modules/experiments/ab-url";
import { concurrentTests, engagementBannerTests, epicTests as hardCodedEpicTests, priorityEpicTest as hardCodedPriorityEpicTest } from "common/modules/experiments/ab-tests";
import { getEngagementBannerTestsFromGoogleDoc, getConfiguredEpicTests } from "common/modules/commercial/contributions-utilities";
import { getMvtValue } from "common/modules/analytics/mvt-cookie";
import { getSync as geolocationGetSync } from "lib/geolocation";
import { compareVariantDecision, getViewLog, getWeeklyArticleHistory } from "@guardian/automat-contributions";
import { getLastOneOffContributionDate, isRecurringContributor, shouldNotBeShownSupportMessaging } from "common/modules/commercial/user-features";
import { automatLog, logAutomatEvent } from "common/modules/experiments/automatLog";
...
export const getEpicTestToRun = memoize((): Promise<Runnable<ABTest> | null | undefined> => {
  const highPriorityHardCodedTests = hardCodedEpicTests.filter(test => test.highPriority);
  const lowPriorityHardCodedTests = hardCodedEpicTests.filter(test => !test.highPriority);
  ...
  if (config.get('switches.useConfiguredEpicTests')) {
    return getConfiguredEpicTests().then(configuredEpicTests => {
      ...
      configuredEpicTests.forEach(test => config.set(`switches.ab${test.id}`, true));

      const highPriorityConfiguredTests = configuredEpicTests.filter(test => test.highPriority);
      const lowPriorityConfiguredTests = configuredEpicTests.filter(test => !test.highPriority);

      const result = firstRunnableTest<AcquisitionsABTest>([hardCodedPriorityEpicTest, ...highPriorityConfiguredTests, ...highPriorityHardCodedTests, ...lowPriorityConfiguredTests, ...lowPriorityHardCodedTests]);

      const page = config.get('page');

      // No point in going forward with variant comparison in
      // these cases
      if (page.contentType !== 'Article' || configuredEpicTests.length === 0 || (result && result.id !== 'RemoteEpicVariants')) {
        return result;
      }
        ...
        if (Math.random() < 0.1) {
          ...
          compareVariantDecision({
            targeting: {
              ...
              isPaidContent: page.sponsorshipType === 'paid-content',
            ...
            expectedCampaignCode: result ? result.variantToRun.campaignCode : '',
  ...
  return Promise.resolve(firstRunnableTest<ABTest>([hardCodedPriorityEpicTest, ...highPriorityHardCodedTests, ...lowPriorityHardCodedTests]));
});

export const getEngagementBannerTestToRun = memoize((): Promise<Runnable<AcquisitionsABTest> | null | undefined> => {
  if (config.get('switches.engagementBannerTestsFromGoogleDocs')) {
    return getEngagementBannerTestsFromGoogleDoc().then(asyncEngagementBannerTests => {
      asyncEngagementBannerTests.forEach(test => config.set(`switches.ab${test.id}`, true));
      return firstRunnableTest<AcquisitionsABTest>([...engagementBannerTests, ...asyncEngagementBannerTests]);
    });
  }
...
export const getAsyncTestsToRun = (): Promise<ReadonlyArray<Runnable<ABTest>>> => Promise.all([getEpicTestToRun(), getEngagementBannerTestToRun()]).then(tests => tests.filter(Boolean));

// This excludes epic & banner tests
export const getSynchronousParticipations = (): Participations => runnableTestsToParticipations(getSynchronousTestsToRun());

// This excludes epic & banner tests
export const isInVariantSynchronous = (test: ABTest, variantId: string): boolean => getSynchronousTestsToRun().some(t => t.id === test.id && t.variantToRun.id === variantId);

// This excludes epic & banner tests
// checks if the user in in a given test with any variant
export const isInABTestSynchronous = (test: ABTest): boolean => getSynchronousTestsToRun().some(t => t.id === test.id);
