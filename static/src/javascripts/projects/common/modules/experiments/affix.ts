...
const getPixels = (top: string): number => top !== 'auto' ? parseInt(top, 10) : 0;
...
class Affix {
  ...
  constructor(options: {
    ...
    topMarker: HTMLElement | null | undefined;
    bottomMarker: HTMLElement | null | undefined;
  ...
  calculateContainerPositioning(): void {
    ...
    }).then(() => fastdom.measure(() => this.$markerTop.offset().top - this.$container.offset().top)).then(containerTop => {
      fastdom.mutate(() => {
  ...
  checkPosition(): void {
    ...
    if (this.affixed !== affix) {
      ...
      if (bottomCheck) {
        ...
        const oldContainerStyling = getPixels(this.$container.css('top'));
        const topStyle = markerBottomTop - markerTopTop - elHeight + oldContainerStyling;
        fastdom.mutate(() => {
