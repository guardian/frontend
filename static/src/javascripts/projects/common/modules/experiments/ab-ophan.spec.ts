...
import { registerCompleteEvents, registerImpressionEvents, buildOphanPayload } from "common/modules/experiments/ab-ophan";
...
describe('A/B Ophan analytics', () => {
  ...
  test('Ophan data structure contains the correct values', () => {
    expect(buildOphanPayload([genRunnableAbTestWhereControlIsRunnable('DummyTest'), genRunnableAbTestWhereControlIsRunnable('DummyTest2')])).toEqual({
      DummyTest: {
        variantName: 'control',
        complete: 'false'
      },
  ...
  test('defer firing the impression when the function is provided', () => {
    ...
    expect(controlSpy.mock.calls.length + variantSpy.mock.calls.length).toEqual(1);
  });
});