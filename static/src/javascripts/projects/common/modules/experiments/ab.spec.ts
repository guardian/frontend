...
import { getAsyncTestsToRun, getSynchronousTestsToRun, isInVariantSynchronous, runAndTrackAbTests } from "common/modules/experiments/ab";
import { concurrentTests, epicTests, priorityEpicTest, engagementBannerTests } from "common/modules/experiments/ab-tests";
...
describe('A/B', () => {
  ...
  describe('runAndTrackAbTests', () => {
    test('should run all concurrent tests whose canRun is true, but just the first epic test & first banner test', () => {
      ...
      const shouldRun = [jest.spyOn(concurrentTests[0].variants[0], 'test'), jest.spyOn(concurrentTests[1].variants[0], 'test'), jest.spyOn(epicTests[0].variants[0], 'test'), jest.spyOn(engagementBannerTests[0].variants[0], 'test')];

      const shouldNotRun = [jest.spyOn(concurrentTests[2].variants[0], 'test'), jest.spyOn(epicTests[1].variants[0], 'test'), jest.spyOn(engagementBannerTests[1].variants[0], 'test'), jest.spyOn(priorityEpicTest.variants[0], 'test')];
  ...
  describe('getTestsToRun', () => {
    ...
    test('should give the same result whether called before or after runAndTrackAbTests', () => {
      ...
      const checkTests = tests => expect(runnableTestsToParticipations(tests)).toEqual(expectedTestsToRun);

      return getAsyncTestsToRun().then(asyncTests => checkTests([...asyncTests, ...getSynchronousTestsToRun()])).then(runAndTrackAbTests).then(getAsyncTestsToRun).then(asyncTests => checkTests([...asyncTests, ...getSynchronousTestsToRun()]));
    });
  });
  ...
  describe('isInVariantSynchronous', () => {
    test('should respect the URL hash', () => {
      window.location.hash = '#ab-DummyTest=variant';
      expect(isInVariantSynchronous(concurrentTests[0], 'variant')).toEqual(true);
    });
    ...
    test('should respect localStorage and MVT cookie', () => {
      ...
      expect(isInVariantSynchronous(concurrentTests[1], 'control')).toEqual(true);

      expect(isInVariantSynchronous(concurrentTests[2], 'variant')).toEqual(false);

      expect(isInVariantSynchronous(concurrentTests[1], 'variant')).toEqual(false);
    });
  });
});