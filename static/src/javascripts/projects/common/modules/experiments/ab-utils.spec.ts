...
import { genAbTest, genRunnableAbTestWhereControlIsRunnable, genRunnableAbTestWhereVariantIsRunnable, genVariant } from "./__fixtures__/ab-test";
import { runnableTestsToParticipations, testAndParticipationsToVariant, testExclusionsWhoseSwitchExists } from "./ab-utils";
...
describe('A/B utils', () => {
  describe('runnableTestsToParticipations', () => {
    it('should set the runnable variant in the participations', () => {
      expect(runnableTestsToParticipations([genRunnableAbTestWhereControlIsRunnable('a'), genRunnableAbTestWhereControlIsRunnable('b')])).toEqual({
        a: { variant: 'control' },
        b: { variant: 'control' }
      });

      expect(runnableTestsToParticipations([genRunnableAbTestWhereControlIsRunnable('c'), genRunnableAbTestWhereVariantIsRunnable('d')])).toEqual({
        c: { variant: 'control' },
        d: { variant: 'variant' }
      });
    });
  ...
  describe('testExclusionsWhoseSwitchExists', () => {
    ...
    it('should filter out non-NOT_IN_TEST variants', () => {
      ...
      expect(testExclusionsWhoseSwitchExists({
  ...
  describe('testAndParticipationsToVariant', () => {
    it('should return the NOT_IN_TEST variant if present', () => {
      ...
      expect(testAndParticipationsToVariant(genAbTest('a', true, '9999-12-12', [genVariant('control'), notInTestVariant]), {
    ...
    it('should return a normal variant if no NOT_IN_TEST variant present', () => {
      ...
      expect(testAndParticipationsToVariant(genAbTest('b', true, '9999-12-12', [genVariant('control'), variant]), {
        a: { variant: 'hey' },
        b: { variant: 'Variant' }
      })).toEqual(variant);
    });
