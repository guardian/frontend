...
class Component {
  ...
  render(parent: HTMLElement | (Node | null | undefined) = document.body): Component {
    this.checkAttached();
  ...
  _ready(elem?: (HTMLElement | HTMLAnchorElement | HTMLInputElement) | null | undefined): void {
    if (!this.destroyed) {
      this.rendered = true;
      this._autoupdate();
      this.ready(elem);
  ...
  _autoupdate(): void {
    ...
    const update = (): void => {
      this._fetch().then(resp => {
        this.autoupdate(bonzo.create(resp[this.responseDataKey])[0]);
   ...
   */
  // eslint-disable-next-line class-methods-use-this, no-unused-vars
  ready(elem: (HTMLElement | HTMLAnchorElement | HTMLInputElement) | null | undefined): void {}
  ...
  on(eventType: string, elem: string | ((arg: any) => void), handler?: (arg: any) => void): Component {
  ...
  getElem(elemName: string): (HTMLElement | HTMLInputElement) | null | undefined {
  ...
  setState(state: string, elemName: string | null | undefined): void {
    const elem = elemName ? this.getElem(elemName) : this.elem;
    const $elem = bonzo(elem);

    if (this.componentClass) {
      $elem.addClass(`${this.componentClass + (elemName ? `__${elemName}` : '')}--${state}`);
    }
  }

  removeState(state: string, elemName: string | null | undefined): void {
    const elem = elemName ? this.getElem(elemName) : this.elem;
    const $elem = bonzo(elem);

    if (this.componentClass) {
      $elem.removeClass(`${this.componentClass + (elemName ? `__${elemName}` : '')}--${state}`);
    }
  }

  toggleState(state: string, elemName: string | null | undefined): void {
    const elem = elemName ? this.getElem(elemName) : this.elem;
    const $elem = bonzo(elem);

    if (this.componentClass) {
      $elem.toggleClass(`${this.componentClass + (elemName ? `__${elemName}` : '')}--${state}`);
    }
  }

  hasState(state: string, elemName: string | null | undefined): boolean {
    const elem = elemName ? this.getElem(elemName) : this.elem;
    const $elem = bonzo(elem);

    if (this.componentClass) {
      return $elem.hasClass(`${this.componentClass + (elemName ? `__${elemName}` : '')}--${state}`);
    }
