...
const fetch = (): void => {
  const url = `${config.get('page.ajaxUrl')}/sharecount/${config.get('page.pageId')}.json`;
...
const loadShareCounts = (): void => {
  ...
  if (config.get('switches.serverShareCounts') && $shareCountEls.length && !config.get('page.isPreview')) {
    try {
      fetch();
    } catch (e) {
