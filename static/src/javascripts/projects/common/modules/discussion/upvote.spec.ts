...
describe('Recommendations of comments', () => {
  ...
  it('should send a request to discussion API if the user is logged in', () => {
    ...
    return handle(target, document.querySelector('.recommendation-test'), fakeUser).then(() => {
      expect(recommendComment).toHaveBeenCalled();
      expect(target.classList.contains('d-comment__recommend--recommended')).toBe(true);
      expect(target.classList.contains('js-recommend-comment')).toBe(false);
    });
  });
  ...
  it('should send an anonymous request to the discussion API when permitted', () => {
    ...
    return handle(target, document.querySelector('.recommendation-test'), null).then(() => {
      expect(recommendComment).toHaveBeenCalled();
      expect(target.classList.contains('d-comment__recommend--recommended')).toBe(true);
      expect(target.classList.contains('js-recommend-comment')).toBe(false);
    });
  ...
  it('should allow retry if the discussion api returns an error', () => {
    ...
    recommendComment.mockImplementationOnce(() => Promise.reject(new Error('discussion api error')));

    config.set('switches.discussionAllowAnonymousRecommendsSwitch', false);

    return handle(target, document.querySelector('.recommendation-test'), fakeUser).catch(() => {
      expect(recommendComment).toHaveBeenCalled();
      expect(target.classList.contains('d-comment__recommend--recommended')).toBe(false);
      expect(target.classList.contains('js-recommend-comment')).toBe(true);
    });
  });
  ...
  it('should show a tooltip with a return link to the upvoted comment', () => {
    ...
    return handle(target, document.querySelector('.recommendation-test'), null).then(() => {
      ...
      expect(link.getAttribute('href')).toBe('http://theguardian.com/test/signin?keep=this&returnUrl=http%3A%2F%2Ftheguardian.com%2Fcomment-1');
