...
const loadDiscussionFrontend = (loader: Loader, opts: Object): Promise<void> => {
  ...
  const onDiscussionFrontendLoad = (emitter: mediator): void => {
    ...
    emitter.once('comment-count', (value: number): void => {
      if (value === 0) {
        ...
        const otherValues = Array.from(document.getElementsByClassName('js_commentcount_actualvalue'));
  ...
  return loadScript(config.get('page.discussionFrontendUrl')).then(() => {
