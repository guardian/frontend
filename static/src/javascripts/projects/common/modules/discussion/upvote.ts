...
const updateReturnUrl = (links: NodeList<HTMLElement>, returnLink: string | null | undefined): void => {
...
const isOpenForRecommendations = (element: Element | null | undefined): boolean => !!element && !!element.querySelector('.d-discussion--recommendations-open');

const setClicked = (target: Element): Promise<void> => fastdom.mutate(() => {
  target.classList.remove(RECOMMENDATION_CLASS);
  target.classList.add('d-comment__recommend--clicked');
});

const unsetClicked = (target: Element): Promise<void> => fastdom.mutate(() => {
  target.classList.add(RECOMMENDATION_CLASS);
  target.classList.remove('d-comment__recommend--clicked');
});

const setRecommended = (target: Element): Promise<void> => fastdom.mutate(() => {
  target.classList.add('d-comment__recommend--recommended');
});

const handle = (target: Element, container: Element | null | undefined, user: DiscussionProfile | null | undefined): Promise<void> => {
  const discussionAllowAnonymousRecommendsSwitch = config.get('switches.discussionAllowAnonymousRecommendsSwitch');
  if (!discussionAllowAnonymousRecommendsSwitch && !user) {
    ...
    return Promise.all([setClicked(target), recommendComment(id)]).then(() => setRecommended(target)).catch(ex => unsetClicked(target).then(() => {
      reportError(ex, {
...
const closeTooltip = (): Promise<void> => fastdom.mutate(() => {
  const tooltip = document.querySelector(`.${TOOLTIP_CLASS}`);
