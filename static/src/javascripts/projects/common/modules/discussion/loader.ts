...
import { handle as upvoteHandle, closeTooltip as upvoteCloseTooltip } from "common/modules/discussion/upvote";
...
class Loader extends Component {
  ...
  getDiscussionId(): string | null | undefined {
  ...
  getDiscussionClosed(): boolean {
    return !!(this.elem && this.elem instanceof HTMLElement && this.elem.getAttribute('data-discussion-closed') === 'true');
  }

  // eslint-disable-next-line class-methods-use-this
  getCommentIdFromHash(): number | null | undefined {
    const reg = /#comment-(\d+)/;
  ...
  isCommentable(): boolean {
    ...
    return !!(userCanPost && commentsReadOnly && !this.getDiscussionClosed());
  }
  ...
  ready(): void {
    ...
    mediator.on('discussion:commentbox:post:success', () => this.removeState('empty'));

    mediator.on('module:clickstream:click', clickspec => {
      const shouldRemoveTruncation = clickspec && 'hash' in clickspec.target && clickspec.target.hash === '#comments';
  ...
  initToolbar(): void {
    ...
    this.on('click', '.js-comment-order-dropdown .popup__action', (e: Event) => {
      ...
      userPrefs.set('discussion.order', this.comments && this.comments.options.order);

      this.loadComments({ page: 1 });
    });

    this.on('click', '.js-comment-threading-dropdown .popup__action', (e: Event) => {
      ...
      userPrefs.set('discussion.threading', this.comments && this.comments.options.threading);
    ...
    if (config.get('page.section') === 'crosswords') {
      ...
      this.on('click', '.js-timestamps-dropdown .popup__action', (e: Event) => {
  ...
  initPageSizeDropdown(pageSize: number | string): void {
    ...
    this.on('click', '.js-comment-pagesize-dropdown .popup__action', (e: Event) => {
  ...
  initMainComments(): void {
    ...
    this.on('click,', '.js-discussion-author-link', () => this.removeTruncation());
    ...
    this.on('user:loaded', () => {
      ...
      if (this.user) {
        ...
        if (typeof userPageSize === 'number') {
          ...
          pageSize = config.get('switches.discussionAllPageSize') ? 'All' : 100;
        }

        this.initPageSizeDropdown(pageSize);

        if (this.comments && config.get('switches.discussionPageSize') && isBreakpoint({ min: 'tablet' })) {
  ...
  initTopComments(): Promise<void> {
    ...
    return fetchJson(`/discussion/top-comments/${discussionId}.json?commentsClosed=${this.getDiscussionClosed().toString()}`, {
      mode: 'cors'
    }).then(resp => {
  ...
  loadComments(options?: {
    ...
    if (opts.shouldTruncate && (this.comments && this.comments.isAllPageSizeActive())) {
      opts.pageSize = 10;
    }
  ...
  renderCommentBox(elem: HTMLElement): void {
    new CommentBox({
      ...
      newCommenter: this.user && this.user.privateFields && !this.user.privateFields.hasCommented,
      hasUsername: this.username !== null,
      shouldRenderMainAvatar: false
    }).render(elem).on('post:success', comment => this.commentPosted(comment));
  }
  ...
  renderCommentCount(): void {
    loadDiscussionFrontend(this, {
      ...
      profileClientId: config.get('switches.registerWithPhone') ? 'comments' : '',
      Promise
    });
  }
}
