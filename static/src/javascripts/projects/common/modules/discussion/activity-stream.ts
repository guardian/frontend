...
import $ from "lib/$";
import { getUrlVars, constructQuery, replaceQueryString, pushQueryString } from "lib/url";
...
class ActivityStream extends Component {
  ...
  constructor(opts: Object): void {
    ...
    this.endpoint = '/discussion/profile/:userId/:streamType.json?page=:page';
  ...
  applyState(html: string, streamType: string): void {
    const selectTab = (type: string): void => {
      ...
      bonzo($(`a[data-stream-type=${type}]`)).parent().addClass('tabs__tab--selected');
    };
  ...
  ready(): void {
    const pagination = (activityStream: ActivityStream): void => {
      bean.on(activityStream.elem, 'click', '.js-activity-stream-page-change', (e: Event) => {
        const target: HTMLElement = (e.currentTarget as any);
        const page: string | null | undefined = target.getAttribute('data-page');
        e.preventDefault();
