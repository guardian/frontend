...
import { getUser, postComment, previewComment } from "common/modules/discussion/api";
import { getUserFromCookie, reset, updateUsername, getUserFromApiWithRefreshedCookie } from "common/modules/identity/api";
...
class CommentBox extends Component {
  ...
  static async refreshUsernameHtml(): Promise<void> {
    ...
    const discussionUser: DiscussionProfile = discussionUserResponse.userProfile;

    const displayName = discussionUser.displayName;
    const menuHeaderUsername = document.querySelector('.js-profile-info');
    const discussionHeaderUsername = document.querySelector('._author_tywwu_16');
  ...
  constructor(options: Object): void {
    ...
    mediator.on('module:identity:validation-email:success', () => this.verificationEmailSuccess());

    mediator.on('module:identity:validation-email:fail', () => this.verificationEmailFail());
  }
  ...
  getDiscussionId(): string {
    ...
    let discussionKey = this.elem && this.elem instanceof HTMLElement && this.elem.getAttribute('data-discussion-key');
  ...
  postCommentSuccess(comment: commentType, resp: Object): commentType {
    ...
    const body = ((this.getElem('body') as any) as HTMLInputElement);
  ...
  postComment(): Promise<any> {
    ...
    const {
      value
    } = (commentBody && commentBody instanceof HTMLTextAreaElement && commentBody) || {};
    ...
    const postCommentToDAPI = (): Promise<any> => {
      ...
      return postComment(this.getDiscussionId(), comment).then((resp: Object) => this.postCommentSuccess(comment, resp)).catch((err: Object) => this.fail(err));
    };
    ...
    const updateUsernameFailure = (errorResponse: Object): void => {
      ...
      const errorMessage = this.getElem('onboarding-username-error-message');
    ...
    const validEmailCommentSubmission = (): Promise<any> => {
      ...
      if (comment.body.length > this.options.maxLength) {
        this.error('COMMENT_TOO_LONG', `<b>Comments must be shorter than ${this.options.maxLength} characters.</b>` + `Yours is currently ${comment.body.length - this.options.maxLength} character(s) too long.`);
      }
      ...
      if (this.errors.length === 0) {
        if (this.options.newCommenter && !this.options.hasUsername) {
          const userNameInput = ((this.getElem('onboarding-username-input') as any) as HTMLInputElement);
          return updateUsername(userNameInput.value).then(updateUsernameSuccess, updateUsernameFailure);
        }
  ...
  ready(): void {
    ...
    if (this.options.newCommenter) {
      bean.on(document.body, 'submit', [this.elem], (event: Event) => this.showOnboarding(event));
      bean.on(document.body, 'click', this.getClass('onboarding-cancel'), (event: Event) => this.hideOnboarding(event));
    } else {
      bean.on(document.body, 'submit', [this.elem], (event: Event) => this.submitPostComment(event));
    }

    bean.on(document.body, 'change keyup', [commentBody], () => this.setFormState());

    bean.on(commentBody, 'focus', () => this.setExpanded());

    this.on('change', '.d-comment-box__body', () => this.resetPreviewComment());
    this.on('click', this.getClass('preview'), () => this.previewComment('previewCommentSuccess'));
    this.on('click', this.getClass('hide-preview'), () => this.resetPreviewComment());

    this.on('click', this.getClass('cancel'), () => this.cancelComment());
    this.on('click', this.getClass('show-parent'), () => this.setState('parent-visible'));
    this.on('click', this.getClass('hide-parent'), () => this.removeState('parent-visible'));
  ...
  prerender(): void {
    ...
    if (this.options.replyTo) {
      ...
      const setSpoutMargin = () => {
        ...
        const spoutOffset = replyToAuthor ? replyToAuthor.offsetLeft + replyToAuthor.getBoundingClientRect().width / 2 : false;
  ...
  previewComment(methodName: string): void {
    ...
    if (comment && comment.body && comment.body.length > this.options.maxLength) {
    ...
    if (this.errors.length === 0) {
      previewComment(comment).then((resp: Object) => callback(comment, resp)).catch((err: Object) => this.fail(err));
    }
  }
  ...
  formatComment(formatStyle: string): void {
    ...
    const formatSelection = (startTag: string, endTag: string): void => {
      ...
      commentBody.value = commentBody.value.substring(0, commentBody.selectionStart) + newText + commentBody.value.substring(commentBody.selectionEnd);
