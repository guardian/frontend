...
const runConcurrently = (workFunction: (num: number) => Promise<any>, items: Array<number>): Promise<void> => new Promise(resolve => {
  const queue = items;
  let workers = 0;
...
class WholeDiscussion {
  ...
  loadPage(pageNumber: number): Promise<Object> {
    ...
    const url = `/discussion/${this.discussionId}.json?${constructQuery(queryParams)}`;
  ...
  loadAllComments(): Promise<DiscussionResponse> {
    return this.loadPage(1).then(resp => this.firstPageLoaded(resp)).then(pages => this.loadRemainingPages(pages)).then(() => this.makeDiscussionResponseObject());
  }
}
