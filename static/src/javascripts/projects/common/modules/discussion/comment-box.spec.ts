...
jest.mock('common/modules/identity/api', () => ({
  ...
  getUserFromApiWithRefreshedCookie: jest.fn().mockReturnValue(Promise.resolve({
    user: {
      statusFields: {
...
const getUserFromApiWithRefreshedCookie: JestMockFn<any, any> = (getUserFromApiWithRefreshedCookie_ as any);
const postComment: JestMockFn<any, any> = (postComment_ as any);
...
describe('Comment box', () => {
  ...
  const apiPostValidCommentButIdentityUsernameMissing = '{"status":"error","statusCode":400,"message":"Username is missing","errorCode":"USERNAME_MISSING"}';
  let commentBox;
  let commentBoxEl;
  ...
  describe('Post comment', () => {
    it('should only disable button when there is no comment body', () => {
      ...
      if (commentBody && commentBody instanceof HTMLTextAreaElement && button) {
        commentBody.value = '';
    ...
    it('should error on invalid email address', () => {
      ...
      if (commentBody && commentBody instanceof HTMLTextAreaElement) {
        ...
        getUserFromApiWithRefreshedCookie.mockReturnValueOnce(Promise.resolve({
          user: {
            statusFields: {
    ...
    it('should error on discussion closed', () => {
      ...
      if (commentBody && commentBody instanceof HTMLTextAreaElement) {
        ...
        postComment.mockReturnValueOnce( // eslint-disable-next-line prefer-promise-reject-errors
        Promise.reject({
    ...
    it('should send a success message to the user when comment is valid', () => {
      ...
      if (commentBody && commentBody instanceof HTMLTextAreaElement) {
        ...
        postComment.mockReturnValueOnce(Promise.resolve(JSON.parse(apiPostValidCommentResp)));
