...
import { reportComment, pickComment, unPickComment } from "common/modules/discussion/api";
...
class Comments extends Component {
  ...
  getMoreReplies(event: Event): void {
    ...
    if (commentId) {
      ...
      fetchJson(endpoint, {
        ...
        const replies = qwery(this.getClass('reply'), comment).slice(this.options && this.options.showRepliesCount);
  ...
  addMoreRepliesButtons(comms: Array<HTMLElement>): void {
    ...
    comments.forEach(elem => {
      ...
      if (renderedReplies.length < replies) {
        ...
        const $btn = $.create(btnMarkup).attr({
          'data-link-name': 'Show more replies',
          'data-is-ajax': '',
          'data-comment-id': elem.getAttribute('data-comment-id')
        }).data('source-comment', elem);

        $.create(`<li class="${this.getClass('showReplies', true)}"></li>`).append($btn).appendTo($('.d-thread--responses', elem));
      }
    });
  }

  fetchComments(options: Object = {}): Promise<unknown> {
    const {
      discussionId
    } = this.options || {};
    const url = `/discussion/${options.comment ? `comment-context/${options.comment}` : discussionId}.json`;
    ...
    const queryParams: Object = {
      orderBy,
      pageSize: options.pagesize || (this.options && this.options.pagesize),
      displayThreaded: !!(this.options && this.options.threading !== 'unthreaded'),
    ...
    if (!options.comment && this.options && this.options.threading === 'collapsed') {
  ...
  handlePickClick(e: Event): void {
    ...
    const highlighted = $thisButton[0].getAttribute('data-comment-highlighted');
    const action = highlighted === 'true' ? this.unPickComment : this.pickComment;
  ...
  ready(): void {
    ...
    this.on('click', this.getClass('showRepliesButton'), (event: Event) => this.getMoreReplies(event));

    this.on('click', this.getClass('commentReport'), (event: Event) => this.reportComment(event));
    ...
    this.on('click', '.js-report-comment-close', () => {
      const commentForm = document.querySelector('.js-report-comment-form');
  ...
  isReadOnly(): boolean {
    return !!(this.elem && this.elem instanceof HTMLElement && this.elem.getAttribute('data-read-only') === 'true');
  }
  ...
  addComment(comment: CommentType, parent?: HTMLElement): void {
    ...
    const replyButton = commentElem && commentElem.getElementsByClassName(this.getClass('commentReply', true))[0];
    const map: Object = {
  ...
  replyToComment(e: Event): void {
    ...
    const replyLink: HTMLElement = (e.currentTarget as any);
    ...
    const parentCommentEl = $replyToComment.hasClass(this.getClass('topLevelComment', true)) ? $replyToComment[0] : $replyToComment.parent().parent()[0];

    const showRepliesElem = qwery(this.getClass('showReplies'), parentCommentEl);
  ...
  reportComment(e: Event): void {
    ...
    const currentTarget: HTMLElement = (e.currentTarget as any);
    const commentId = currentTarget.getAttribute('data-comment-id');
    const submitHandler = (form: HTMLFormElement) => {
      ...
      bean.one(form, 'submit', (submitEvent: Event) => {
        ...
        const reportCommentFailure = (): void => {
          ...
          const commentError = document.querySelector('.js-discussion__report-comment-error');
        ...
        if (commentId && category.value !== '0') {
          ...
          }).then(() => reportCommentSuccess(form)).catch(() => reportCommentFailure());
        }
      });
    };

    if (commentId) {
      const reportContainer = document.querySelector(`#comment-${commentId} .js-report-comment-container`);

      $('.js-report-comment-form').first().each(submitHandler).appendTo(reportContainer);
    }
  }
  ...
  addUser(user: DiscussionProfile): void {
    ...
    if (!this.isReadOnly()) {
      if (this.user && this.user.privateFields.canPostComment) {
        ...
        this.on('click', this.getClass('commentReply'), event => this.replyToComment(event));

        this.on('click', this.getClass('commentPick'), event => this.handlePickClick(event));
      }
    }
  ...
  isAllPageSizeActive(): boolean {
    return !!(config.get('switches.discussionAllPageSize') && (this.options && this.options.pagesize === 'All') && !this.wholeDiscussionErrors);
  }
