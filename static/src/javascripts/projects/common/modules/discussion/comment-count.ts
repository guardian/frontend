...
const getElementsIndexedById = (context: HTMLElement): Promise<any> => fastdom.measure(() => Array.from(context.querySelectorAll(`[${ATTRIBUTE_NAME}]`))).then(elements => {
  if (elements.length === 0) {
    return;
  }

  return elements.reduce((groupedVals: Object, el: HTMLElement): Object => {
...
const getContentIds = (indexedElements: IndexedElements): string => Object.keys(indexedElements).sort().join(',');
...
const renderCounts = (counts: Array<{id: string;count: number;}>, indexedElements: IndexedElements): Promise<any> => {
  const elementUpdates = counts.map(c => indexedElements[c.id].map(el => updateElement(el, c.count)));

  return Promise.all(elementUpdates);
};

const getCommentCounts = (context?: HTMLElement): Promise<void> => {
  const queryContext: HTMLElement | null | undefined = context || document.body;
