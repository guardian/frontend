...
const replaceContent = (isSuccess: boolean, $form: bonzo, $formHeader: bonzo | null | undefined): void => {
  const formData = $form.data('formData');
  const statusClass = isSuccess ? 'email-sub__message--success' : 'email-sub__message--failure';
  const submissionHeadline = isSuccess ? formData.customSuccessHeadline || messages.defaultSuccessHeadline : 'Something went wrong';
  const submissionMessage = isSuccess ? formData.customSuccessDesc || messages.defaultSuccessDesc : 'Please try again.';
...
const removeAndRemember = (e: Event, {
  iframe,
  analytics
}: {iframe: HTMLElement;analytics: Analytics;}): void => {
  ...
  trackNonClickInteraction(`rtrt | email form inline | ${analytics.formType} | ${analytics.listName} | ${analytics.signedIn} | form hidden`);
};

const updateFormForLoggedIn = (userFromId: IdentityUser, el: HTMLElement): void => {
...
const updateForm = (thisRootEl: HTMLElement, $el: bonzo, analytics: Analytics): void => {
  const formData = $(thisRootEl).data();
  const formDisplayNameNormalText = formData.formDisplayNameNormalText || false;
  const formDisplayNameAccentedText = formData.formDisplayNameAccentedText || false;
...
const setIframeHeight = (iFrameEl: HTMLIFrameElement, callback: () => void): () => void => () => {
  fastdom.read(() => iFrameEl.contentWindow.document.body.clientHeight).then(height => fastdom.write(() => {
    iFrameEl.height = `${height}px`;
  })).then(callback());
};

const handleSubmit = (isSuccess: boolean, $form: bonzo, $formHeader: bonzo | null | undefined): () => void => () => {
  replaceContent(isSuccess, $form, $formHeader);
  state.submitting = false;
};

const submitForm = ($form: bonzo, $formHeader: bonzo | null | undefined, url: string, analytics: Analytics): (arg0: Event) => Promise<any> | null | undefined => {
  // simplistic email address validation to prevent misfired omniture events
  const validate = (emailAddress: string): boolean => emailAddress.includes('@');
  ...
  return event => {
    ...
    if (!state.submitting && validate(emailAddress)) {
      const formData = $form.data('formData');
      const data = `email=${encodeURIComponent(emailAddress)}&name=${encodeURIComponent(dummy)}&campaignCode=${formData.campaignCode}&referrer=${formData.referrer}&csrfToken=${encodeURIComponent(csrfToken)}&listName=${listName.val()}`;

      analyticsInfo = `rtrt | email form inline | ${analytics.formType} | ${analytics.listName} | ${analytics.signedIn} | %action%`;
      ...
      return new Promise(() => {
        ...
        trackNonClickInteraction(analyticsInfo.replace('%action%', 'subscribe clicked'));
        return fetch(config.get('page.ajaxUrl') + url, {
          ...
          trackNonClickInteraction(analyticsInfo.replace('%action%', 'subscribe successful'));
        }).then(handleSubmit(true, $form, $formHeader)).catch(error => {
          logError('c-email', error);
          trackNonClickInteraction(analyticsInfo.replace('%action%', 'error'));
...
const bindSubmit = ($form: bonzo, $formHeader: bonzo | null | undefined, analytics: Analytics): void => {
  const url = '/email';

  bean.on($form[0], 'submit', submitForm($form, $formHeader, url, analytics));
};

const setup = (rootEl: (HTMLElement | null | undefined) | (Document | null | undefined), iframeEl: HTMLIFrameElement | null | undefined): void => {
  ...
  $(`.${classes.wrapper}`, rootEl).each(el => {
    ...
    const analytics = {
      ...
      signedIn: isUserLoggedIn() ? 'user signed-in' : 'user not signed-in'
    };
    let onResize;
...
const initEmail = (iframeEl?: HTMLIFrameElement): void => {
  ...
  if (typeof getUserAgent === 'object' && getUserAgent.browser === 'MSIE' && ['7', '8', '9'].includes(getUserAgent.version)) {
