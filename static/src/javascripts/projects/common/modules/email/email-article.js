...
const insertBottomOfArticle = $iframeEl => {
      ...
      addListToPage = (listConfig, successEventName) => {
          ...
          if (listConfig) {
              ...
              if (listConfig.insertMethod) {
                  ...
                  spaceFiller.spaceFiller.fillSpace(getSpacefinderRules(), paras => {
...
export default {
    init() {
        checkMediator.waitForCheck('emailCanRun').then(emailCanRun => {
            if (emailCanRun) {
                checkMediator.waitForCheck('emailCanRunPostCheck').then(emailCanRunPostCheck => {
