...
const classes = {
      ...
      removeAndRemember = (e, data) => {
          const iframe = data[0], analytics = data[1], currentListPrefs = userPrefs.get('email-sign-up-' + analytics.formType) || [];
      ...
      ui = {
          updateForm(thisRootEl, el, analytics, opts) {
              const formData = $(thisRootEl).data(), formDisplayNameNormalText = (opts && opts.displayName && opts.displayName.normalText) || formData.formDisplayNameNormalText || false, formDisplayNameAccentedText = (opts && opts.displayName && opts.displayName.accentedText) || formData.formDisplayNameAccentedText || false, formTitle = (opts && opts.formTitle) || formData.formTitle || false, formDescription = (opts && opts.formDescription) || formData.formDescription || false, formCampaignCode = (opts && opts.formCampaignCode) || formData.formCampaignCode || '', formSuccessHeadline = (opts && opts.formSuccessHeadline) || formData.formSuccessHeadline, formSuccessDesc = (opts && opts.formSuccessDesc) || formData.formSuccessDesc, removeComforter = (opts && opts.removeComforter) || formData.removeComforter || false, formCloseButton = (opts && opts.formCloseButton) || formData.formCloseButton || false, formSuccessEventName = (opts && opts.formSuccessEventName) || formData.formSuccessEventName || false;
      ...
      setup = (rootEl, thisRootEl, isIframed) => {
