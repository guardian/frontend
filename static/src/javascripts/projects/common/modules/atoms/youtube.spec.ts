...
describe('youtube', () => {
  ...
  describe('muting iframes to trigger autoplay', () => {
    ...
    it('adds a new parameter to mute iframe if parameters already exist', () => {
      ...
      muteIFrame(iframe);
      if (document.body) {
        expect(iframe.src).toBe('http://www.example.com/q?randomParam=abc&mute=1');
      }
    });
  ...
  describe(`getting correct iframe behaviour config`, () => {
    ...
    beforeEach(() => {
      ...
      iframe = ((document.getElementById('iframeId') as any) as HTMLIFrameElement);
    });
    ...
    it('correctly identifies Internal Referrer', () => {
      ...
      jest.spyOn(global.document, 'referrer', 'get').mockReturnValueOnce('https://www.theguardian.com');
      expect(getIFrameBehaviourConfig(iframe).isInternalReferrer).toBe(true);
      // $FlowFixMe jest.spyon can take a third accesstype argument since jest 22.1.0
      jest.spyOn(global.document, 'referrer', 'get').mockReturnValueOnce('https://www.garbage-site.com');
      expect(getIFrameBehaviourConfig(iframe).isInternalReferrer).toBe(false);
    });
    ...
    it('correctly configures for flashing element preferences', () => {
      ...
      expect(getIFrameBehaviourConfig(iframe).flashingElementsAllowed).toBe(true);
      accessibilityIsOn.mockReturnValueOnce(false);
      expect(getIFrameBehaviourConfig(iframe).flashingElementsAllowed).toBe(false);
    });
