...
import { isAndroid, isBreakpoint, isIOS } from 'lib/detect';
...
const players: Record<string, AtomPlayer> = {};
...
const getIFrameBehaviourConfig = (
    ...
    const isInternalReferrer = ((): boolean => {
        ...
        return document.referrer.startsWith(config.get('page.host'));
    })();
...
const getUniqueAtomId = (atomId: string): string =>
    `${atomId}/${Math.random().toString(36).substr(2, 9)}`;
...
export const checkElemsForVideos = (
    elems: HTMLElement[] | null | undefined
): void => {
