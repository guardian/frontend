...
import { trackYoutubeEvent, initYoutubeEvents } from "common/modules/atoms/youtube-tracking";
...
interface AtomPlayer {
  ...
  endSlate?: Component;
  duration?: number;
  progressTracker?: IntervalID | null | undefined;
}
...
document.addEventListener('focusin', () => {
  fastdom.read(() => $('.vjs-big-play-button')).then(($playButton: bonzo | null | undefined) => {
    fastdom.write(() => {
      if ($playButton) {
...
const recordPlayerProgress = (uniqueAtomId: string): void => {
  ...
  if (duration) {
    ...
    if (pendingTrackingCalls.length && percentPlayed >= pendingTrackingCalls[0]) {
...
const onPlayerPlaying = (uniqueAtomId: string): void => {
  ...
  if (latestYoutubeId !== youtubeId) {
    fetchJson(`/atom/youtube/${latestYoutubeId}.json`).then(resp => {
      ...
      reportError(Error(`Failed to get atom ID for youtube ID ${latestYoutubeId}. ${err}`), { feature: 'youtube' }, false);
    });
  } else {
    handlePlay(uniqueAtomId, player);
  }
...
const onPlayerEnded = (atomId: string): void => {
  ...
  const mainMedia = (player.iframe && player.iframe.closest('.immersive-main-media')) || null;
  if (mainMedia) {
...
const getIFrameBehaviourConfig = (iframe: HTMLIFrameElement): IFrameBehaviourConfig => {
  ...
  const isMainVideo = (iframe && !!iframe.closest('figure[data-component="main video"]')) || false;

  const flashingElementsAllowed = accessibilityIsOn('flashing-elements');

  const isVideoArticle = config.get('page.contentType', '').toLowerCase() === 'video';
...
const getIFrameBehaviour = (iframeConfig: IFrameBehaviourConfig): IFrameBehaviour => {
  const {
    isAutoplayBlockingPlatform,
    isInternalReferrer,
    isMainVideo,
  ...
  const isUsPaidContentVideo = isUSContent && isPaidContent && ((isVideoArticle && isMainVideo) || isFront) && flashingElementsAllowed;
  ...
  return {
    autoplay: ((isVideoArticle && isInternalReferrer && isMainVideo) || isFront) && !isAutoplayBlockingPlatform && flashingElementsAllowed,
    mutedOnStart: false
  };
};

const getEndSlate = (overlay: HTMLElement): Component | null | undefined => {
...
const updateImmersiveButtonPos = (): void => {
  const player = document.querySelector('.immersive-main-media__media .youtube-media-atom');
  const playerHeight = player ? player.offsetHeight : 0;
  const headline = document.querySelector('.immersive-main-media__headline-container');
  const headlineHeight = headline ? headline.offsetHeight : 0;
  const buttonOffset = playerHeight - headlineHeight;
  const immersiveInterface = document.querySelector('.youtube-media-atom__immersive-interface');
...
const onPlayerReady = (atomId: string, uniqueAtomId: string, iframeId: string, overlay: HTMLElement | null | undefined, event: YoutubePlayerEvent): void => {
  const iframe = ((document.getElementById(iframeId) as any) as HTMLIFrameElement);
  ...
  if (overlay) {
    ...
    if (!!config.get('page.section') && isBreakpoint({
      min: 'desktop'
    })) {
...
const isAnyPlayerPlaying = (): boolean => Object.keys(players).map(key => players[key]).filter((p: AtomPlayer): boolean => p.playing).length > 0;
...
const onPlayerStateChange = (atomId: string, event: YoutubePlayerEvent): void => {
  ...
  const stateKey = Object.keys(STATES).find(key => stateId === window.YT.PlayerState[key]);
...
const getUniqueAtomId = (atomId: string): string => `${atomId}/${Math.random().toString(36).substr(2, 9)}`;

const initYoutubePlayerForElem = (el: HTMLElement | null | undefined): void => {
  fastdom.read(() => {
    if (!el) return;
    ...
    initYoutubePlayer(iframe, {
      onPlayerReady: (event: YoutubePlayerEvent) => {
        onPlayerReady(atomId, uniqueAtomId, iframeId, overlay, event);
      },
      onPlayerStateChange: (event: YoutubePlayerEvent) => {
        onPlayerStateChange(uniqueAtomId, event);
      }
    }, iframe.dataset.assetId, channelId);
  });
};

const checkElemForVideo = (elem: HTMLElement | null | undefined): void => {
  if (!elem) return;
...
export const checkElemsForVideos = (elems: Array<HTMLElement> | null | undefined): void => {
