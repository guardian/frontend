...
jest.mock('lib/config', () => ({
  get: jest.fn(key => {
    ...
    throw new Error(`Unexpected config lookup '${key}', check the mock is still correct`);
  })
}));

jest.mock('@guardian/consent-management-platform', () => ({
  onConsentChange: jest.fn(callback => callback({ tcfv2: { consents: { '1': true } } }))
}));
...
describe('create ads config', () => {
  it('disables ads in ad-free', () => {
    const result = youtubePlayer.createAdsConfig(true, // ad-free
    null, null);
  ...
  it('in non ad-free includes url-escaped targeting params', () => {
    ...
    expect(result.adTagParameters).toBeDefined();
    if (result.adTagParameters) {
      expect(result.adTagParameters.cust_params).toEqual('key%3Dvalue%26permutive%3D42');
    }
  });
});