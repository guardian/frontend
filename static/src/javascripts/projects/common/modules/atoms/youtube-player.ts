...
const addVideoStartedClass = (el: HTMLElement | null | undefined) => {
  if (el) {
...
onConsentChange(state => {
  if (state.ccpa) {
    ...
    tcfState = state.tcfv2 ? Object.values(state.tcfv2.consents).every(Boolean) : state[1] && state[2] && state[3] && state[4] && state[5];
  }
});

const onPlayerStateChangeEvent = (event, handlers: Handlers, el: HTMLElement | null | undefined) => {
  if (el && config.get('page.isDev')) {
    const states = window.YT.PlayerState;
    const state: string | null | undefined = Object.keys(states).find(key => states[key] === event.data);
    if (state) {
  ...
  fastdom.write(() => {
    ['ENDED', 'PLAYING', 'PAUSED', 'BUFFERING', 'CUED'].forEach(status => {
      if (el) {
        ...
        if (fcItem) {
          $(fcItem)[0].classList.toggle(`fc-item--has-video-main-media__${status.toLocaleLowerCase()}`, event.data === window.YT.PlayerState[status]);
        }
...
const onPlayerReadyEvent = (event, handlers: Handlers, el: HTMLElement | null | undefined) => {
  fastdom.write(() => {
    if (el) {
      el.classList.add('youtube__video-ready');
      const fcItem = $.ancestor(el, 'fc-item');
      if (fcItem) {
        $(fcItem)[0].classList.add('fc-item--has-video-main-media__ready');
      }
    }
  });
...
const createAdsConfig = (adFree: boolean, tcfStateFlag: boolean | null, ccpaStateFlag: boolean | null): AdsConfig => {
  if (adFree) {
...
const setupPlayer = (elt: HTMLElement, videoId: string, channelId?: string, onReady, onStateChange, onError, onAdStart, onAdEnd) => {
   ...
   * empty array.
   */
  const adsConfig = createAdsConfig(commercialFeatures.adFree, tcfState, ccpaState);

  return new window.YT.Player(elt.id, {
    host: commercialFeatures.adFree || !elt.classList.contains('youtube-media-atom__iframe') ? 'https://www.youtube-nocookie.com' : 'https://www.youtube.com',
    videoId,
    width: '100%',
    height: '100%',
    events: {
...
const getPlayerIframe = videoId => document.getElementById(`youtube-${videoId}`);

export const initYoutubePlayer = (el: HTMLElement, handlers: Handlers, videoId: string, channelId?: string): Promise<void> => {
  loadYoutubeJs();
  return promise.then(() => {
    ...
    const onAdStart = (): void => {
      window.ga(`${gaTracker}.send`, 'event', buildPfpEvent('adStart', videoId));
    };

    const onAdEnd = (): void => {
      window.ga(`${gaTracker}.send`, 'event', buildPfpEvent('adEnd', videoId));
    };

    return setupPlayer(el, videoId, channelId, onPlayerReady, onPlayerStateChange, onPlayerError, onAdStart, onAdEnd);
  });
};
