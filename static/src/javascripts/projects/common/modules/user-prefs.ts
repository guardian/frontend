...
const set = (name: string, value: any, {
  type
}: StorageOptions = defaultOptions): void => {
  storage[type].set(storagePrefix + name, value);
};

const get = (name: string, {
  type
}: StorageOptions = defaultOptions): any => storage[type].get(storagePrefix + name);
...
const isOn = (name: string, {
  type
}: StorageOptions = defaultOptions): boolean => storage[type].get(`${storagePrefix}switch.${name}`) === true;

const isOff = (name: string, {
  type
}: StorageOptions = defaultOptions): boolean => storage[type].get(`${storagePrefix}switch.${name}`) === false;
