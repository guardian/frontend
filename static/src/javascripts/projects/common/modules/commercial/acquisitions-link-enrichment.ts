...
const validIframeUrls: string[] = ['https://interactive.guim.co.uk/embed/2017/12/the-mother-load/', 'https://interactive.guim.co.uk/embed/2018/this-wide-brown-land/'];

const isCurrentCampaign = (iframeSrc: string): boolean => validIframeUrls.some(validIframeUrl => iframeSrc.startsWith(validIframeUrl));
...
const addReferrerDataToAcquisitionLink = (rawUrl: string): string => {
  ...
  let acquisitionData;
  try {
    const acquisitionDataJsonString = url.searchParams.get(acquisitionDataField);
  ...
  window.addEventListener('message', event => {
    ...
    if (data.type === 'enrich-acquisition-links' && data.id) {
      ...
      Array.from(document.getElementsByTagName('iframe')).forEach(iframe => {
        iframe.contentWindow.postMessage(JSON.stringify(data), 'https://interactive.guim.co.uk');
      });
    }
