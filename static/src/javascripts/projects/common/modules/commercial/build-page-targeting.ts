...
import { getReferrer as detectGetReferrer, getBreakpoint, getViewport } from "lib/detect";
import { getSync as geolocationGetSync } from "lib/geolocation";
import { storage } from "@guardian/libs";
import { getUrlVars } from "lib/url";
import { getPrivacyFramework } from "lib/getPrivacyFramework";
import { onConsentChange } from "@guardian/consent-management-platform";
import { getPermutiveSegments, clearPermutiveSegments } from "common/modules/commercial/permutive";
...
const format = (keyword: string): string => keyword.replace(/[+\s]+/g, '-').toLowerCase();

// flowlint sketchy-null-string:warn
const formatTarget = (target: string | null | undefined): string | null | undefined => target ? format(target).replace(/&/g, 'and').replace(/'/g, '') : null;
...
const getReferrer = (): string | null | undefined => {
  ...
  const matchedRef: MatchType = referrerTypes.filter(referrerType => detectGetReferrer().indexOf(referrerType.match) > -1)[0] || {};
...
const formatAppNexusTargeting = (obj: {
  [key: string]: string;
}): string => flattenDeep(Object.keys(obj).filter((key: string) => obj[key] !== '' && obj[key] !== null).map((key: string) => {
  const value: Array<string> | string = obj[key];
  return Array.isArray(value) ? value.map(nestedValue => `${key}=${nestedValue}`) : `${key}=${value}`;
})).join(',');

const buildAppNexusTargetingObject = once((pageTargeting: PageTargeting): {} => removeFalseyValues({
...
const buildAppNexusTargeting = once((pageTargeting: PageTargeting): string => formatAppNexusTargeting(buildAppNexusTargetingObject(pageTargeting)));
...
const buildPageTargetting = (adConsentState: boolean | null, ccpaState: boolean | null, tcfv2EventStatus: string | null): {
  [key: string]: unknown;
} => {
  ...
  const pageTargets: PageTargeting = Object.assign({
    ...
    rp: config.get('isDotcomRendering', false) ? 'dotcom-rendering' : 'dotcom-platform', // rendering platform
    dcre: config.get('isDotcomRendering', false) || config.get('page.dcrCouldRender', false) ? 't' : 'f',
...
const getPageTargeting = (): {
  ...
  onConsentChange(state => {
    ...
    if (state.ccpa) {
      ...
      // TCFv2 mode
      canRun = state.tcfv2.consents ? Object.keys(state.tcfv2.consents).length > 0 && Object.values(state.tcfv2.consents).every(Boolean) : false;
    } else if (state.aus) {
      // AUS mode
...
export { getPageTargeting, buildAppNexusTargeting, buildAppNexusTargetingObject };
