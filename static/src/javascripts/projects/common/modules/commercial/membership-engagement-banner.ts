...
import { ReaderRevenueRegion, getReaderRevenueRegion, canShowBannerSync, getVisitCount } from "common/modules/commercial/contributions-utilities";
import { Banner } from "common/modules/ui/bannerPicker";
import bean from "bean";
import fetchJson from "lib/fetch-json";
import reportError from "lib/report-error";

import { submitComponentEvent, addTrackingCodesToUrl } from "common/modules/commercial/acquisitions-ophan";
 ...
 */
const getTimestampOfLastBannerDeployForLocation = (region: ReaderRevenueRegion): Promise<string> => fetchJson(`/reader-revenue/contributions-banner-deploy-log/${region === 'european-union' ? 'rest-of-world' : region}`, {
  mode: 'cors'
}).then((resp: BannerDeployLog) => resp && resp.time);

const hasBannerBeenRedeployedSinceClosed = (userLastClosedBannerAt: string, region: ReaderRevenueRegion): Promise<boolean> => getTimestampOfLastBannerDeployForLocation(region).then(timestamp => {
  ...
  // Capture in sentry
  reportError(new Error(`Unable to get engagement banner deploy log: ${err}`), { feature: 'engagement-banner' }, false);
  return false;
});

const deriveBannerParams = (testToRun: Runnable<AcquisitionsABTest> | null | undefined): Promise<EngagementBannerParams> => getControlEngagementBannerParams().then(defaultParams => {
...
const selectSequentiallyFrom = (array: Array<string>): string => array[getVisitCount() % array.length];
...
const pageIsIdentity = (): boolean => {
  const isIdentityPage = config.get('page.contentType') === 'Identity' || config.get('page.section') === 'identity';
  return isIdentityPage;
};

const bannerParamsToHtml = (params: EngagementBannerParams): string => {
  const messageText = Array.isArray(params.messageText) ? selectSequentiallyFrom(params.messageText) : params.messageText;
  ...
  return params.template ? params.template(templateParams) : acquisitionsBannerControlTemplate(templateParams);
};

const showBannerAsMessage = (code: string, params: EngagementBannerParams, html: string): boolean => new Message(code, {
  ...
  customJs() {
    bean.on(document, 'click', '.js-engagement-banner-close-button', () => hideBanner(this));
  }
}).show(html);
...
const getBannerParams = memoize((): Promise<EngagementBannerParams> => getEngagementBannerTestToRun().then(deriveBannerParams));

const show = (): Promise<boolean> => getBannerParams().then(showBanner).catch(err => {
  reportError(new Error(`Could not show banner. ${err.message}. Stack: ${err.stack}`), { feature: 'engagement-banner' }, false);
  return false;
});

const canShow = (): Promise<boolean> => {
  if (!config.get('switches.membershipEngagementBanner') || isBlocked() || pageIsIdentity()) {
  ...
  return getBannerParams().then(params => {
    if (canShowBannerSync(params.minArticlesBeforeShowingBanner, params.userCohort)) {
      ...
      return hasBannerBeenRedeployedSinceClosed(userLastClosedBannerAt, getReaderRevenueRegion(geolocationGetSync()));
    }
    return Promise.resolve(false);
  });
};
...
export { membershipEngagementBanner, canShow, messageCode, hideBanner, clearBannerHistory, minArticlesBeforeShowingBanner, deriveBannerParams, bannerParamsToHtml, trackBanner };