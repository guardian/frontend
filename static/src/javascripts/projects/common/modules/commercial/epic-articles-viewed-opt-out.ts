...
): ArticlesViewedOptOutElements | null | undefined => {
    const checkbox = container.querySelector(
        '#epic-article-count__dialog-checkbox'
    );

    const labelElement = container.querySelector(
        '.epic-article-count__prompt-label'
    );
    const optOutButton = container.querySelector(
        '.epic-article-count__button-opt-out'
    );
    const optInButton = container.querySelector(
        '.epic-article-count__button-opt-in'
    );
...
const setupArticlesViewedOptOut = () => {
    ...
    if (articleCountElement) {
        const elements:
            | ArticlesViewedOptOutElements
            | null
            | undefined = getElements(articleCountElement);
