...
jest.mock('common/modules/commercial/membership-engagement-banner-parameters', () => ({
...
jest.mock('common/modules/commercial/membership-engagement-banner-block', () => ({
...
const fakeIsBlocked: any = require('common/modules/commercial/membership-engagement-banner-block').isBlocked;
const fakeGet: any = require('lib/storage').local.get;
const fakeShouldHideReaderRevenue: any = require('common/modules/commercial/contributions-utilities').pageShouldHideReaderRevenue;
const fakeCanShowBannerSync: any = require('common/modules/commercial/contributions-utilities').canShowBannerSync;

const fetchJsonMock: JestMockFn<any, any> = (fetchJson as any);
const fakeUserPrefs: JestMockFn<any, any> = (userPrefs.get as any);
...
beforeEach(() => {
  ...
  fetchJsonMock.mockImplementation(() => Promise.resolve({ time: '2018-07-25T17:05:46+0000' }));
});
...
describe('Membership engagement banner', () => {
  ...
  describe('canShow returns true', () => {
    ...
    it('should show the membership engagement banner', () => membershipEngagementBanner.show().then(shown => {
      expect(shown).toBe(true);
      expect(FakeMessage.prototype.show).toHaveBeenCalledTimes(1);
    }));

    it('should record the component event in ophan with a/b test info', () => membershipEngagementBanner.show().then(() => expect(fakeOphan.record).toHaveBeenCalledWith({
      componentEvent: {
        component: {
          componentType: 'ACQUISITIONS_ENGAGEMENT_BANNER',
          products: ['CONTRIBUTION'],
  ...
  describe('If user already member', () => {
    it('should not show any messages even to engaged readers', () => {
      (pageShouldHideReaderRevenue as any).mockImplementationOnce(() => true);
  ...
  describe('creates message with', () => {
    ...
    it('correct campaign code', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.mock.calls[0][1].siteMessageComponentName).toBe('fake-test-id_fake-variant-id')));

    it('correct CSS modifier class', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.mock.calls[0][1].cssModifierClass).toBe('engagement-banner')));
  });
  ...
  describe('renders message with', () => {
    beforeEach(() => {
      ...
      fakeConstructQuery.mockImplementationOnce(() => 'fake-query-parameters');
    });

    it('message text', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.prototype.show.mock.calls[0][0]).toMatch(/fake-message-text/)));

    it('colour class', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.prototype.show.mock.calls[0][0]).toMatch(/engagement-banner/)));

    it('link URL', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.prototype.show.mock.calls[0][0]).toMatch(/https:\/\/support\.theguardian\.com\/contribute\?fake-query-parameters/)));

    it('button caption', () => membershipEngagementBanner.show().then(() => expect(FakeMessage.prototype.show.mock.calls[0][0]).toMatch(/fake-button-caption/)));
  });
});