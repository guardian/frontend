...
const isInExperiment = (): boolean => config.get('switches.scAdFreeBanner', false);

const hideBanner = (banner: Message) => {
  banner.acknowledge();
};

const canShow: () => Promise<boolean> = () => Promise.resolve(!hasUserAcknowledgedBanner(messageCode) && isAdFreeUser() && isInExperiment());
...
const show = (): Promise<boolean> => {
  new Message(messageCode, {
    ...
    customJs() {
      bean.on(document, 'click', '.js-ad-free-banner-dismiss-button', () => hideBanner(this));
    }
  }).show(`
