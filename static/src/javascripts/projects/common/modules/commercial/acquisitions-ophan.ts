...
export const submitInsertEvent = (componentEvent: ComponentEventWithoutAction) => submitComponentEvent({
  ...componentEvent,
  action: 'INSERT'
});

export const submitViewEvent = (componentEvent: ComponentEventWithoutAction) => submitComponentEvent({
  ...componentEvent,
  action: 'VIEW'
});

export const submitClickEvent = (componentEvent: ComponentEventWithoutAction) => submitComponentEvent({
  ...componentEvent,
  action: 'CLICK'
});
...
export const updateAcquisitionData = (url: URL, acquisitionData: AcquisitionData): URL => {
  let existingAcquisitionData;

  try {
    const existingAcquisitionJsonString = url.searchParams.get(ACQUISITION_DATA_FIELD);
  ...
  newUrl.searchParams.set(ACQUISITION_DATA_FIELD, JSON.stringify({
...
export const addReferrerData = (acquisitionData: {}): {} => ( // Note: the current page is the referrer data in the context of the acquisition.
{
  ...acquisitionData,
