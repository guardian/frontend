...
const allSwitches: Array<string> = ['Uk', 'Us', 'Au'].map(region => `membershipEngagementBannerBlock${region}`);

export const isBlocked = (switches: Array<string> = allSwitches, switchBlockConfig: SwitchBlockConfig = defaultSwitchConfig, pathname: string = document.location.pathname, geolocation: string = getSync()): boolean => {
  // the enabled block configurations that apply to the provided geolocation
  const activeBlockConfigs: Array<BlockConfig> = switches.filter(switchName => !!config.get(`switches.${switchName}`)).map(switchName => switchBlockConfig[switchName]).filter(conf => conf.geolocation === geolocation);

  const blockedUrls: Array<string> = flattenDeep(activeBlockConfigs.map(conf => conf.urls));
