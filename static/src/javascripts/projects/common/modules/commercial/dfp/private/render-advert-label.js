            ...
            if (config.switches.abAdFeedback) {
                try {
                    ...
                    if (abParticipant && abParticipant.variant === 'ad-feedback-variant') {
                        var feedBackOpts = '<li class="popup__item"><a><div class="popup__item-problem--option" slot="' + adSlotNode.id + '" problem="distracting">Distracting</div></a></li>';
                        feedBackOpts = feedBackOpts + '<li class="popup__item"><a><div class="popup__item-problem--option" slot="' + adSlotNode.id + '" problem="obscures">Obscures content</div></a></li>';
                        feedBackOpts = feedBackOpts + '<li class="popup__item"><a><div class="popup__item-problem--option" slot="' + adSlotNode.id + '" problem="inappropriate">Inappropriate</div></a></li>';
                        feedBackOpts = feedBackOpts + '<li class="popup__item"><a><div class="popup__item-problem--option" slot="' + adSlotNode.id + '" problem="repetetive">Repetitive</div></a></li>';
                        feedBackOpts = feedBackOpts + '<li class="popup__item"><a><div class="popup__item-problem--option" slot="' + adSlotNode.id + '" problem="irrelevant">Irrelevant</div></a></li>';
                        var feedbackMenu = '<div class="ad-feedback popup popup--default popup__group is-off ' + adSlotNode.id + '__popup--feedback" id="ad-feedback-menu__' + adSlotNode.id + '" style="z-index: ' + zIndexOverlay + ';"><h3 class="ad-feedback popup__group-header"><p>Thank you for reporting this advert.</p><p>To help improve the ad experience on the Guardian, can you tell us why you\'ve reported the ad?</p></h3><ul>' + feedBackOpts + '</ul></div>';
