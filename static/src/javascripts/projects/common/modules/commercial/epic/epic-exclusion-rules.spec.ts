...
describe('isArticleWorthAnEpicImpression', () => {
  describe('when an article matches section but not toneIds of an exclusion rule', () => {
    it('is worth an epic impression', () => {
      const isItWorthIt = isArticleWorthAnEpicImpression({ section: 'a', toneIds: 'tone/news', keywordIds: 'us-news' }, [{ section: 'a', toneIds: ['tone/blah'] }]);
  ...
  describe('when an article matches section but not keywordIds of an exclusion rule', () => {
    it('is worth an epic impression', () => {
      const isItWorthIt = isArticleWorthAnEpicImpression({ section: 'a', toneIds: 'tone/news', keywordIds: 'us-news' }, [{ section: 'a', keywordIds: ['us-news', 'something-else'] }]);
      expect(isItWorthIt).toBe(true);
    });
  });
  ...
  describe('when an article matches toneIds but not section of an exclusion rule', () => {
    it('is worth an epic impression', () => {
      const isItWorthIt = isArticleWorthAnEpicImpression({ section: 'a', toneIds: 'tone/news', keywordIds: 'us-news' }, [{ section: 'b', toneIds: ['tone/news'] }]);
      expect(isItWorthIt).toBe(true);
    });
  });
