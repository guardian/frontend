...
const controlEpicComponent = (abTest?: ABTestVariant): Promise<EpicComponent> => {
...
const insertAtSubmeta = (epic: EpicComponent): Promise<EpicComponent> => fastdom.read(() => document.querySelector('.submeta')).then(element => {
...
const displayControlEpic = (abTest?: ABTestVariant): Promise<EpicComponent> => controlEpicComponent(abTest).then(epic => insertAtSubmeta(epic)).catch(error => {
  const controlEpicError = new Error(`unable to display control epic - ${error}`);
  reportEpicError(controlEpicError);
  return Promise.reject(controlEpicError);
});

const displayControlEpicInAbTest = (abTest: ABTestVariant): Promise<EpicComponent> => displayControlEpic(abTest);
...
const awaitEpicButtonClicked = (): Promise<void> => new Promise(resolve => {
...
export { reportEpicError, insertAtSubmeta, displayControlEpic, displayControlEpicInAbTest, trackEpic, awaitEpicButtonClicked };