...
const createEpicIframe = (url: string): Error | IframeEpicComponent => {
  ...
  try {
    ...
    const iframeError = new Error(`unable to get origin of iframe Epic - ${error}`);
...
const displayIframeEpic = (url: string): Promise<IframeEpicComponent> => {
  ...
  return insertAtSubmeta(iframeEpicComponent).then(() => iframeEpicComponent).catch(error => {
    const iframeError = new Error(`unable to display iframe epic with url ${url} - ${error}`);
    reportEpicError(iframeError);
