...
const SUPPORT_RECURRING_CONTRIBUTOR_MONTHLY_COOKIE = 'gu.contributions.recurring.contrib-timestamp.Monthly';
const SUPPORT_RECURRING_CONTRIBUTOR_ANNUAL_COOKIE = 'gu.contributions.recurring.contrib-timestamp.Annual';
const SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE = 'gu.contributions.contrib-timestamp';
...
const accountDataUpdateWarning = (): string | null | undefined => getCookie(ACTION_REQUIRED_FOR_COOKIE);
...
const persistResponse = (JsonResponse: () => void) => {
  ...
  addCookie(RECURRING_CONTRIBUTOR_COOKIE, JsonResponse.contentAccess.recurringContributor);
  addCookie(DIGITAL_SUBSCRIBER_COOKIE, JsonResponse.contentAccess.digitalPack);
  addCookie(HIDE_SUPPORT_MESSAGING_COOKIE, !JsonResponse.showSupportMessaging);
  if (JsonResponse.oneOffContributionDate) {
    addCookie(ONE_OFF_CONTRIBUTION_DATE_COOKIE, JsonResponse.oneOffContributionDate);
  }
...
const requestNewData = (): Promise<void> => fetchJson(`${config.get('page.userAttributesApiUrl')}/me`, {
  mode: 'cors',
  credentials: 'include'
}).then(persistResponse).catch(() => {});
...
const featuresDataIsMissing = (): boolean => !getCookie(USER_FEATURES_EXPIRY_COOKIE);

const featuresDataIsOld = (): boolean => datedCookieIsOld(USER_FEATURES_EXPIRY_COOKIE);
...
const userNeedsNewFeatureData = (): boolean => featuresDataIsMissing() || featuresDataIsOld() || (adFreeDataIsPresent() && adFreeDataIsOld());

const userHasDataAfterSignout = (): boolean => !isUserLoggedIn() && userHasData();
...
const supportSiteRecurringCookiePresent = () => getCookie(SUPPORT_RECURRING_CONTRIBUTOR_MONTHLY_COOKIE) != null || getCookie(SUPPORT_RECURRING_CONTRIBUTOR_ANNUAL_COOKIE) != null;

/**
 * Does our _existing_ data say the user is a paying member?
 * This data may be stale; we do not wait for userFeatures.refresh()
 */
const isPayingMember = (): boolean => // If the user is logged in, but has no cookie yet, play it safe and assume they're a paying user
...
const getSupportFrontendOneOffContributionDate = (): number | null => {
  const supportFrontendCookie = getCookie(SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE);
...
const getLastOneOffContributionDate = (): number | null => getSupportFrontendOneOffContributionDate() || getAttributesOneOffContributionDate();
...
const getLastRecurringContributionDate = (): number | null => {
  ...
  const monthlyCookie = getCookie(SUPPORT_RECURRING_CONTRIBUTOR_MONTHLY_COOKIE);
  ...
  const monthlyMS = monthlyTime && Number.isInteger(monthlyTime) ? monthlyTime : null;
  const annualMS = annualTime && Number.isInteger(annualTime) ? annualTime : null;
...
const isRecurringContributor = (): boolean => // If the user is logged in, but has no cookie yet, play it safe and assume they're a contributor
(isUserLoggedIn() && getCookie(RECURRING_CONTRIBUTOR_COOKIE) !== 'false') || supportSiteRecurringCookiePresent();

const isDigitalSubscriber = (): boolean => getCookie(DIGITAL_SUBSCRIBER_COOKIE) === 'true';

const shouldNotBeShownSupportMessaging = (): boolean => getCookie(HIDE_SUPPORT_MESSAGING_COOKIE) === 'true';
...
*/
const shouldHideSupportMessaging = (): boolean => shouldNotBeShownSupportMessaging() || isRecentOneOffContributor() || // because members-data-api is unaware of one-off contributions so relies on cookie
...
const readerRevenueRelevantCookies = [PAYING_MEMBER_COOKIE, DIGITAL_SUBSCRIBER_COOKIE, RECURRING_CONTRIBUTOR_COOKIE, SUPPORT_RECURRING_CONTRIBUTOR_MONTHLY_COOKIE, SUPPORT_RECURRING_CONTRIBUTOR_ANNUAL_COOKIE, SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE, HIDE_SUPPORT_MESSAGING_COOKIE];
...
const isAdFreeUser = (): boolean => isDigitalSubscriber() || (adFreeDataIsPresent() && !adFreeDataIsOld());
...
const extendContribsCookieExpiry = (): void => {
  ...
  if (cookie) {
    ...
    if (Number.isInteger(contributionDate)) {
      ...
      addCookie(SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE, contributionDate.toString(), daysToLive);
    }
  }
};
