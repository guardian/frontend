...
const getLiveblogEntryTimeData = (el: Element): TimeData | null | undefined => {
...
const getBlocksToInsertEpicAfter = (): Array<HTMLElement> => {
  ...
  const blocksToInsertManualEpicAfter = document.getElementsByClassName(INSERT_EPIC_AFTER_CLASS);
...
const setEpicLiveblogEntryTimeData = (el: Element, timeData: TimeData): void => {
...
const setupViewTracking = (el: HTMLElement, variant: EpicVariant, parentTest: EpicABTest): void => {
...
const addEpicToBlocks = (epicHtml: string, variant: EpicVariant, parentTest: EpicABTest): Promise<void> => {
  const elementsWithTimeData = getBlocksToInsertEpicAfter().map(el => [el, getLiveblogEntryTimeData(el)]);
...
export const setupEpicInLiveblog = (epicHtml: string, variant: EpicVariant, parentTest: EpicABTest): void => {
