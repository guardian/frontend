...
const getFields = (): Fields | null | undefined => {
  const helpText = document.querySelector('.epic-reminder__email-help-text');
  const submitButton = document.querySelector('.epic-reminder__submit-button');
  ...
  const reminderPrompt = document.querySelector('.component-button--reminder-prompt');
  const reminderToggle = document.querySelector('.epic-reminder__reveal-reminder');

  if (helpText && submitButton && emailInput && titleField && thankYouText && formWrapper && closeButton && reminderPrompt && reminderToggle && submitButton instanceof HTMLButtonElement && emailInput instanceof HTMLInputElement && reminderToggle instanceof HTMLInputElement) {
...
const epicReminderEmailSignup = (fields: Fields) => {
  ...
  const setState = (state: ReminderState) => {
    switch (state) {
      case 'invalid':
        ...
        fields.helpText.innerHTML = `${errorTriangle.markup} Invalid email address`;
        break;
      case 'pending':
      ...
      case 'success':
        ...
        fields.titleField.innerHTML = 'Thank you! Your reminder is set.';
        addCookie(CONTRIBUTIONS_REMINDER_SIGNED_UP.name, new Date().getTime().toString(), CONTRIBUTIONS_REMINDER_SIGNED_UP.daysToLive);
        break;
      case 'failure':
        fields.submitButton.innerHTML = 'Something went wrong';
        fields.helpText.innerHTML = "Sorry we weren't able to sign you up for a reminder, please try again later";
        break;
      default:
        break;
  ...
  const sendReminderEvent = (): Promise<Response> => {
    ...
    const reminderDate = fields.submitButton.getAttribute('data-reminder-date');

    const createReminderEndpoint = isProd ? 'https://contribution-reminders.support.guardianapis.com/remind-me' : 'https://contribution-reminders-code.support.guardianapis.com/remind-me';
  ...
  const sendPromptClickEvent = () => {
    const ctaAttribute = fields.reminderPrompt.getAttribute('data-cta-copy');
    const ctaForAnalytics = ctaAttribute ? ctaAttribute.toLowerCase().replace(/\s/g, '-') : '';
  ...
  fields.reminderPrompt.addEventListener('keypress', (event: KeyboardEvent) => {
    if (event.key === 'Enter') {
