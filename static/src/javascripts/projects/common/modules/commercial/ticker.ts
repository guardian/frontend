...
const parseTickerSettings = (
    obj: Object
): TickerSettings | null | undefined => {
    const endType =
        obj.endType === 'unlimited' || obj.endType === 'hardstop'
            ? obj.endType
            : null;
    const countType =
        obj.countType === 'money' || obj.countType === 'people'
            ? obj.countType
            : null;
    const copy =
        obj.copy &&
        obj.copy.countLabel &&
        obj.copy.goalReachedPrimary &&
        obj.copy.goalReachedSecondary
            ? obj.copy
            : null;
...
const populateGoal = (
    ...
    if (goalElement) {
        ...
        if (countElement && labelElement) {
            const amount =
                goalReached() && tickerSettings.endType === 'unlimited'
                    ? total
                    : goal;
            countElement.innerHTML = `${getCurrencySymbol(
                tickerSettings
