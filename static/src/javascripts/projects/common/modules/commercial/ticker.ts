...
const parseTickerSettings = (obj: Object): TickerSettings | null | undefined => {
...
const getCurrencySymbol = (tickerSettings: TickerSettings): string => tickerSettings.countType === 'money' ? tickerSettings.currencySymbol : '';
...
const animateBar = (parentElement: HTMLElement, tickerType: TickerEndType) => {
  const progressBarElement = parentElement.querySelector('.js-ticker-filled-progress');
  ...
  if (progressBarElement && progressBarElement instanceof HTMLElement) {
    ...
    if (end > goal) {
      ...
      const marker = parentElement.querySelector('.js-ticker-goal-marker');
      if (marker) {
...
const increaseCounter = (parentElementSelector: string, counterElement: HTMLElement, tickerSettings: TickerSettings) => {
  // Count is local to the parent element
  const newCount = count[parentElementSelector] + Math.floor(total / 100);

  const finishedCounting = newCount <= count[parentElementSelector] || newCount >= total; // either we've reached the total or the count isn't going up because total is too small
  ...
  if (finishedCounting) {
    ...
    counterElement.innerHTML = `${currencySymbol}${count[parentElementSelector].toLocaleString()}`;

    window.requestAnimationFrame(() => increaseCounter(parentElementSelector, counterElement, tickerSettings));
  }
};

const populateStatusSoFar = (parentElementSelector: string, parentElement: HTMLElement, tickerSettings: TickerSettings) => {
  const counterElement = parentElement.querySelector(`.js-ticker-amounts .js-ticker-count`);

  const labelElement = parentElement.querySelector(`.js-ticker-amounts .js-ticker-label`);
...
const populateGoal = (parentElement: HTMLElement, tickerSettings: TickerSettings) => {
  ...
  if (goalElement) {
    ...
    if (countElement && labelElement) {
      const amount = goalReached() && tickerSettings.endType === 'unlimited' ? total : goal;
...
const animate = (parentElementSelector: string, tickerSettings: TickerSettings) => {
  ...
  if (parentElement && parentElement instanceof HTMLElement) {
    ...
    window.setTimeout(() => {
      ...
      window.requestAnimationFrame(() => populateStatusSoFar(parentElementSelector, parentElement, tickerSettings));
...
const getTickerUrl = (countType: TickerCountType): string => countType === 'people' ? 'https://support.theguardian.com/supporters-ticker.json' : 'https://support.theguardian.com/ticker.json';

const fetchDataAndAnimate = (parentElementSelector: string, tickerSettings: TickerSettings) => {
...
const initTicker = (parentElementSelector: string, tickerSettings: TickerSettings | null | undefined) => {
  fetchDataAndAnimate(parentElementSelector, tickerSettings || defaultSettings);
};

export { initTicker, parseTickerSettings };