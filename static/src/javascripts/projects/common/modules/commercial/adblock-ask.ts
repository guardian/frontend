...
const canShow = () => !shouldHideSupportMessaging() && !pageShouldHideReaderRevenue() && !config.get('page.hasShowcaseMainElement');

export const initAdblockAsk = () => {
  if (canShow()) {
    fastdom.measure(() => $('.js-aside-slot-container')).then(slot => {
      if (slot) {
        fastdom.mutate(() => {
          slot.append(askHtml);
        });
