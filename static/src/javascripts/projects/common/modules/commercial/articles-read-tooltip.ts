...
): ArticlesViewedTooltipElements | null | undefined => {
    const articlesRead = container.querySelector(
        '.engagement-banner__articles-read'
    );
    const wrapper = container.querySelector(
        '.engagement-banner__articles-read-tooltip'
    );
    const buttons = container.querySelector(
        '.engagement-banner__articles-read-tooltip-buttons'
    );
    const optOutButton = container.querySelector(
        '.engagement_banner__articles-read-tooltip-button-opt-out'
    );
    const optInButton = container.querySelector(
        '.engagement_banner__articles-read-tooltip-button-opt-in'
    );
    const header = container.querySelector(
        '.engagement-banner__articles-read-tooltip-header'
    );
    const body = container.querySelector(
        '.engagement-banner__articles-read-tooltip-body'
    );
    const note = container.querySelector(
        '.engagement-banner__articles-read-tooltip-note'
    );
    const closeButton = container.querySelector(
        '.engagement-banner__articles-read-tooltip-close'
    );
...
const setupHandlers = (elements: ArticlesViewedTooltipElements) => {
    ...
    const onOptOutClick = () => {
        ...
        addCookie(
            ARTICLES_VIEWED_OPT_OUT_COOKIE.name,
            new Date().getTime().toString(),
            ARTICLES_VIEWED_OPT_OUT_COOKIE.daysToLive
        );
...
const bannerSetupArticlesViewedOptOut = () => {
    ...
    const articlesReadTooltipWrapper = document.querySelector(
        '.engagement-banner__articles-read-tooltip-wrapper'
    );

    if (articlesReadTooltipWrapper) {
        const elements:
            | ArticlesViewedTooltipElements
            | null
            | undefined = getElements(articlesReadTooltipWrapper);
