...
const getAcquisitionsBannerParams = (rowsFromGoogleDoc: any): EngagementBannerParams => {
  const firstRow = rowsFromGoogleDoc && rowsFromGoogleDoc[0];

  if (!(firstRow && firstRow.messageText && firstRow.ctaText && firstRow.buttonCaption && firstRow.linkUrl)) {
    throw new Error(`Required data from the Google Doc was missing. Got row: ${firstRow}`);
  }

  const ctaText = `<span class="engagement-banner__highlight"> ${firstRow.ctaText.replace(/%%CURRENCY_SYMBOL%%/g, getLocalCurrencySymbolSync())}</span>`;
...
export const getControlEngagementBannerParams = (): Promise<EngagementBannerParams> => getEngagementBannerControlFromGoogleDoc().then(rows => getAcquisitionsBannerParams(rows)).catch(err => {
  reportError(new Error(`Could not fetch control banner copy from Google Doc. ${err.message}. Stack: ${err.stack}`), {
