...
import { setupOphanView, emitBeginEvent, setupClickHandling, makeEvent, submitOphanInsert, getVisitCount } from "common/modules/commercial/contributions-utilities";
...
export const fetchBannerData: () => Promise<BannerDataResponse | null | undefined> = () => {
...
export const fetchAndRenderEpic = async (id: string): Promise<void> => {
  try {
    ...
    submitOphanInsert(abTestName, abTestVariant, componentType, products, campaignCode);
    setupOphanView(el, viewEvent, abTestName, abTestVariant, campaignCode, campaignId, componentType, products, abTestVariant.showTicker, abTestVariant.tickerSettings);
  } catch (error) {
