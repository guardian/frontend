...
import { setupOphanView, emitBeginEvent, setupClickHandling, submitOphanInsert, getVisitCount } from "common/modules/commercial/contributions-utilities";
...
import { getLastOneOffContributionDate, isRecurringContributor, shouldHideSupportMessaging, ARTICLES_VIEWED_OPT_OUT_COOKIE } from "common/modules/commercial/user-features";
...
const hasOptedOutOfArticleCount = () => !!getCookie(ARTICLES_VIEWED_OPT_OUT_COOKIE.name);
...
export const setLocalNoBannerCachePeriod = (): void => window.localStorage.setItem(NO_RR_BANNER_TIMESTAMP_KEY, `${Date.now()}`);
...
const checkResponseOk = response => {
  ...
  throw new Error(`Contributions fetch failed with response code: ${response.status}`);
};
...
const getEpicUrl = (contentType: string): string => {
  ...
  return config.get('page.isDev') ? `https://contributions.code.dev-guardianapis.com/${path}` : `https://contributions.guardianapis.com/${path}`;
};
...
const renderEpic = async (module, meta): Promise<void> => {
  ...
  setupOphanView(el, abTestName, abTestVariant, campaignCode, campaignId, componentType, products, abTestVariant.showTicker, abTestVariant.tickerSettings);
};

export const fetchBannerData: () => Promise<BannerDataResponse | null | undefined> = async () => {
  ...
  if (payload.targeting.engagementBannerLastClosedAt && payload.targeting.subscriptionBannerLastClosedAt && withinLocalNoBannerCachePeriod()) {
    return Promise.resolve(null);
  }
  ...
  return getStickyBottomBanner(payload).then(json => {
    ...
    return (json as BannerDataResponse);
  });
};

export const renderBanner: (arg0: BannerDataResponse) => Promise<boolean> = response => {
  const {
    module,
    meta
  }: {
  ...
  // $FlowFixMe
  return window.guardianPolyfilledImport(module.url).then(bannerModule => {
    ...
    return fastdom.mutate(() => {
      ...
      return mountDynamic(container, Banner, { submitComponentEvent, ...module.props }, true);
    }).then(() => {
      const {
        abTestName,
        abTestVariant,
