...
import { refresh, isAdFreeUser, isPayingMember, isRecurringContributor, accountDataUpdateWarning, isDigitalSubscriber, getLastOneOffContributionDate, getDaysSinceLastOneOffContribution, isRecentOneOffContributor, shouldNotBeShownSupportMessaging, getLastRecurringContributionDate, isPostAskPauseOneOffContributor } from "./user-features";
...
const PERSISTENCE_KEYS = {
  ...
  SUPPORT_MONTHLY_CONTRIBUTION_COOKIE: 'gu.contributions.recurring.contrib-timestamp.Monthly',
  SUPPORT_ANNUAL_CONTRIBUTION_COOKIE: 'gu.contributions.recurring.contrib-timestamp.Annual'
};

const setAllFeaturesData = opts => {
  const currentTime = new Date().getTime();
  const msInOneDay = 24 * 60 * 60 * 1000;
  const expiryDate = opts.isExpired ? new Date(currentTime - msInOneDay) : new Date(currentTime + msInOneDay);
  const adFreeExpiryDate = opts.isExpired ? new Date(currentTime - msInOneDay * 2) : new Date(currentTime + msInOneDay * 2);
  ...
  addCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE, adFreeExpiryDate.getTime().toString());
  addCookie(PERSISTENCE_KEYS.USER_FEATURES_EXPIRY_COOKIE, expiryDate.getTime().toString());
...
const setExpiredAdFreeData = () => {
  ...
  addCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE, expiryDate.getTime().toString());
};
...
describe('Refreshing the features data', () => {
  describe('If user signed in', () => {
    ...
    it('Does not delete the data just because it has expired', () => {
      setAllFeaturesData({ isExpired: true });
      refresh();
      expect(getCookie(PERSISTENCE_KEYS.PAYING_MEMBER_COOKIE)).toBe('true');
      expect(getCookie(PERSISTENCE_KEYS.RECURRING_CONTRIBUTOR_COOKIE)).toBe('true');
      expect(getCookie(PERSISTENCE_KEYS.USER_FEATURES_EXPIRY_COOKIE)).toEqual(expect.stringMatching(/\d{13}/));
      expect(getCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE)).toEqual(expect.stringMatching(/\d{13}/));
    });
  ...
  describe('If user signed out', () => {
    ...
    it('Deletes leftover feature data', () => {
      ...
      expect(getCookie(PERSISTENCE_KEYS.RECURRING_CONTRIBUTOR_COOKIE)).toBeNull();
      expect(getCookie(PERSISTENCE_KEYS.DIGITAL_SUBSCRIBER_COOKIE)).toBeNull();
      expect(getCookie(PERSISTENCE_KEYS.USER_FEATURES_EXPIRY_COOKIE)).toBeNull();
    });
  });
});
...
describe('Storing new feature data', () => {
  ...
  it('Puts the paying-member state and ad-free state in appropriate cookie', () => {
    fetchJsonSpy.mockReturnValueOnce(Promise.resolve({
      contentAccess: {
        paidMember: false,
    ...
    return refresh().then(() => {
      expect(getCookie(PERSISTENCE_KEYS.PAYING_MEMBER_COOKIE)).toBe('false');
      expect(getCookie(PERSISTENCE_KEYS.RECURRING_CONTRIBUTOR_COOKIE)).toBe('false');
      expect(getCookie(PERSISTENCE_KEYS.DIGITAL_SUBSCRIBER_COOKIE)).toBe('false');
      expect(getCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE)).toBeNull();
    });
  });
  ...
  it('Puts the paying-member state and ad-free state in appropriate cookie', () => {
    ...
    return refresh().then(() => {
      expect(getCookie(PERSISTENCE_KEYS.PAYING_MEMBER_COOKIE)).toBe('true');
      expect(getCookie(PERSISTENCE_KEYS.RECURRING_CONTRIBUTOR_COOKIE)).toBe('true');
      expect(getCookie(PERSISTENCE_KEYS.DIGITAL_SUBSCRIBER_COOKIE)).toBe('true');
      expect(getCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE)).toBeTruthy();
      expect(Number.isNaN(parseInt(getCookie(PERSISTENCE_KEYS.AD_FREE_USER_COOKIE), 10))).toBe(false);
    });
  });

  it('Puts an expiry date in an accompanying cookie', () => refresh().then(() => {
    const expiryDate = getCookie(PERSISTENCE_KEYS.USER_FEATURES_EXPIRY_COOKIE);
    expect(expiryDate).toBeTruthy();
    expect(Number.isNaN(parseInt(expiryDate, 10))).toBe(false);
  }));

  it('The expiry date is in the future', () => refresh().then(() => {
    const expiryDateString = getCookie(PERSISTENCE_KEYS.USER_FEATURES_EXPIRY_COOKIE);
...
const setSupportFrontendOneOffContributionCookie = (value: any): void => addCookie(PERSISTENCE_KEYS.SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE, value);

const removeSupportFrontendOneOffContributionCookie = (): void => removeCookie(PERSISTENCE_KEYS.SUPPORT_ONE_OFF_CONTRIBUTION_COOKIE);

const setAttributesOneOffContributionCookie = (value: any): void => addCookie(PERSISTENCE_KEYS.ONE_OFF_CONTRIBUTION_DATE_COOKIE, value);

const removeAttributesOneOffContributionCookie = (): void => removeCookie(PERSISTENCE_KEYS.ONE_OFF_CONTRIBUTION_DATE_COOKIE);
...
describe('getting the last one-off contribution date of a user', () => {
  ...
  it('return the correct date if the user support-frontend contribution cookie is set', () => {
    setSupportFrontendOneOffContributionCookie(contributionDateTimeEpoch.toString());
...
const setMonthlyContributionCookie = (value: any): void => addCookie(PERSISTENCE_KEYS.SUPPORT_MONTHLY_CONTRIBUTION_COOKIE, value);

const setAnnualContributionCookie = (value: any): void => addCookie(PERSISTENCE_KEYS.SUPPORT_ANNUAL_CONTRIBUTION_COOKIE, value);

const removeMonthlyContributionCookie = (): void => removeCookie(PERSISTENCE_KEYS.SUPPORT_MONTHLY_CONTRIBUTION_COOKIE);

const removeAnnualContributionCookie = (): void => removeCookie(PERSISTENCE_KEYS.SUPPORT_ANNUAL_CONTRIBUTION_COOKIE);
...
describe('getting the last recurring contribution date of a user', () => {
  ...
  it('return the correct date if the user is a monthly recurring contributor', () => {
    ...
    expect(getLastRecurringContributionDate()).toBe(monthlyContributionTimestamp);
  });
  ...
  it('return the correct date if the user is a annual recurring contributor', () => {
    ...
    expect(getLastRecurringContributionDate()).toBe(annualContributionTimestamp);
  });
