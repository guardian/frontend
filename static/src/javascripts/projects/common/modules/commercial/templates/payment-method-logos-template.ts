...
export const paymentMethodLogosTemplate = (classNames: string) => {
  ...
  const paymentMethodLogos = config.get(inUS ? 'images.acquisitions.payment-methods-us' : 'images.acquisitions.payment-methods', '');

  const paymentMethodAltText = `Accepted payment methods: Visa, Mastercard, American Express ${inUS ? ' Paypal, Diners Club and Discover' : ' and Paypal'}`;
