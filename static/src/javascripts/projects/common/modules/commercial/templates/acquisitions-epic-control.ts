...
const buildFooter = (
    footer: string[]
): string => `<div class="contributions__epic-footer">
        ${footer.map((line) => `<h2>${line}</h2>`).join('')}
    </div>`;

const buildImage = (
    url: string
): string => `<div class="contributions__epic-image">
...
export const acquisitionsEpicControlTemplate = ({
    ...
    return `<div class="contributions__epic ${extraClasses}" data-component="${componentName}" data-link-name="epic">
        <div class="${wrapperClass}">
            ...
            ${
                canShowContributionsReminderFeature() && reminderFields
                    ? acquisitionsEpicReminderTemplate(reminderFields)
                    : ''
            }
        </div>
    </div>`;
