...
const buildImage = (url: string): string => `<div class="contributions__epic-image">
...
export const acquisitionsEpicControlTemplate = ({
  ...
  const extraClasses = (backgroundImageUrl ? epicClassNames.concat(['contributions__epic--with-image']) : epicClassNames).join(' ');
  ...
  return `<div class="contributions__epic ${extraClasses}" data-component="${componentName}" data-link-name="epic">
        <div class="${wrapperClass}">
            <div>
                ...
                ${appendToLastElement(paragraphs, highlightedText ? ` <strong><span class="contributions__highlight">${highlightedText}</span></strong>` : '').map(paragraph => `<p>${paragraph}</p>`).join('')}
            </div>
