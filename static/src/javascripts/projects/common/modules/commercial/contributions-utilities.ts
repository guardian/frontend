...
} from 'common/modules/commercial/epic-articles-viewed-opt-out';
import {
...
const getTargets = (insertAtSelector: string): HTMLElement[] => {
...
const copyHasVariables = (text: string | null | undefined): boolean =>
    !!text && text.includes('%%');
...
const setupOphanView = (
    ...
    products: readonly OphanProduct[],
    showTicker = false,
    tickerSettings: TickerSettings | null | undefined
) => {
...
const makeEpicABTestVariant = (
    ...
    return {
        ...
        secondaryCta:
            initVariant.secondaryCta &&
            addTrackingAndCountryGroupToCta(initVariant.secondaryCta),
...
const buildEpicCopy = (
    ...
    const countryName: string | null | undefined = testHasCountryName
        ? getCountryName(geolocation)
        : undefined;

    const localCurrencySymbol = getLocalCurrencySymbol(geolocation);
    const replaceCurrencySymbol = (s: string): string =>
        s.replace(/%%CURRENCY_SYMBOL%%/g, localCurrencySymbol);
    ...
    return {
        ...
        variants: test.variants.map((variant) => ({
            ...
            tickerSettings: variant.tickerSettings
                ? parseTickerSettings(variant.tickerSettings)
                : null,
            showReminderFields: variant.showReminderFields,
...
export const getConfiguredLiveblogEpicTests = (): Promise<
    readonly EpicABTest[]
> =>
    getLiveblogEpicTestData()
        ...
        .catch((err: Error) => {
            reportError(
                new Error(
                    `Error getting multiple configured liveblog epic tests. ${err.message}. Stack: ${err.stack}`
                ),
                {
                    feature: 'epic',
                },
...
> =>
    getGoogleDoc(bannerMultipleTestsGoogleDocUrl)
        ...
        .catch((err: Error) => {
            reportError(
                new Error(
                    `Error getting multiple engagement banner tests from Google Docs. ${err.message}. Stack: ${err.stack}`
                ),
                {
