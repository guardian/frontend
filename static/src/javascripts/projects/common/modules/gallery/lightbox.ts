...
class GalleryLightbox {
  ...
  constructor(): void {
    // CONFIG
    this.showEndslate = getBreakpoint() !== 'mobile' && config.get('page.section') !== 'childrens-books-site' && config.get('page.contentType') === 'Gallery';
    ...
    this.toggleInfo = (e): void => {
      const infoPanelClick = bonzo(e.target).hasClass('js-gallery-lightbox-info') || $.ancestor(e.target, 'js-gallery-lightbox-info');
    ...
    this.fsm = new FiniteStateMachine({
      ...
      onChangeState(oldState: string, newState: string): void {
        this.$lightboxEl.removeClass(`gallery-lightbox--${oldState}`).addClass(`gallery-lightbox--${newState}`);
      },
      context: this,
      states: this.states
    });
  ...
  generateImgHTML(img: Object, i: number): string {
    ...
    const shareItems: Array<{
      ...
      url: `https://twitter.com/intent/tweet?text=${encodeURIComponent(config.get('page.webTitle'))}&url=${encodeURIComponent(`${blockShortUrl}/stw#img-${i}`)}`
    }, {
      text: 'Pinterest',
      css: 'pinterest',
      icon: inlineSvg('sharePinterest', ['icon']),
      url: encodeURI(`http://www.pinterest.com/pin/create/button/?description=${config.get('page.webTitle')}&url=${blockShortUrl}&media=${urlPrefix}${img.src}`)
    }];
    ...
    return template(blockSharingTpl.replace(/^\s+|\s+$/gm, ''))({
      ...
      shareButtons: shareItems.map(s => template(shareButtonTpl)(s)).join('')
    });
  }
  ...
  initSwipe(): void {
    ...
    bean.on(this.$swipeContainer[0], 'touchstart', (e: TouchEvent): void => {
  ...
  static loadNextOrPrevious(currentImageId: string, direction: string): Promise<Object> {
    const pathPrefix = direction === 'forwards' ? 'getnext' : 'getprev';
    const seriesTag = config.get('page.nonKeywordTagIds', '').split(',').filter(tag => tag.includes('series'))[0];
    const fetchUrl = `/${pathPrefix}/${seriesTag}/${currentImageId}`;
    return fetch(fetchUrl).then(response => response.json()).then(json => // filter out non-lightbox items in the series
  ...
  fetchSurroundingJson(galleryJson: GalleryJson): Promise<Object> {
    ...
    if (config.get('page.contentType') === 'ImageContent' && galleryJson.images.length < 2) {
      ...
      return Promise.all([GalleryLightbox.loadNextOrPrevious(currentId, 'forwards'), GalleryLightbox.loadNextOrPrevious(currentId, 'backwards')]).then(([nextResult, previousResult]) => ({
        ...
        const allImages = previous.reverse().concat(galleryJson.images, next);
  ...
  loadHtml(json: GalleryJson): void {
    ...
    const imagesHtml = json.images.map((img, i) => this.generateImgHTML(img, i + 1)).join('');
  ...
  states = {
    ...
    image: {
      enter(): void {
        ...
        this.translateContent(this.index, 0, this.useSwipe && isBreakpoint({
          max: 'tablet'
        }) ? 100 : 0);
  ...
  loadCssPromise.then(() => {
    ...
    if (images && images.images.length > 0) {
      ...
      bean.on(document.body, 'click', '.js-gallerythumbs', (e: Event) => {
        ...
        const galleryIndex = Number.isNaN(parsedGalleryIndex) ? 1 : parsedGalleryIndex; // 1-based index

        lightbox.fetchSurroundingJson(images).then(allImages => {
          const startIndex = lightbox.startIndex ? lightbox.startIndex : galleryIndex;
