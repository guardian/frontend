...
import { trackSamePageLinkClick as trackSamePageLinkClickGoogle, trackNonClickInteraction as trackNonClickInteractionGoogle, trackExternalLinkClick as trackExternalLinkClickGoogle, trackSponsorLogoLinkClick as trackSponsorLogoLinkClickGoogle } from "common/modules/analytics/google";
...
const isSponsorLogoLinkClick = (target: Object): boolean => target.hasAttribute('data-sponsor');
...
const addHandlers = (): void => {
  ...
  mediator.on('module:clickstream:click', spec => {
    ...
    catchErrorsWithContext([['c-analytics', () => {
      trackClick(spec);
    }]]);
  });
};
