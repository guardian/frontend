...
const renderBlock = (state: Object): Promise<void> => fastdom.mutate(() => {
  state.$el.css('height', '');
}).then(() => {
  if (state.isMobile) {
    return fastdom.measure(() => state.$el.height()).then(height => fastdom.mutate(() => {
...
const getState = (): Promise<{
  ...
  const elements = Array.from(document.getElementsByClassName('js-is-fixed-height'));
