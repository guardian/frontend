...
const impressions = cookieVal && !Number.isNaN(cookieVal) ? parseInt(cookieVal, 10) : 0;

const tmp = '<img src="<%=LOGO%>" class="app__logo" alt="Guardian App logo" /><div class="app__cta"><h4 class="app__heading">The Guardian app</h4>' + '<p class="app__copy">Instant alerts. Offline reading.<br/>Tailored to you.</p>' + '<p class="app__copy"><strong>FREE</strong> – <%=STORE%></p></div><a href="<%=LINK%>" class="app__link">View</a>';
...
const canUseSmartBanner = (): boolean => config.get('switches.smartAppBanner') && getUserAgent.browser === 'Safari' && isIOS();

const canShow = (): Promise<boolean> => new Promise(resolve => {
  const result = !canUseSmartBanner() && isDevice() && validImpressionCount() && !hasUserAcknowledgedBanner(messageCode);
  resolve(result);
});

const show = (): Promise<boolean> => loadCssPromise.then(() => {
