...
const init = (): void => {
  bean.on(document.body, 'click', `.${buttonCN}`, (e: Event) => {
    const container = (e.currentTarget as any).closest(containerSelector);

    if (container) {
      fastdom.measure(() => {
        const documentElement: HTMLElement | null | undefined = document.documentElement;
        const content = container.querySelector(`.${contentCN}`);
        const isActive: boolean = container.classList.contains('dropdown--active');
        const isAnimated: boolean = container.classList.contains('dropdown--animated') && (documentElement !== null && documentElement !== undefined && documentElement.classList.contains('disable-flashing-elements') === false);
        const contentEstimatedHeight = content.offsetHeight !== undefined && content.offsetHeight < window.innerHeight ? content.offsetHeight : window.innerHeight;
        ...
        */
        ...
        if (isAnimated && 'ontransitionend' in window) {
          fastdom.mutate(() => {
            ...
            content.style.height = isActive ? `${contentEstimatedHeight}px` : 0;
          }).then(() => {
            requestAnimationFrame(() => {
              content.style.height = isActive ? 0 : `${contentEstimatedHeight}px`;
