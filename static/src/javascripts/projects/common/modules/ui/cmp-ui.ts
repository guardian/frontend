...
export const addPrivacySettingsLink = (): void => {
  ...
  const privacyLink: HTMLElement | null | undefined = document.querySelector('a[data-link-name=privacy]');

  if (privacyLink) {
    const privacyLinkListItem: Element | null | undefined = privacyLink.parentElement;
    ...
    if (privacyLinkListItem) {
      ...
      newPrivacyLink.innerText = getPrivacyFramework().ccpa ? 'California resident – Do Not Sell' : 'Privacy settings';

      const newPrivacyLinkListItem: Element = privacyLinkListItem.cloneNode(false);

      newPrivacyLinkListItem.appendChild(newPrivacyLink);

      privacyLinkListItem.insertAdjacentElement('beforebegin', newPrivacyLinkListItem);
