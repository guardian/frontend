...
const getClickSpec = (spec: Spec, forceValid: boolean | null | undefined): Spec | boolean => {
  if (!spec.el) {
    return false;
  }
  ...
  spec.customEventProperties = Object.assign(customEventProperties, spec.customEventProperties);
  ...
  if (!spec.validTarget) {
    ...
    if (spec.validTarget) {
      ...
      spec.target = el;
      spec.samePage = (href && href.startsWith('#')) || elName === 'button' || !!el.hasAttribute('data-is-ajax');
