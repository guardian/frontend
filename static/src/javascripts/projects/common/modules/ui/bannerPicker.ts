...
const init = (banners: Array<Banner>): Promise<void> => {
  const results: Array<"pending" | boolean> = new Array(banners.length).fill('pending', 0);
  ...
  const getSuccessfulBannerIndex = (): number => {
    ...
    const pendingHigherPriorityCheckIndex = results.slice(0, firstCheckPassedIndex).findIndex(item => item === 'pending');
