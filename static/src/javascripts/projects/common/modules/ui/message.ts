...
class Message {
  ...
  show(message: string): boolean {
    ...
    if (this.position === 'top') {
      const bodyEl: HTMLElement | null | undefined = document.body;
      if (!bodyEl) throw new Error('Missing <body>');
      const bodyElFirstChild: Node | null | undefined = bodyEl.childNodes[0];
    ...
    if (this.cssModifierClass) {
      this.$siteMessageContainer.addClass(`site-message--${this.cssModifierClass}`);
    }
    ...
    if (this.siteMessageComponentName) {
      this.$siteMessageContainer.attr('data-component', this.siteMessageComponentName);
      if (this.trackDisplay) {
        begin(this.siteMessageComponentName);
      }
    }
    if (this.siteMessageLinkName) {
      this.$siteMessageContainer.attr('data-link-name', this.siteMessageLinkName);
    }
    if (this.siteMessageCloseBtn) {
      $('.site-message__close-btn', '.js-site-message').attr('data-link-name', this.siteMessageCloseBtn);
    }

    this.$siteMessageContainer.addClass(`site-message--${this.type}`).addClass(`site-message--${this.id}`);
  ...
  updateMessageOnWidth(): void {
    ...
    if (this.widthBasedMessage && this.$siteMessage && this.$siteMessage.length) {
      this.updateMessageFromData(isBreakpoint({
        max: 'tablet'
