        ...
        fetch('https://sndrs.github.io/nfn/nfn.json')
            ...
            .then(({ dictionary, transforms }) => {
                Object.keys(dictionary).forEach((key) => {
                    dictionary[key.toLowerCase()] = dictionary[
                        key
                    ].toLowerCase();
                });

                const dictionaryRegex = new RegExp(
                    `(\\W)(${Object.keys(dictionary).join(
                        '|'
                    )})(s|ed|ing|)(\\W)`,
                    'g'
                );

                const transformRegexs = Object.keys(transforms).reduce(
                    (regexs, transform) => ({
                ...
                const normalise = (s: string) => {
                    transformers.forEach((transformer) => {
                        ...
                        s = s.replace(
                            transformRegexs[transformer],
                            transformer
                        );
                    });
                    return s.replace(
                        dictionaryRegex,
                        (match, p1, p2, p3, p4) => p1 + dictionary[p2] + p3 + p4
                    );
                };
