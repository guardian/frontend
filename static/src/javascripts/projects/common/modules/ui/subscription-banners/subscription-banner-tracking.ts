...
import { submitViewEvent, submitClickEvent, addTrackingCodesToUrl } from "common/modules/commercial/acquisitions-ophan";
...
const createTracking = (region: ReaderRevenueRegion, defaultTracking: BannerTracking) => {
  ...
  return isGuardianWeeklyRegion ? { ...defaultTracking, ...guardianWeeklyTracking } : defaultTracking;
};
  ...
  const defaultTracking = {
    ...
    trackBannerClick: button => {
      submitClickEvent({
        component: {
          componentType: COMPONENT_TYPE,
          id: button.id === 'js-site-message--subscription-banner__cta' ? CLICK_EVENT_CTA : CLICK_EVENT_SIGN_IN
        }
      });
    },
    trackCloseButtons: button => {
      submitClickEvent({
        component: {
          componentType: COMPONENT_TYPE,
          id: button && button.id === 'js-site-message--subscription-banner__close-button' ? CLICK_EVENT_CLOSE_BUTTON : CLICK_EVENT_CLOSE_NOT_NOW
        }
      });
    }
  };
