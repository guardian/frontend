...
describe('Subscription Banner', () => {
  ...
  it('Should append a Subscription Banner to the document', () => {
    const show = createBannerShow(mockTracking, subscriptionBannerTemplate, false);
    show();

    const banner = subscriptionBannerTemplate(mockTracking.subscriptionUrl, mockTracking.signInUrl, false);

    const appendedBanner = document.body ? document.body.innerHTML : 'Banner Not Appended';
  ...
  describe('Button Clicks', () => {
    ...
    it('Should remove banner when close button is clicked', () => {
      const closeButton: HTMLElement | null | undefined = document.querySelector(bannerSelector('close-button'));
    ...
    it('should remove the banner when the "not now" button is clicked', () => {
      const notNowButton: HTMLElement | null | undefined = document.querySelector(bannerSelector('cta-dismiss'));
    ...
    it('Should have "subscriptionUrl" as the cta href', () => {
      ...
      const ctaButtonHref = ctaButton instanceof HTMLAnchorElement && ctaButton.href;

      expect(ctaButtonHref).toEqual(`http://testurl.theguardian.com/${mockTracking.subscriptionUrl}`);
    });

    it('Should have "signInUrl" as the signInLink href', () => {
      const signInLink = document.querySelector(bannerSelector('sign-in'));

      const signInLinkHref = signInLink instanceof HTMLAnchorElement && signInLink.href;

      expect(signInLinkHref).toEqual(`http://testurl.theguardian.com/${mockTracking.signInUrl}`);
    });
  });
  ...
  describe('Banner Tracking', () => {
    it('Should call trackCloseButtons when close button is clicked', () => {
      ...
      const show = createBannerShow(mockTrackingSUT, subscriptionBannerTemplate, false);
      show();
    ...
    it('should hide sign-in link if user is signed in', () => {
      ...
      const show = createBannerShow(mockTrackingSUT, subscriptionBannerTemplate, isUserSignedIn);
      show();

      const signInContainer = document.querySelector('.site-message--subscription-banner__sign-in');

      const hasSignedInClass: boolean | null | undefined = signInContainer && signInContainer.classList.contains('site-message--subscription-banner__sign-in--already-signed-in');
