...
import { pageShouldHideReaderRevenue, getReaderRevenueRegion } from "common/modules/commercial/contributions-utilities";
...
// types
import { ReaderRevenueRegion } from "common/modules/commercial/contributions-utilities";
...
const subscriptionBannerSwitchIsOn: boolean = config.get('switches.subscriptionBanner') && !remoteSubscriptionsBannerSwitchIsOn;
...
const selectorName = (bannerName: string) => (actionId: string | null | undefined) => `#js-site-message--${bannerName}${actionId ? `__${actionId}` : ''}`;
...
const hasAcknowledgedBanner = (region: ReaderRevenueRegion): Promise<boolean> => fetchJson(`/reader-revenue/subscriptions-banner-deploy-log/${region}`, {
  mode: 'cors'
}).then(resp => hasAcknowledged(resp.time)).catch(err => {
  reportError(new Error(`Unable to get subscriptions banner deploy log: ${err}`), { feature: 'subscriptions-banner' }, false);
  return true;
});

const pageIsIdentity = (): boolean => config.get('page.contentType') === 'Identity' || config.get('page.section') === 'identity';
...
const bindClickHandlers = (bannerName: string, trackBannerClick: (button: any) => void, trackBannerCloseButtons: (button: any) => void) => {
  const closeActions = (bannerCloseKey): void => {
    const closedAt = (lastClosedAtKey: string) => userPrefs.set(lastClosedAtKey, new Date().toISOString());
...
const createBannerShow = (tracking: BannerTracking, bannerTemplate: (subscriptionUrl: string, signInUrl: string, userLoggedIn: boolean) => string, userLoggedIn: boolean) => async (): Promise<boolean> => {
  tracking.trackBannerView();
  tracking.gaTracking();

  if (document.body) {
    document.body.insertAdjacentHTML('beforeend', bannerTemplate(tracking.subscriptionUrl, tracking.signInUrl, userLoggedIn));
  }

  bindClickHandlers(MESSAGE_CODE, tracking.trackBannerClick, tracking.trackCloseButtons);

  return Promise.resolve(true);
};

const chooseBanner = (region: ReaderRevenueRegion) => (region === 'australia' || region === 'rest-of-world') ? gwBannerTemplate : subscriptionBannerTemplate;

const show = createBannerShow(bannerTracking(currentRegion), chooseBanner(currentRegion), isUserLoggedIn());

const canShow: () => Promise<boolean> = async () => {
  const hasAcknowledgedSinceLastRedeploy = await hasAcknowledgedBanner(currentRegion);

  const can = Promise.resolve(subscriptionBannerSwitchIsOn && pageviews >= 4 && !hasAcknowledgedSinceLastRedeploy && !shouldHideSupportMessaging() && !pageShouldHideReaderRevenue() && !pageIsIdentity());
