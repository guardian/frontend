...
const dayOfWeek = (day): string => ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][day];

const monthAbbr = (month): string => ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][month];
...
const withTime = (date): string => ` ${date.getHours()}:${pad(date.getMinutes())}`;

const makeRelativeDate = (epoch: number, opts: RelativeDateOptions = {}): false | string => {
  let minutes;
  let hours;
  let days;
  let delta = 0;
