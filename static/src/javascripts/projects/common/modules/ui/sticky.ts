...
class Sticky {
  ...
  init(): void {
    ...
    fastdom.read(() => {
      this.offsetFromParent = this.element.getBoundingClientRect().top - parentElement.getBoundingClientRect().top;
    }, this);
  ...
  updatePosition(): void {
    ...
    if (parentRect.top + this.offsetFromParent > 0) {
      ...
      stick = true;
      const top = this.opts.containInParent && parentRect.bottom <= elementRect.height ? Math.floor(parentRect.bottom - elementHeight - this.opts.top) : this.opts.top;
      css = {
        top: `${top}px`
      };
      message = 'fixed';
