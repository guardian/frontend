...
const EVENTS = ['preroll:request', 'preroll:ready', 'preroll:play', 'preroll:end', 'content:ready', 'content:play', 'content:end'];
const gaTracker = config.get('googleAnalytics.trackers.editorial');

const bindCustomMediaEvents = (eventsMap: Object, player: Object, mediaId: string, mediaType: string, isPreroll: boolean): void => {
...
const addContentEvents = (player: Object, mediaId: any, mediaType: any): void => {
  const eventsMap = {
    ready: 'ready',
    play: 'play',
    passed25: 'watched25',
...
const bindGoogleAnalyticsEvents = (player: Object, canonicalUrl: string) => {
  ...
  Object.keys(events).map(eventName => `media:${eventName}`).forEach(playerEvent => {
    player.on(playerEvent, (_, mediaEvent) => {
      window.ga(`${gaTracker}.send`, 'event', buildGoogleAnalyticsEvent(mediaEvent, events, canonicalUrl, 'guardian-videojs', getGoogleAnalyticsEventAction, mediaEvent.mediaId));
    });
  });
};
...
const shouldAutoPlay = player => isDesktop && !isRevisit(config.get('page.pageId')) && player.guAutoplay;

const constructEventName = (eventName: string, player: Object): string => `${getMediaType(player)}:${eventName}`;

const ophanRecord = (id: string | null | undefined, event: Object, player: Object) => {
  if (!id) return;
