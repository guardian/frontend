...
const getEndpoint = (mediaType: string): string => {
  ...
  return `/${config.get('page.isPodcast') ? 'podcast' : mediaType}/most-viewed.json`;
};

const createComponent = (el: HTMLElement, endpoint: string, manipulationType: string, page?: string | null | undefined): Promise<void> => {
...
const initEvents = (el: HTMLElement, manipulationType: string, endpoint: string): void => {
