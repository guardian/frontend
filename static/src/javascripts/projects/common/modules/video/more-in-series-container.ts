...
const moreInSeriesContainerInit = (el: HTMLElement, mediaType: string, section: string, shortUrl: string, series?: string): void => {
  const component = new Component();
  const seriesStr = series ? `/${series}` : '';
  // exclude professional network content from video pages
  const excludeProfContent = mediaType === 'video' ? '&exclude-tag=guardian-professional/guardian-professional' : '';
