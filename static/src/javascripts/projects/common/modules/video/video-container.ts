...
const updateYouTubeVideo = (currentItem: Element | null | undefined): void => {
  if (currentItem != null) {
  ...
  INIT: function init(previousState: State): State {
    const makeYouTubeNonPlayableAtSmallBreakpoint = state => {
      if (isBreakpoint({
        ...
        const youTubeIframes = Array.from(state.container.querySelectorAll('.youtube-media-atom iframe'));
        youTubeIframes.forEach(el => {
          el.remove();
        });
        const overlayLinks = Array.from(state.container.querySelectorAll('.video-container-overlay-link'));
        ...
        const atomWrapper = Array.from(state.container.querySelectorAll('.youtube-media-atom'));
    ...
    fastdom.read(() => {
      ...
      $('.js-video-playlist-image', previousState.container).each(el => {
        const inview = elementInView(el, $('.js-video-playlist-inner', previousState.container).get(0), {
...
const update = (state: State, container: Element): Promise<number> => {
  ...
  return fastdom.write(() => {
    const activeEl = container.querySelector('.video-playlist__item--active');
    ...
    if (newActive != null) {
      ...
      $('.video-overlay .fc-item__link', newActive).removeAttr('aria-hidden');
    }
    ...
    if (activePlaylistItem != null) activePlaylistItem.classList.add('video-playlist__item--active');

    const playlistInner = container.querySelector('.js-video-playlist-inner');
    if (playlistInner != null) playlistInner.setAttribute('style', `-webkit-transform: translate(${translateWidth}px);` + `transform: translate(${translateWidth}px);`);
  });
};
...
const setupDispatches = (dispatch: (a: Action) => void, container: Element): void => {
...
const reducer = (previousState: State, action: Action): State => reducers[action.type] ? reducers[action.type](previousState) : previousState;

const createStore = (storeReducer: (s: State, a: Action) => State, initialState: State) => {
