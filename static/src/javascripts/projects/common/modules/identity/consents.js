...
const buildConsentUpdatePayload = (fields = new NodeList()) => {
	const consent = {};
...
const checkAllOptOuts = () => toggleInputsWithSelector(optOutClassName, true);

const uncheckAllOptIns = () => toggleInputsWithSelector(optInClassName, false);
...
const showUnsubscribeConfirmation = () => {
	...
	const updateVisibilityAndShowMessage = (elem) =>
		fastdom.mutate(() => {
	...
	return fetchButton().then((button) =>
		updateVisibilityAndShowMessage(button),
	);
};
