...
import { getUserFromApi, init, decodeBase64, getUserFromCookie, reset, getUserOrSignIn, shouldAutoSigninInUser } from "common/modules/identity/api.js";
...
describe('Identity API', () => {
  beforeEach(() => {
    ...
    getCookieStub.mockImplementation(() => 'WyIyMzEwOTU5IiwiamdvcnJpZUBnbWFpbC5jb20iLCJBbSVDMyVBOWxpZSBKJUMzJUI0c2UiLCI1MzQiLDEzODI5NTMwMzE1OTEsMV0' + '.MC0CFBsFwIEITO91EGONK4puyO2ZgGQcAhUAqRa7PVDCoAjrbnJNYYvMFec4fAY');
  ...
  it('gets user from the idapi', done => {
    ...
    ajax.mockImplementationOnce(() => Promise.resolve({
      status: 'ok',
      user: expectedUser
    }));
  ...
  it('should redirect to sign in when user is not signed in', () => {
    ...
    expect(window.location.href).toBe(`${config.page.idUrl}/signin?returnUrl=${encodeURIComponent(returnUrl)}&componentEventParams=componentType%3Didentityauthentication%26componentId%3Demail_sign_in_banner`);
  ...
  it('should attempt to autosigin an user who is not currently signed in and has not previously signed out', () => {
    getCookieStub.mockImplementationOnce(() => null) // GU_U
