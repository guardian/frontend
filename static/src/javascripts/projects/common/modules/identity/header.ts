...
const showAccountMenu = (buttonEl: HTMLElement, menuEl: HTMLElement): Promise<void> => fastdom.write(() => {
  menuEl.classList.add('is-active');
  menuEl.setAttribute('aria-hidden', 'false');
  buttonEl.setAttribute('aria-expanded', 'true');
});

const hideAccountMenu = (buttonEl: HTMLElement, menuEl: HTMLElement): Promise<void> => fastdom.write(() => {
...
const bindNavToggle = (buttonEl: HTMLElement): void => {
  buttonEl.addEventListener('click', (ev: Event) => {
    ...
    fastdom.read(() => document.getElementById(menuElSelector)).then((menuEl: HTMLElement) => {
