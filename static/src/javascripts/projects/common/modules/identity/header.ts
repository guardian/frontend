...
const showAccountMenu = (buttonEl: HTMLElement, menuEl: HTMLElement): Promise<void> => fastdom.mutate(() => {
  menuEl.classList.add('is-active');
  menuEl.setAttribute('aria-hidden', 'false');
  buttonEl.setAttribute('aria-expanded', 'true');
});

const hideAccountMenu = (buttonEl: HTMLElement, menuEl: HTMLElement): Promise<void> => fastdom.mutate(() => {
...
const bindNavToggle = (buttonEl: HTMLElement): void => {
  buttonEl.addEventListener('click', (ev: Event) => {
    const menuElSelector: string | null | undefined = buttonEl.getAttribute('aria-controls');
    if (!menuElSelector) throw new Error(ERR_UNDEFINED_MENU);

    ev.preventDefault();

    fastdom.measure(() => document.getElementById(menuElSelector)).then((menuEl: HTMLElement) => {
