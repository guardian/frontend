...
const isInExperiment = (): boolean => config.get('switches.idEmailSignInUpsell', false);
...
const canShow: () => Promise<boolean> = () => {
  const can = Promise.resolve(!hasUserAcknowledgedBanner(messageCode) && isInExperiment() && isSecondEmailPageview());
  return can;
};
...
const show: () => Promise<boolean> = () => {
  ...
  if (!campaign) {
    throw new Error(`email-sign-in missing campaign for utm (${store.utm})`);
  }
  userPrefs.remove(userPrefsStoreKey);
  ...
  return Promise.resolve(message.show(makeTemplate({
    campaign,
    signInLink,
    emailPrefsLink
  })));
