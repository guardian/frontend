...
Id.getUserFromCookie = () => {
    if (userFromCookieCache === null) {
        const cookieData = cookies.getCookie(Id.cookieName), userData = cookieData ? JSON.parse(Id.decodeBase64(cookieData.split('.')[0])) : null;
        if (userData) {
 ...
 * @return {string}
 */
Id.getCookie = () => cookies.getCookie(Id.cookieName);
 ...
 * @return {string}
 */
Id.decodeBase64 = str => decodeURIComponent(escape(window.atob(str.replace(/-/g, '+').replace(/_/g, '/').replace(/,/g, '='))));

/**
 * @return {Boolean}
 */
Id.hasUserSignedOutInTheLast24Hours = () => {
    const cookieData = cookies.getCookie(Id.signOutCookieName);
...
Id.shouldAutoSigninInUser = function() {
    const signedInUser = !!cookies.getCookie(Id.cookieName), checkFacebook = !!storage.local.get(Id.fbCheckKey);
