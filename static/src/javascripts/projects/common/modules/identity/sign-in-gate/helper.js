...
const retrieveDismissedCount = (variant, name, componentName) => {
	try {
...
// Stubbed due to legacy AB test framework removal - full cleanup in follow-up ticket
export const isInTest = () => false;

// Stubbed due to legacy AB test framework removal - full cleanup in follow-up ticket
export const getTestforMultiTest = () => undefined;
...
export const setGatePageTargeting = (isGateDismissed, canShowCheck) => {
...
export const isInvalidTag = (include = []) => {
	...
	return invalidTags
		.filter((el) => !include.includes(el))
		.some((tag) =>
			String(config.get(`page.keywordIds`) ?? '')
				.split(',')
				.includes(tag),
		);
};
