...
import type { CurrentABTest, DismissalWindow, GateStatus } from './types';
...
const localStorageDismissedCountKey = (variant: string, name: string): string =>
    `gate-dismissed-count-${name}-${variant}`;
...
export const isInTest: (arg0: ABTest) => boolean = (test) =>
...
export const getTestforMultiTest: (arg0: ABTest[]) => ABTest = (tests) =>
...
export const incrementUserDismissedGateCount = (
    ...
    try {
        const prefs = userPrefs.get(componentName) || {};
        prefs[localStorageDismissedCountKey(variant, name)] =
            retrieveDismissedCount(variant, name, componentName) + 1;
        userPrefs.set(componentName, prefs);
    } catch (error) {
...
export const isNPageOrHigherPageView = (n = 2): boolean => {
    ...
    const dailyArticleCount =
        storage.local.get('gu.history.dailyArticleCount') || [];
...
export const isInvalidArticleType = (include: string[] = []): boolean => {
...
// add to the include parameter array if there are specific types that should be included/overridden
export const isInvalidSection = (include: string[] = []): boolean => {
...
export const isInvalidTag = (include: string[] = []): boolean => {
    const invalidTags = ['info/newsletter-sign-up'];

    return invalidTags
        .filter((el) => !include.includes(el))
        .some((tag) =>
            config.get(`page.keywordIds`, '').split(',').includes(tag)
        );
};
