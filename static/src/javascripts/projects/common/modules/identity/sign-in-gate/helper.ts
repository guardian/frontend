...
import { getSynchronousTestsToRun, isInABTestSynchronous } from "common/modules/experiments/ab";
import { isUserLoggedIn } from "common/modules/identity/api";
import { cmp } from "@guardian/consent-management-platform";
import { submitClickEventTracking } from "./component-event-tracking";
import { CurrentABTest, GateStatus, DismissalWindow } from "./types";

// Helper for setGatePageTargeting function
const setGoogleTargeting = (canShow: GateStatus): void => {
  if (window.googletag) {
    window.googletag.cmd.push(() => {
      window.googletag.pubads().setTargeting('gate', canShow.toString().slice(0, 1)); // must be a short string so we slice to "t"/"f"/"d"/"s"
    });
  }
};
...
const retrieveDismissedCount = (variant: string, name: string, componentName: string): number => {
  try {
...
export const isInTest: (arg0: ABTest) => boolean = test => isInABTestSynchronous(test);
...
export const getTestforMultiTest: (arg0: Array<ABTest>) => ABTest = tests => tests.reduce((acc, test) => {
  const checkTest = getSynchronousTestsToRun().find(t => t.id === test.id);
  if (checkTest) return checkTest;
  return acc;
}, undefined);

// get the current variant id the user is in
export const getVariant: (arg0: ABTest) => string = test => {
...
export const setUserDismissedGate: (arg0: {
  name?: string;
  variant: string;
...
// name is optional, but can be used to differentiate between multiple sign in gate tests
export const unsetUserDismissedGate: (arg0: {
  componentName: string;
}) => void = ({
  componentName
}) => {
...
// name is optional, but can be used to differentiate between multiple sign in gate tests
export const hasUserDismissedGate: (arg0: {
  name?: string;
  variant: string;
  componentName: string;
}) => boolean = ({
...
export const hasUserDismissedGateInWindow: (arg0: {
  ...
  const dismissalTZ = Date.parse(prefs[`${name ? `${name}-` : ''}${variant}`]);
...
export const hasUserDismissedGateMoreThanCount = (variant: string, name: string, componentName: string, count: number): boolean => retrieveDismissedCount(variant, name, componentName) > count;

// Increment the number of times a user has dismissed this gate variant
export const incrementUserDismissedGateCount = (variant: string, name: string, componentName: string): void => {
  try {
    const prefs = userPrefs.get(componentName) || {};
    prefs[localStorageDismissedCountKey(variant, name)] = retrieveDismissedCount(variant, name, componentName) + 1;
    userPrefs.set(componentName, prefs);
  } catch (error) {// localstorage isn't available so show the gate
  }
};

// Dynamically sets the gate custom parameter for Google ad request page targeting
export const setGatePageTargeting = (isGateDismissed: boolean, canShowCheck: boolean): void => {
  if (isUserLoggedIn()) {
...
export const isInvalidArticleType = (include: Array<string> = []): boolean => {
  ...
  const invalidTypes = ['isColumn', 'isFront', 'isHosted', 'isImmersive', 'isLive', 'isLiveBlog', 'isNumberedList', 'isPaidContent', 'isPhotoEssay', 'isSensitive', 'isSplash', 'isPodcast'];

  return invalidTypes.filter(el => !include.includes(el)).reduce((isArticleInvalid, type) => {
...
export const isInvalidSection = (include: Array<string> = []): boolean => {
  const invalidSections = ['about', 'info', 'membership', 'help', 'guardian-live-australia', 'gnm-archive'];

  return invalidSections.filter(el => !include.includes(el)).reduce((isSectionInvalid, section) => {
...
export const isInvalidTag = (include: Array<String> = []): boolean => {
  ...
  return invalidTags.filter(el => !include.includes(el)).some(tag => config.get(`page.keywordIds`, "").split(',').includes(tag));
};

// html event wrapper using bean
export const addEventHandler: (arg0: {
  element: HTMLElement;
  event: string;
  selector: string;
  handler: Function;
...
export const addClickHandler: (arg0: {
  element: HTMLElement;
  selector: string;
...
export const addCSSOnOpinion: (arg0: {
  element: HTMLElement;
  selector: string;
  css: string;
}) => void = ({
...
export const addOpinionBgColour: (arg0: {
  element: HTMLElement;
  selector: string;
}) => void = ({
  element,
  ...
  if (config.get(`page.tones`) === 'Comment') {
    ...
    if (overlay) {
      overlay.classList.add('signin-gate__first-paragraph-overlay--comment');
    }
  }
};

// change the colour of the text depending the pillar that the user is on
export const addPillarColour: (arg0: {
  element: HTMLElement;
  selector: string;
}) => void = ({
  element,
  ...
  if (paragraphText) {
    switch (config.get(`page.pillar`)) {
      case 'News':
        ...
        break;
      case 'Opinion':
        paragraphText.classList.add(`signin-gate__benefits--text-comment`);
        break;
      case 'Sport':
        paragraphText.classList.add(`signin-gate__benefits--text-sport`);
        break;
      case 'Arts':
        paragraphText.classList.add(`signin-gate__benefits--text-culture`);
        break;
      case 'Lifestyle':
        paragraphText.classList.add(`signin-gate__benefits--text-lifestyle`);
        break;
      default:
        break;
...
export const addOverlayVariantCSS: (arg0: {
  element: HTMLElement;
  selector: string;
}) => void = ({
  element,
...
export const showGate: (arg0: {
  ...
  const articleBody = document.querySelector('.js-article__body') || document.querySelector('.article-body-commercial-selector');
  ...
  if (articleBody) {
    ...
    if (articleBody.children.length) {
      ...
      if (articleBody.children[0].clientHeight < 125 && articleBody.children.length > 1) {
        ...
        if (pIndexes.length > 1) {
          shadowOverlay.appendChild(articleBody.children[pIndexes[1]].cloneNode(true));
        }
      }
