...
const bindAccountCreation = (el): void => {
  ...
  fastdom.mutate(() => {
    render(<AccountCreationCompleteConsentsFlow csrfToken={el.dataset.csrf} accountToken={el.dataset.accountToken} email={el.dataset.email} />, el);
  });
};
...
const bindBlockList = (el): void => {
  fastdom.measure(() => getPrefill(el)).then(prefill => fastdom.mutate(() => {
    render(<StatefulConfirmEmailPage csrfToken={prefill.csrfToken} accountToken={prefill.accountToken} email={prefill.email} hasPassword={prefill.hasPassword} hasSocialLinks={prefill.hasSocialLinks} />, el);
  }));
};

const enhanceUpsell = (): void => {
  loadEnhancers([['.js-identity-upsell-account-creation', bindAccountCreation], ['.js-identity-block-list', bindBlockList]]);
};
