...
import { isUserLoggedIn, sendValidationEmail } from "common/modules/identity/api";

const init = (): void => {
  fastdom.measure(() => document.getElementsByClassName('js-id-send-validation-email')).then(elems => {
    if (elems.length) {
          ...
          sendValidationEmail().then(resp => {
            if (resp.status === 'error') {
              ...
              if (resendButtonParent) {
                ...
                sentMsgEl.innerText = 'Sent. Please check your email and follow the link.';

                fastdom.mutate(() => {
                  resendButtonParent.replaceChild(sentMsgEl, resendButton);
                });
              }
            }
          }, () => {
