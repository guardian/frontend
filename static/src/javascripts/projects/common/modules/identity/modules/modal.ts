...
const bindCloserOnce = (modalEl: HTMLElement): Promise<void[]> => fastdom.measure(() => Array.from(modalEl.querySelectorAll('.js-identity-modal__closer'))).then(buttonEls => buttonEls.filter(buttonEl => !buttonEl.dataset.closeIsBound).map(buttonEl => fastdom.mutate(() => {
...
const getModal = (name: string): Promise<HTMLElement> => fastdom.measure(() => {
...
const getContents = (name: string): Promise<HTMLElement> => getModal(name).then(modalEl => fastdom.measure(() => {
...
const show = (name: string): Promise<void> => getModal(name).then(modalEl => fastdom.measure(() => {
  modalEl.classList.add('identity-modal--active');
}));

const hide = (name: string): Promise<void> => getModal(name).then(modalEl => fastdom.measure(() => {
  modalEl.classList.remove('identity-modal--active');
}));
