...
const renderError = (error: IdentityRenderableError): Promise<void> => fastdom.measure(() => window.document.querySelector(`.${formErrorHolderClassName}`)).then((errorHolderEl: HTMLElement) => fastdom.mutate(() => {
  const errorEl = document.createElement('div');
  errorEl.setAttribute('role', 'alert');
  errorEl.setAttribute('aria-live', 'polite');
  errorEl.innerHTML = `<p>${error.message}.${error.times > 1 ? ` (${error.times} times)` : ``}</p>`;
...
const renderList = (): Promise<void> => fastdom.measure(() => window.document.querySelector(`.${formErrorHolderClassName}`)).then((errorHolderEl: HTMLElement) => fastdom.mutate(() => {
  while (errorHolderEl.firstChild) {
    errorHolderEl.removeChild(errorHolderEl.firstChild);
  }
})).then(() => Promise.all(errors.map(error => renderError(error)))).then(() => Promise.resolve());
