...
const renderError = (error: IdentityRenderableError): Promise<void> => fastdom.read(() => window.document.querySelector(`.${formErrorHolderClassName}`)).then((errorHolderEl: HTMLElement) => fastdom.write(() => {
...
const renderList = (): Promise<void> => fastdom.read(() => window.document.querySelector(`.${formErrorHolderClassName}`)).then((errorHolderEl: HTMLElement) => fastdom.write(() => {
