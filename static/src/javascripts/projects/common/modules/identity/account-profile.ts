  ...
  if (form) {
    ...
    avatarApi.updateAvatar(formData).then(() => {
      ...
      if (err.status >= 400 && err.status < 500) {
        prependErrorMessage(JSON.parse(err.responseText).message || messages.avatarUploadFailure, avatarForm);
      } else {
...
class AccountProfile {
  ...
  constructor() {
    ...
    if (this.accountProfileForms) {
      ...
      this.bindInputs(this.accountProfileForms.querySelector(classes.accountForm));
      this.bindInputs(this.accountProfileForms.querySelector(classes.publicForm));

      const tabs = document.querySelector(classes.tabsContainer);

      require.ensure([], // webpack needs the require function to be called 'require'
  ...
  handleTabsClick(event: Event) {
    if (event.target instanceof HTMLElement && event.target.nodeName.toLowerCase() === 'a') {
      const eventTarget: HTMLAnchorElement = (event.target as any);
      if (eventTarget.dataset.tabsIgnore) {
        ...
        if (!form.querySelector(classes.formError)) {
          ...
          bean.on(form.querySelector('.js-save-unsaved'), 'click', () => {
            form.submit();
          });
        }
      } else {
   ...
   */
  bindAvatarUpload() {
    const avatarForm = this.accountProfileForms.querySelector(classes.avatarUploadForm);

    if (avatarForm) {
      bean.on(avatarForm, 'submit', event => {
        event.preventDefault();

        avatarUploadByApi(((avatarForm as any) as HTMLFormElement));
      });
    }
  }
  ...
  genUnsavedError() {
    ...
    for (let i = 0; i < this.unsavedFields.length; i += 1) {
      ...
      const label = this.accountProfileForms.querySelector(`[for="${labelId}"]`);
      if (label) {
  ...
  onInputChange(event: Event) {
    const input = ((event.target as any) as HTMLInputElement | HTMLTextAreaElement);
