...
export const decodeBase64 = (str: string): string => decodeURIComponent(escape(window.atob(str.replace(/-/g, '+').replace(/_/g, '/').replace(/,/g, '='))));

export const getUserFromCookie = (): IdentityUser | null | undefined => {
...
export const updateNewsletter = (newsletter: Newsletter): Promise<void> => reqwest({
...
export const getCookie = (): string | null | undefined => getCookieByName(cookieName);
...
export const getUserOrSignIn = (componentId: AuthenticationComponentId, paramUrl: string | null | undefined): Object | null | undefined => {
...
export const sendValidationEmail = (): any => {
  ...
  const returnUrl = getUrlVars().returnUrl ? decodeURIComponent(getUrlVars().returnUrl) : (profileRoot || '') + defaultReturnEndpoint;
...
export const setConsent = (consents: SettableConsent[]): Promise<void> => new Promise((success, error) => {
  reqwest({
...
export const ajaxSignIn = (credentials: PasswordCredential) => {
  ...
  if (window.guardian && window.guardian.ophan && window.guardian.ophan.viewId) {
...
export const getUserData = (): Promise<Response> => fetch(`${idApiRoot || ''}/user/me`, {
  method: 'GET',
  mode: 'cors',
  credentials: 'include'
});