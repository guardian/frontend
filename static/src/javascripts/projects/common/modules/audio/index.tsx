...
class AudioContainer extends Component<Props, State> {
  ...
  render() {
    ...
    const sourceUrl = acastEnabled && isPodcast && this.state.acastConsent && !isAdFreeUser() ? this.props.source.replace('https://', 'https://flex.acast.com/') : this.props.source;

    return <AudioPlayer sourceUrl={sourceUrl} mediaId={this.props.mediaId} duration={this.props.duration} />;
  }
}
...
const supportsCSSGrid = window.CSS && window.CSS.supports && window.CSS.supports('display', 'grid');

const init = (): void => {
  const placeholder: HTMLElement | null | undefined = document.getElementById('audio-component-container');
  ...
  if (placeholder && article) {
    ...
    const pillarClassName = Array.from(article.classList).filter(x => x.includes('pillar'))[0];
    const pillar = getPillar(pillarClassName);

    render(supportsCSSGrid ? <AudioContainer source={source} mediaId={mediaId} downloadUrl={downloadUrl} duration={duration} pillar={pillar} /> : <audio src={source} controls data-media-id={mediaId} ref={el => {
      if (el) registerOphanListeners(el);
    }}>
                    <track src={source} kind="captions" srcLang="en" label="English" />
                    <p>
