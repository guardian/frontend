...
import { leftCol, wide } from "@guardian/dotcom-rendering/packages/pasteup/breakpoints";
...
const AudioGrid = styled('div')({
  ...
  gridTemplateRows: isIOS() || isAndroid() ? '30px 40px 120px' : '30px 40px 120px 40px',
...
const ScrubberButton = styled(Button)(({
  position,
  hovering,
  grabbing
}) => ({
...
export class AudioPlayer extends Component<Props, State> {
  ...
  setAudio = (el: HTMLAudioElement | null | undefined) => {
    if (el) {
      this.audio = el;
    }
  ...
  setGeometry = (el: HTMLElement | null | undefined) => {
    if (el) {
      const css = getComputedStyle(el);
      const rect = el.getBoundingClientRect();
      const waveWidthPx = el.clientWidth - parseInt(css.paddingLeft, 10) - parseInt(css.paddingRight, 10);
      Array.from(el.getElementsByTagName('svg')).slice(0, 1).forEach(svg => {
  ...
  buffer = () => {
    if (this.audio.buffered.length > 0) {
      ...
      this.waveBuffered.setAttribute('width', (buffered / this.state.duration * this.state.waveWidthPx).toString());
    }
  };
  ...
  seek = (e: any) => {
    if (!this.state.grabbing) {
      ...
      const currentTime = currentOffsetPx / this.state.waveWidthPx * this.state.duration;
  ...
  updatePlayerTime = (currTime: number) => {
    ...
    const currentOffsetPx = currTime / this.state.duration * this.state.waveWidthPx;
    this.wave.setAttribute('width', currentOffsetPx.toString());
    this.setState({
  ...
  forward = () => {
    const chosenTime = Math.min(this.state.currentTime + 15, this.state.duration);
  ...
  scrub = (e: any) => {
    if (this.state.grabbing) {
      const currentOffsetPx = Math.min(this.state.waveWidthPx, Math.max(0, e.nativeEvent.screenX - this.state.waveX));
  ...
  render() {
    return <AudioGrid onMouseDown={this.grabbing(true)} onMouseUp={this.grabbing(false)} onMouseMove={this.scrub}>
                {
        /* eslint-disable-next-line jsx-a11y/media-has-caption */
      }
                <audio ref={this.setAudio} data-media-id={this.props.mediaId} preload="none">
                ...
                <WaveAndTrack>
                    <FakeWave ref={this.setGeometry} onClick={this.seek}>
                        <div className="wave-holder" dangerouslySetInnerHTML={{ __html: waveW.markup }} />
                    </FakeWave>
                    <ScrubberButton onMouseEnter={this.hovering(true)} onMouseLeave={this.hovering(false)} hovering={this.state.hovering} grabbing={this.state.grabbing} position={this.state.currentOffsetPx} />
                </WaveAndTrack>
                <Controls>
                    <JumpButton playing={this.state.playing} onClick={this.backward} disabled={!this.state.playing} dangerouslySetInnerHTML={{
          __html: fastBackward.markup
        }} />
                    <PlayButton onClick={this.play} dangerouslySetInnerHTML={{
          __html: this.state.playing ? pauseBtn.markup : playBtn.markup
        }} />
                    <JumpButton playing={this.state.playing} onClick={this.forward} disabled={!this.state.playing}>
                        <span dangerouslySetInnerHTML={{
            __html: fastForward.markup
          }} />
                    </JumpButton>
                </Controls>
                {!(isIOS() || isAndroid()) ? <Volume>
                        <VolumeButton isVolume isActive={!this.state.muted} onClick={this.sound} dangerouslySetInnerHTML={{
          __html: volumeOn.markup
        }} />
                        <VolumeButton isVolume isActive={this.state.muted} onClick={this.mute} dangerouslySetInnerHTML={{
