...
jest.mock('commercial/modules/dfp/track-ad-render', () => Promise.resolve(true));
...
describe('spacefinder', () => {
  test('should test elements correctly', () => {
    ...
    others.forEach(other => {
      expect(_.testCandidate(rules, para, other.opponent)).toBe(other.expectedResult);
    });

    expect(_.testCandidates(rules, para, others.map(x => x.opponent))).toBe(false);
    expect(_.testCandidates(rules, para, others.slice(0, 2).map(x => x.opponent))).toBe(true);
  });
});