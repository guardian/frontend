...
const isBusinessFront = () => ['uk/business', 'us/business', 'au/business'].indexOf(config.get('page.pageId')) !== -1;

const getStocksData = () => fetchJson('/business-data/stocks.json', {
  mode: 'cors'
}).catch(ex => {
  reportError(ex, {
    feature: 'stocks'
...
const renderData = data => {
  const stockValues = data.stocks.map(stockValue => template(stockValueTemplate)({
    ...
    marketDownIcon: inlineSvg('marketDownIcon', ['stocks__icon', 'stocks__icon--down']),
    marketUpIcon: inlineSvg('marketUpIcon', ['stocks__icon', 'stocks__icon--up']),
    marketSameIcon: inlineSvg('marketSameIcon', ['stocks__icon', 'stocks__icon--same'])
  })).join('');
