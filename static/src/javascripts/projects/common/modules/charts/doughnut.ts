...
const Doughnut = (data: Object, o: Object): bonzo => {
  ...
  const $svg = $.create('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMinYMin" class="chart chart--doughnut"></svg>').attr({
    width: w,
    height: h,
  ...
  data.forEach(datum => {
     ...
     * Z: Close path
     */
    d = ['M', outer.start.x, outer.start.y, 'A', radius, radius, 0, arc, 1, outer.end.x, outer.end.y, 'L', inner.start.x, inner.start.y, 'A', cutoutRadius, cutoutRadius, 0, arc, 0, inner.end.x, inner.end.y, 'Z'];
    $g = svgEl('g').attr('class', 'chart__arc').append(svgEl('path').attr({
      d: d.join(' '),
      fill: datum.color
    }));
    ...
    if (obj.showValues) {
      $t.append(svgEl('tspan').attr('class', 'chart__label-text').text(datum.label).attr({
        x: 0,
        dy: '0'
      })).append(svgEl('tspan').attr('class', 'chart__label-value').text(datum.value).attr({
        x: 0,
        dy: '.9em'
      }));
    } else {
  ...
  // Unit of measurement
  return $svg.append(svgEl('text').attr('class', 'chart__unit').text(obj.unit).attr({
