...
const richLinkTag = ({
    ...
    href: string;
}): string => `<aside class=" element element-rich-link element-rich-link--tag
...
const insertTagRichLink = (): Promise<void> => {
    ...
    ).map((el: any) => (el as HTMLAnchorElement).href);
    ...
    const isNumberedListArticle =
        document.querySelectorAll('.js-article__body--type-numbered-list')
            .length > 0;
