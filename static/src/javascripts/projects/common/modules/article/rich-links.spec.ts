...
import { richLinkTag, insertTagRichLink } from "common/modules/article/rich-links";
import { commercialFeatures } from "common/modules/commercial/commercial-features";

let mockParas: NodeList<HTMLParagraphElement> | null | undefined;

jest.mock('lib/raven');
jest.mock('lib/config');
jest.mock('common/modules/article/space-filler', () => ({
  spaceFiller: {
    fillSpace: (rules: Object, writer: (arg0: NodeList<HTMLParagraphElement> | null | undefined) => any): Promise<any> => new Promise(resolve => {
      writer(mockParas);
      resolve(true);
    })
  }
...
describe('rich-links', () => {
  const getRichLinkElements = (): HTMLCollection<HTMLElement> => document.getElementsByClassName('element-rich-link');

  beforeEach(() => {
    if (document.body) {
      document.body.innerHTML = '<div class="js-article__body"><p>foo</p></div>';
  ...
  describe('insertTagRichLink', () => {
    ...
    it('should insert a tag rich link', () => insertTagRichLink().then(() => {
      const richLinkElements = getRichLinkElements();

      expect(richLinkElements.length).toBe(1);
      expect(richLinkElements[0].outerHTML.replace(/\s/g, '')).toBe(richLinkTag({ href: config.page.richLink }).replace(/\s/g, ''));
    }));
  });
});