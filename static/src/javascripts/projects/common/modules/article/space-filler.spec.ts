...
const findSpace: JestMockFn<any, any> = (findSpace_ as any);
...
describe('spacefiller', () => {
  ...
  it('Calls writers in order', done => {
    ...
    findSpace.mockReturnValueOnce(Promise.resolve([document.createElement('p')])).mockReturnValueOnce(Promise.resolve([document.createElement('p')]));

    const rules = {};
    const firstWriter = jest.fn();

    spaceFiller.fillSpace(rules, firstWriter).catch(done.fail);
    spaceFiller.fillSpace(rules, () => {
  ...
  it('If a writer throws an exception, we record it', () => {
    findSpace.mockReturnValueOnce(Promise.resolve([document.createElement('p')]));
