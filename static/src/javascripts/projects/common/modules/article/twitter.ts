...
const body: Element | null | undefined = document.querySelector('.js-liveblog-body, .js-article__body, .js-article__body--minute-article');

const renderTweets = (): void => {
  const nativeTweetElements: Element[] = Array.from(document.querySelectorAll('blockquote.twitter-tweet'));
  const widgetScript: Element | null | undefined = document.getElementById('twitter-widget');
...
const enhanceTweets = (): void => {
  if ((getBreakpoint() === 'mobile' && !config.get('page.isMinuteArticle')) || !config.get('switches.enhanceTweets')) {
    return;
  }

  const tweetElements: Element[] = Array.from(document.querySelectorAll('blockquote.js-tweet'));
