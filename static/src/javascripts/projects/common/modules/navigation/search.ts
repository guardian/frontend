...
class Search {
  ...
  constructor(): void {
    fastdom.measure(() => Array.from(document.getElementsByClassName('js-search-toggle'))).then(toggles => {
      ...
      if (googleSearchSwitch && googleSearchUrl && googleSearchId && toggles.length > 0) {
        this.gcsUrl = `${googleSearchUrl}?cx=${googleSearchId}`;
        this.resultSetSize = config.get('page.section') === 'identity' ? 3 : 10;
        ...
        toggles.forEach(toggle => {
          ...
          fastdom.mutate(() => {
            ...
            toggle.setAttribute('aria-controls', 'search-popup');
            toggle.setAttribute('aria-haspopup', 'dialog');
          });

          fastdom.measure(() => document.getElementsByClassName(popupClass)[0]).then(popup => {
            if (!popup) {
              return;
            }
            ...
            bean.on(toggle, 'click', e => {
              const handleEsc = (event: Event): void => {
                const keyboardEvent: KeyboardEvent = (event as any);
              ...
              const dismissSearchPopup = (event: Event): void => {
                ...
                bean.off(document, 'click', // eslint-disable-next-line no-use-before-define
                maybeDismissSearchPopup);
                document.removeEventListener('keyup', handleEsc);
              };
              const maybeDismissSearchPopup = (event: Event): void => {
                let el: Element | null | undefined = (event.target as any);
                ...
                while (el && !clickedPop) {
                  ...
                  */
                  if (el && el.classList && (el.classList.contains(popupClass) || el.classList.contains('gsq_a'))) {
                    clickedPop = true;
                  }
  ...
  load(popup: HTMLElement): void {
    ...
    fastdom.measure(() => ({
      ...
      searchPlaceholder: popup.getElementsByClassName('js-search-placeholder')[0]
    })).then(els => {
      const {
      ...
      if (!searchPlaceholder.innerHTML) {
        fastdom.mutate(() => {
          if (searchPlaceholder) {
            searchPlaceholder.innerHTML = `<div class="search-box" role="search">
                            ...
                            <gcse:searchresults webSearchResultSetSize="'}${this.resultSetSize}" linkTarget="_self"></gcse:searchresults>
                            </div>`;
          }
        });
        ...
        bean.on(searchPlaceholder, 'click', '.search-results', e => {
