...
class Search {
  ...
  constructor(): void {
    fastdom.read(() => Array.from(document.getElementsByClassName('js-search-toggle'))).then(toggles => {
      ...
      if (googleSearchSwitch && googleSearchUrl && googleSearchId && toggles.length > 0) {
        ...
        toggles.forEach(toggle => {
          ...
          fastdom.read(() => document.getElementsByClassName(popupClass)[0]).then(popup => {
            if (!popup) {
              return;
            }
