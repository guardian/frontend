...
class Profile {
  ...
  constructor(options: Object): void {
    ...
    if (document.body) {
      this.dom.container = document.body.getElementsByClassName(CONFIG.classes.container)[0];

      if (this.dom.container) {
        this.dom.content = this.dom.container.getElementsByClassName(CONFIG.classes.content)[0];
      }
    }

    if (document.body) {
      this.dom.popup = document.body.getElementsByClassName(CONFIG.classes.popup)[0];
    }

    if (document.body) {
      this.dom.register = document.body.getElementsByClassName(CONFIG.classes.register)[0];
    }

    if (document.body) {
      this.dom.commentActivity = document.body.getElementsByClassName(CONFIG.classes.commentActivity)[0];
    }
  }
