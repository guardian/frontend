...
describe('Navigation', () => {
  ...
  it('collapse default navigation', () => {
    const navigationContainer = document.getElementsByClassName(NAVIGATION_CONTAINER_CLASS)[0];

    return initNavigation().then(() => {
      if (navigationContainer) {
        expect(navigationContainer.classList.contains(NAVIGATION_CONTAINER_CLASS)).toBeFalsy();
        expect(navigationContainer.classList.contains('navigation-container--collapsed')).toBeTruthy();
      }
    });
  });
  ...
  it('should copy mega nav menu to placeholder', () => {
    ...
    return initNavigation().then(() => {
      if (placeholder) {
        expect(placeholder.innerHTML).toEqual('<div class="global-navigation">Nav</div>');
      }
    });
  });
  ...
  it('should change all sections link', () => {
    ...
    return initNavigation().then(() => {
      if (sectionLink) {
        expect(sectionLink.getAttribute('href')).toEqual('#nav-allsections');
      }
    });
  });
  ...
  it('should return correct discount code path', () => {
    ...
    expect(paths.map(getDiscountCodePath)).toEqual(['uk', '', 'uk', 'au', 'au', '']);
  });
});