...
const jsEnableFooterNav = (): Promise<void> => fastdom.write(() => {
...
const copyMegaNavMenu = (): Promise<void> => {
  ...
  return fastdom.read(() => $('.js-mega-nav')).then(elem => {
    megaNav = elem;
...
const replaceAllSectionsLink = (): Promise<void> => fastdom.read(() => $('.js-navigation-header .js-navigation-toggle')).then(elems => fastdom.write(() => {
  elems.attr('href', '#nav-allsections');
}));

const addOverflowScrollTouch = (): Promise<void> => fastdom.read(() => $('.navigation__scroll')).then(navScroll => {
  if (navScroll) {
    return fastdom.write(() => {
      navScroll.css({
