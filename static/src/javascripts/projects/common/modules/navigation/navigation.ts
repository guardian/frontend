...
const jsEnableFooterNav = (): Promise<void> => fastdom.mutate(() => {
...
const copyMegaNavMenu = (): Promise<void> => {
  ...
  return fastdom.measure(() => $('.js-mega-nav')).then(elem => {
    megaNav = elem;
...
const replaceAllSectionsLink = (): Promise<void> => fastdom.measure(() => $('.js-navigation-header .js-navigation-toggle')).then(elems => fastdom.mutate(() => {
  elems.attr('href', '#nav-allsections');
}));

const addOverflowScrollTouch = (): Promise<void> => fastdom.measure(() => $('.navigation__scroll')).then(navScroll => {
  if (navScroll) {
    return fastdom.mutate(() => {
      navScroll.css({
...
const initNavigation = (): Promise<any> => {
  ...
  const modifications = [jsEnableFooterNav(), copyMegaNavMenu(), replaceAllSectionsLink()];

  if (isIOS() && typeof getUserAgent === 'object' && parseInt(getUserAgent.version, 10) > 5) {
