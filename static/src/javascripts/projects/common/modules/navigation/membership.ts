...
import { isPayingMember, accountDataUpdateWarning, getLastOneOffContributionDate, getLastRecurringContributionDate } from "common/modules/commercial/user-features";
...
const createManageBannerLink = manageProductKeyword => `${config.get('page.mmaUrl')}/banner/${manageProductKeyword}?INTCMP=BANNER`;
...
const accountDataUpdateLink = accountDataUpdateWarningLink => {
  switch (accountDataUpdateWarningLink) {
    ...
    case 'membership':case 'digitalpack':
      return createManageBannerLink(accountDataUpdateWarningLink);
    default:
      return `${config.get('page.idUrl')}/${accountDataUpdateWarningLink}/edit`;
...
const storeBannerCanBeLoadedAgainAfter = () => {
  ...
  userPrefs.set(bannerCanBeLoadedAgainAfterKey, thisTimeTomorrow.toISOString());
};

const gaTrackMMA = (category, action) => label => {
  window.ga(`${config.get('googleAnalytics.trackers.editorial')}.send`, 'event', category, action, label);
};
...
const showAccountDataUpdateWarningMessage = accountDataUpdateWarningLink => {
  const newMessage = new Message(messageCode, {
    ...
    customJs: () => {
      ...
      const gaTrackElementViewMMA = gaTrackMMA('element view', 'internal');

      bean.on(document, 'click', '.js-site-message-close', () => {
        gaTrackClickMMA('membership action required | banner hidden');
        gaTrackClickMMA(`mma action required | banner hidden | ${updateLink || '?'}`);
        storeBannerCanBeLoadedAgainAfter();
      });

      bean.on(document, 'click', '.js-mma-update-details-button', () => {
        gaTrackClickMMA('membership action required | banner update details clicked');
        gaTrackClickMMA(`mma action required | banner update details clicked | ${updateLink || '?'}`);
        storeBannerCanBeLoadedAgainAfter();
      });

      gaTrackElementViewMMA('membership action required | banner impression');
      gaTrackElementViewMMA(`mma action required | banner impression | ${updateLink || '?'}`);
    }
  });

  newMessage.show(`<div class="site-message__copy-heading">
        ...
        <div class="site-message__copy-text">
            <a class="button site-message__copy-button js-mma-update-details-button" href="${accountDataUpdateLink(accountDataUpdateWarningLink)}">
...
const canShow: () => Promise<boolean> = () => {
  const bannerCanBeLoadedAgainAfter = userPrefs.get(bannerCanBeLoadedAgainAfterKey);
  return Promise.resolve(updateLink !== null && isUserLoggedIn() && !(bannerCanBeLoadedAgainAfter && new Date(bannerCanBeLoadedAgainAfter) > new Date()));
};
...
export const initMembership = (): void => {
  ...
  if (isPayingMember()) {
    fastdom.measure(() => document.getElementsByClassName('js-become-member')).then(becomeMemberLinks => {
      if (becomeMemberLinks.length) {
        becomeMemberLinks[0].setAttribute('hidden', 'hidden');
      }
    });

    fastdom.measure(() => document.getElementsByClassName('js-subscribe')).then(subscriberLinks => {
      if (subscriberLinks.length) {
        subscriberLinks[0].classList.remove('brand-bar__item--split--last');
      }
    });
  }
};