...
const getMenu = (): HTMLElement | null | undefined =>
    document.getElementsByClassName('js-main-menu')[0];

const getSectionToggleMenuItem = (
    section: HTMLElement
...
const genericToggleMenu = (
    ...
    const menu: HTMLElement | null | undefined = document.querySelector(
        menuClassName
    );

    const trigger: HTMLElement | null | undefined = document.querySelector(
        triggerClassName
    );
...
const enhanceCheckbox = (checkbox: HTMLElement): void => {
    fastdom.measure(() => {
        ...
        const label: HTMLElement | null | undefined = document.querySelector(
...
const enhanceMenuToggles = (): void => {
    const checkboxs: HTMLInputElement[] = Array.from(
...
const getRecentSearch = (): string | null | undefined =>
    storage.local.get(SEARCH_STORAGE_KEY);
...
const saveSearchTerm = (term: string) =>
    storage.local.set(SEARCH_STORAGE_KEY, term);
...
const addEventHandler = (): void => {
    ...
    if (search) {
        search.addEventListener('submit', (event: Event) => {
            const target = (event.target as any).querySelector(
                '.js-menu-search-term'
            );
