...
const getRecentSearch = (): string | null | undefined => local.get(SEARCH_STORAGE_KEY);
...
const toggleSubnavSections = (moreButton: HTMLElement): void => {
  fastdom.read(() => document.querySelector('.js-expand-subnav')).then(subnav => {
    if (subnav) {
      fastdom.write(() => {
