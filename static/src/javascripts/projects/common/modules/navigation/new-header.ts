...
const getMenu = (): HTMLElement | null | undefined => document.getElementsByClassName('js-main-menu')[0];

const getSectionToggleMenuItem = (section: HTMLElement): HTMLElement | null | undefined => {
...
const closeAllMenuSections = (exclude?: Node): void => {
  const sections = Array.from(document.querySelectorAll('.js-navigation-item'));
...
const toggleMenu = (): void => {
  ...
  const resetItemOrder = (): void => {
    const items = Array.from(document.querySelectorAll('.js-navigation-item'));
  ...
  const update = () => {
    ...
    const haveToCalcTogglePosition = (): boolean => isBreakpoint({
      min: 'tablet',
      max: 'desktop'
    });
    ...
    */
    ...
    menuToggle.setAttribute('data-link-name', `nav2 : veggie-burger : ${isOpen ? 'show' : 'hide'}`);
...
const toggleDropdown = (menuAndTriggerEls: MenuAndTriggerEls): void => {
  ...
  fastdom.measure(() => menuAndTriggerEls).then(els => {
    const {
      menu,
      trigger
    } = els;
...
const returnFocusToButton = (btnId: string): void => {
  fastdom.measure(() => document.getElementById(btnId)).then(btn => {
    if (btn) {
      btn.focus();
...
const genericToggleMenu = (menuClassName: string, triggerClassName: string): void => {
  const menu: HTMLElement | null | undefined = document.querySelector(menuClassName);

  const trigger: HTMLElement | null | undefined = document.querySelector(triggerClassName);
...
const enhanceCheckbox = (checkbox: HTMLElement): void => {
  fastdom.measure(() => {
    ...
    const label: HTMLElement | null | undefined = document.querySelector(`label[for='${checkboxId}']`);
...
const enhanceMenuToggles = (): void => {
  const checkboxs: Array<HTMLInputElement> = (Array.from(document.getElementsByClassName('js-enhance-checkbox')) as Array<any>);
...
const getRecentSearch = (): string | null | undefined => storage.local.get(SEARCH_STORAGE_KEY);
...
const showMoreButton = (): void => {
  fastdom.measure(() => {
    ...
    const subnavList = document.querySelector('.js-get-last-child-subnav');
    ...
    moreButton: HTMLElement | null | undefined;
...
const toggleSubnavSections = (moreButton: HTMLElement): void => {
  fastdom.measure(() => document.querySelector('.js-expand-subnav')).then(subnav => {
    if (subnav) {
      fastdom.mutate(() => {
        const isOpen = subnav.classList.contains('subnav--expanded');
...
const addEventHandler = (): void => {
  ...
  const toggleWithMoreButton = document.querySelector('.js-toggle-more-sections');
  ...
  if (menu) {
    menu.addEventListener('click', (event: Event) => {
      ...
      if (target.matches(selector)) {
        const parent: HTMLElement = (target.parentNode as any);
  ...
  if (search) {
    search.addEventListener('submit', (event: Event) => {
      const target = (event.target as any).querySelector('.js-menu-search-term');
