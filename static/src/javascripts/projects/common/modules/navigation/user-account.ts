...
const updateCommentLink = (commentItems): void => {
  ...
  if (user) {
    commentItems.forEach(commentItem => {
      fastdom.measure(() => commentItem.querySelector('.js-add-comment-activity-link')).then(commentLink => fastdom.mutate(() => {
...
const showMyAccountIfNecessary = (): void => {
  ...
  fastdom.measure(() => ({
    ...
    return fastdom.mutate(() => {
      ...
      Array.from(document.querySelectorAll('.js-user-account-trigger')).forEach(accountTrigger => {
