...
define([
    ...
    var refContainerId = '#most-popular';

    var containerUrlTemplate = '/container/{containerId}.json';
    ...
    function getFavouriteContainerIds(opts) {
        ...
        var favouriteTags = history.getPopular().map(function(pair) {
            return pair[0];
        });
    ...
    function injectContainers(opts) {
        ...
        if (favouriteContainers.length > 0) {
            Promise.all(favouriteContainers.map(fetchContainerHtml)).then(function(containers) {
                ...
                var allContainers = containers.join('');
