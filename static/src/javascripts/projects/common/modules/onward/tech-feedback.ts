...
const DISABLED_ELEMENTS = '#feedback__form input, #feedback__form textarea, #feedback__form button';
const MANDATORY_ELEMENTS = '#feedback__form input, #feedback__form textarea';
const MEMBERS_DATA_API_ENDPOINT = 'https://members-data-api.theguardian.com/user-attributes/me';
...
const getExtraDataInformation = (): Promise<Object> => fetchJson(MEMBERS_DATA_API_ENDPOINT, {
  mode: 'cors'
}).then(membershipData => ({
...
const toggleFormVisibility = (evt: Event): void => {
  const evtTarget: HTMLInputElement = (evt.target as any);
