...
const isWithinSecondsMock: any = require('common/modules/ui/relativedates').isWithinSeconds;
...
describe('breaking news', () => {
  ...
  describe('canShow', () => {
    ...
    it('should return false if fetchBreakingNews returns no collection', () => {
      fakeFetchJson.mockReturnValueOnce(Promise.resolve({
        webTitle: 'Breaking News'
      }));
    ...
    it('should wait 3 seconds before displaying new alert', () => {
      ...
      return breakingNews.canShow().then(canShow => {
        ...
        if (canShow) {
          ...
          expect(setTimeout.mock.calls[0][1]).toBe(BREAKING_NEWS_DELAY);
