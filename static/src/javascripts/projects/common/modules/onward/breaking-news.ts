...
const userCanDismissAlerts = (): boolean | null | undefined => storage.local.isAvailable();

const fetchBreakingNews = (): Promise<any> => fetchJson(breakingNewsURL, {
  mode: 'cors'
});

// handle the breaking news JSON
const parseResponse = (response: Object): Array<Alert> => (response.collections || []).filter(collection => Array.isArray(collection.content) && collection.content.length).map(collection => {
...
const getRelevantAlerts = (alerts: Array<Alert>): Array<Alert> => {
  ...
  return flattenDeep([alerts.filter(alert => alert.href === 'global').map(alert => alert.content), alerts.filter(alert => alert.href === edition).map(alert => alert.content), alerts.filter(alert => section && alert.href === section).map(alert => alert.content)]);
};
...
const pruneKnownAlertIDs = (alerts: Array<Alert>): Array<Alert> => {
  ...
  knownAlertIDs = pickBy(knownAlertIDs, (state: boolean, id: string) => alerts.some(alert => alert.id === id));
...
const filterAlertsByDismissed = (alerts: Array<Alert>): Array<Alert> => alerts.filter(alert => knownAlertIDs[alert.id] !== true);

// don't show alerts if they're over a certain age
const filterAlertsByAge = (alerts: Array<Alert>): Array<Alert> => alerts.filter(alert => {
...
const show = (): Promise<boolean> => {
  ...
  const alertDelay = knownAlertIDs.hasOwnProperty(alertToShow.id) ? 0 : DEFAULT_DELAY;
...
const canShow = (): Promise<boolean> => {
  if (userCanDismissAlerts()) {
    ...
    return fetchBreakingNews().then(parseResponse).then(getRelevantAlerts).then(pruneKnownAlertIDs).then(filterAlertsByDismissed).then(filterAlertsByAge).then(pickNewest).then(alert => {
      if (alert) {
        alertToShow = alert;
        return true;
      }
