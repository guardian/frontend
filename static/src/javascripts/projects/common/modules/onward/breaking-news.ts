...
type AlertIds = Record<string, boolean>;
...
const userCanDismissAlerts = (): boolean | null | undefined =>
...
const parseResponse = (response: Object): Alert[] =>
...
const getRelevantAlerts = (alerts: Alert[]): Alert[] => {
...
const pruneKnownAlertIDs = (alerts: Alert[]): Alert[] => {
...
const filterAlertsByDismissed = (alerts: Alert[]): Alert[] =>
    alerts.filter((alert) => !knownAlertIDs[alert.id]);

// don't show alerts if they're over a certain age
const filterAlertsByAge = (alerts: Alert[]): Alert[] =>
    alerts.filter((alert) => {
...
const pickNewest = (alerts: Alert[]): Alert =>
