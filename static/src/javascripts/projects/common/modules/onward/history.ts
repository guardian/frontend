...
const editionalised = ['business', 'commentisfree', 'culture', 'environment', 'media', 'money', 'sport', 'technology'];
...
let historyCache: Array<Array<any>> | null | undefined;
...
const getSummary = (): Object => {
  if (!summaryCache) {
    ...
    if (!isObject(summaryCache) || !isObject(summaryCache.tags) || typeof summaryCache.periodEnd !== 'number') {
      summaryCache = {
        periodEnd: today,
        tags: {},
        showInMegaNav: true
...
const mostViewedSeries = (): string => {
  ...
  return Object.keys(summary).reduce((topSeries, currentSeries) => summary[topSeries] > summary[currentSeries] ? topSeries : currentSeries, '');
};
    ...
    Object.keys(summary.tags).forEach(tid => {
      ...
      const result = buckets.map(bucket => {
        if (record[bucket.indexInRecord]) {
          const visits = record[bucket.indexInRecord].map(day => {
...
const getPopular = (opts: Object | null | undefined): Array<Array<string>> => {
  ...
  return tids.map(tid => {
    ...
    const rank = buckets.reduce((r, bucket) => r + tally(record[bucket.indexInRecord], op.weights[bucket.type], op.thresholds[bucket.type]), 0);

    return {
      idAndName: [tid, record[0]],
      rank
    };
  }).filter(Boolean).sort((a, b) => a.rank - b.rank).slice(-op.number).map(tid => tid.idAndName).reverse();
};
...
const getTopNavItems = (): Array<string> => {
  topNavItemsCache = topNavItemsCache || $('.js-navigation-header .js-top-navigation a').map(item => collapsePath(getPath($(item).attr('href'))));
...
const tagHtml = (tag: Array<string>, index: number): string => `<li class="inline-list__item">
        <a href="/${tag[0]}" class="button button--small button--tag button--secondary" data-link-name="${index + 1} | ${tag[1]}">${tag[1]}</a>
    </li>`;
...
const showInMegaNavEnabled = (): boolean => getSummary().showInMegaNav !== false;
...
const incrementWeeklyArticleCount = (pageConfig: Object): void => {
  if (!pageConfig.isFront && !getCookie(ARTICLES_VIEWED_OPT_OUT_COOKIE.name)) {
    const weeklyArticleCount = storage.local.get(storageKeyWeeklyArticleCount) || [];
    if (weeklyArticleCount[0] && weeklyArticleCount[0].week && weeklyArticleCount[0].week === startOfThisWeek) {
      ...
      const firstOldWeekIndex = weeklyArticleCount.findIndex(c => c.week && c.week < cutOff);
...
const getArticleViewCountForWeeks = (weeks: number): number => {
  ...
  const firstOldWeekIndex = weeklyCount.findIndex(c => c.week && c.week <= cutOff);
  const weeklyCountWindow = firstOldWeekIndex >= 0 ? weeklyCount.slice(0, firstOldWeekIndex) : weeklyCount;

  return weeklyCountWindow.reduce((acc, current) => current.count + acc, 0);
};

export { logHistory, logSummary, showInMegaNav, showInMegaNavEnable, showInMegaNavEnabled, getPopular, getPopularFiltered, getContributors, deleteFromSummary, isRevisit, reset, seriesSummary, mostViewedSeries, incrementDailyArticleCount, incrementWeeklyArticleCount, getArticleViewCountForDays, getArticleViewCountForWeeks, getMondayFromDate, storageKeyDailyArticleCount, storageKeyWeeklyArticleCount };
