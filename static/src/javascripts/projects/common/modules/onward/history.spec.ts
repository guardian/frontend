...
describe('history', () => {
  ...
  it('removes old daily history while incrementing the article count', () => {
    ...
    expect(localStorageStub.get('gu.history.dailyArticleCount')).toEqual([{ day: today, count: 1 }]);
  });
  ...
  it('removes old weekly history while incrementing the article count', () => {
    ...
    expect(localStorageStub.get('gu.history.weeklyArticleCount')).toEqual([{ week: startOfThisWeek, count: 1 }]);
  });
