...
import { logHistory, logSummary, showInMegaNavEnable, getPopular, getContributors, isRevisit, reset, seriesSummary, mostViewedSeries, getArticleViewCountForDays, _, incrementDailyArticleCount, getMondayFromDate, getArticleViewCountForWeeks, incrementWeeklyArticleCount } from "common/modules/onward/history";
...
describe('history', () => {
  ...
  it('should increment a count in the summary, for the 1st value from each of various page metadata', () => {
    ...
    expect(_.getSummary().tags['foo/series/bar'][0]).toEqual('Foobar Series');
    expect(_.getSummary().tags['foo/series/bar'][1][0][1]).toEqual(1);

    expect(_.getSummary().tags['profile/finbarrsaunders'][0]).toEqual('Finbarr Saunders');
    expect(_.getSummary().tags['profile/finbarrsaunders'][1][0][1]).toEqual(1);
  ...
  it('should age the data points in the the summary', () => {
    /* eslint-disable dot-notation */
    expect(_.pruneSummary({
      periodEnd: today,
      tags: { foo: ['Foo', [[0, 1]]] }
    }).tags['foo'][1][0][0]).toEqual(0);

    expect(_.pruneSummary({
      periodEnd: today - 5,
      tags: { foo: ['Foo', [[0, 1]]] }
    }).tags['foo'][1][0][0]).toEqual(5);
    /* eslint-enable dot-notation */
  });
  ...
  it('removes tags from megaNav', () => {
    ...
    megaNav.innerHTML = '<div class="js-global-navigation__section--history"></div>' + '<div class="js-global-navigation__section--history"></div>';
    ...
    expect(megaNav.querySelectorAll('.js-global-navigation__section--history').length).toEqual(2);

    showInMegaNavEnable(false);

    expect(megaNav.querySelectorAll('.js-global-navigation__section--history').length).toEqual(0);
  });
  ...
  it('adds tags to megaNav', () => {
    ...
    expect(megaNav.querySelectorAll('.js-global-navigation__section--history').length).toEqual(1);
  });
  ...
  it('removes old daily history while incrementing the article count', () => {
    ...
    expect(storageStub.local.get('gu.history.dailyArticleCount')).toEqual([{ day: today, count: 1 }]);
  });

  // weeklyArticleCountB tests
  it('gets weekly count for this week only', () => {
    const counts = [{ week: startOfThisWeek, count: 1 }, { week: startOfThisWeek - 7, count: 1 }];
  ...
  it('removes old weekly history while incrementing the article count', () => {
    ...
    expect(storageStub.local.get('gu.history.weeklyArticleCount')).toEqual([{ week: startOfThisWeek, count: 1 }]);
  });
