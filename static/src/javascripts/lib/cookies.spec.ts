...
import { cleanUp, addCookie, addSessionCookie, addForMinutes, removeCookie, getCookie, _ } from "lib/cookies";
...
describe('Cookies', () => {
  ...
  it('should be able the clean a list of cookies', () => {
    ...
    expect(cookie).toMatch(new RegExp('cookie-1-name=;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT; domain=.theguardian.com'));
    expect(cookie).toMatch(new RegExp('cookie-2-name=;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT; domain=.theguardian.com'));
  });
  ...
  it('should be able to set a cookie', () => {
    ...
    expect(document.cookie).toMatch(new RegExp('cookie-1-name=cookie-1-value; path=/; expires=Mon, 01 Jun 1970 00:00:00 GMT; domain=.theguardian.com'));
  });

  it('should be able to set a cookie for a specific number of days', () => {
    addCookie('cookie-1-name', 'cookie-1-value', 7);
    expect(document.cookie).toEqual('cookie-1-name=cookie-1-value; path=/; expires=Thu, 08 Jan 1970 00:00:00 GMT; domain=.theguardian.com');
  });

  it('should be able to set a cookie for a specific number of minutes', () => {
    addForMinutes('cookie-1-name', 'cookie-1-value', 91);
    expect(document.cookie).toEqual('cookie-1-name=cookie-1-value; path=/; expires=Thu, 01 Jan 1970 01:31:00 GMT; domain=.theguardian.com');
  });

  it('should be able to set a session cookie', () => {
    addSessionCookie('cookie-1-name', 'cookie-1-value');
    expect(document.cookie).toEqual('cookie-1-name=cookie-1-value; path=/; domain=.theguardian.com');
  });

  it('should be able remove cookies', () => {
    removeCookie('cookie-1-name');
    expect(document.cookie).toEqual('cookie-1-name=;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT; domain=.theguardian.com');
  });
  ...
  it('should be able to get a cookie', () => {
    ...
    expect(getCookie('s_vi')).toEqual('[CS]v1|29AB343C05013259-4000014980021C3D[CE]');
  });
});
