...
type BreakpointName = "mobile" | "mobileMedium" | "mobileLandscape" | "phablet" | "tablet" | "desktop" | "leftCol" | "wide";
...
let pageVisibility = document.visibilityState || // $FlowFixMe
document.webkitVisibilityState || // $FlowFixMe
document.mozVisibilityState || // $FlowFixMe
document.msVisibilityState || 'visible';

const breakpointNames: Array<BreakpointName> = breakpoints.map(breakpoint => breakpoint.name);
...
const onMatchingBreakpoint = function (mql: MediaQueryListEvent | MediaQueryList): void {
  if (mql && mql.matches) {
...
const updateBreakpoints = (): void => {
  ...
  const breakpointName = bodyStyle.content.substring(1, bodyStyle.content.length - 1);
...
const initMediaQueryListeners = (): void => {
  breakpoints.forEach((bp, index, bps) => {
    ...
    bp.mql = index < bps.length - 1 ? window.matchMedia(`${minWidth} and (max-width: ${bps[index + 1].width - 1}px)`) : window.matchMedia(minWidth);
...
const getBreakpoint = (includeTweakpoint?: boolean): BreakpointName => includeTweakpoint ? currentTweakpoint : currentBreakpoint;
...
const hasTouchScreen = (): boolean => 'ontouchstart' in window || (window.DocumentTouch && document instanceof window.DocumentTouch);
...
const adblockInUse: Promise<boolean | null | undefined> = getAdblockInUse();
...
const getUserAgent = ((): string | {browser: string;version: string;} => {
  ...
  let tem: Array<any> | null | undefined;
  let M: Array<any> | null | undefined = ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [];
...
const isGoogleProxy: () => boolean = () => !!(navigator && navigator.userAgent && (navigator.userAgent.indexOf('Google Web Preview') > -1 || navigator.userAgent.indexOf('googleweblight') > -1));

initBreakpoints();

export { hasCrossedBreakpoint, hasTouchScreen, hasPushStateSupport, getBreakpoint, getUserAgent, getViewport, isIOS, applePayApiAvailable, isAndroid, isBreakpoint, initPageVisibility, pageVisible, breakpoints, isEnhanced, adblockInUse, getReferrer, isGoogleProxy };