...
import { getUrlVars, getPath, pushUrl, constructQuery, back, pushQueryString, replaceQueryString } from "./url";
...
describe('url', () => {
  ...
  test('getUrlVars() - should get url vars', () => {
    ...
    const QUERIES = [['foo', { foo: true }], ['?foo', { foo: true }], ['?foo=bar', { foo: 'bar' }], ['foo=bar&foo=baz', { foo: 'baz' }], // Last value wins
    ['?foo=bar&boo=far', { foo: 'bar', boo: 'far' }], ['foo=bar&boo=far&foo=baz', { foo: 'baz', boo: 'far' }], ['?foo=bar&boo=far&', { foo: 'bar', boo: 'far' }], ['foo=bar&boo', { foo: 'bar', boo: true }], ['boo=&foo=bar', { foo: 'bar', boo: true }], ['name=J%C3%A9r%C3%B4me', { name: 'Jérôme' }], ['', {}]];
  ...
  test('constructQuery() - should be able to construct query', () => {
    [[{ foo: true }, 'foo=true'], [{ foo: 'bar', bar: true }, 'foo=bar&bar=true'], [{ foo: 'bar' }, 'foo=bar'], [{ foo: 'bar', boo: 'far' }, 'foo=bar&boo=far'], [{ foo: ['bar1', 'bar2'], boo: 'far' }, 'foo=bar1,bar2&boo=far'], [{}, '']].forEach(([vars, expected]) => {
  ...
  test('getPath() - should return a path', () => {
    ...
    const QUERIES = [[`${HOST}/foo/bar/baz`, '/foo/bar/baz'], [`${HOST}/foo?foo=bar`, '/foo'], [`${HOST}/foo#foo`, '/foo']];
  ...
  test('pushUrl() - should either replace or push the new state', () => {
    ...
    expect(window.history.pushState).toHaveBeenCalledWith(state, title, path);
    ...
    expect(window.history.replaceState).toHaveBeenCalledWith(state, title, path);
  ...
  const testQueryString = (historyMethod, moduleMethod) => {
    ...
    expect(window.history[historyMethod]).toHaveBeenCalledWith(state, title, querystring);

    window.history[historyMethod].mockClear();
    moduleMethod({ querystring });
    expect(window.history[historyMethod]).toHaveBeenCalledWith({}, title, querystring);
