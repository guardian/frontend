...
const pbTestNameMap: () => {
  ...
  [key: string]: boolean;
} => new URLSearchParams(window.location.search).getAll('pbtest').reduce((acc, value) => {
  acc[value] = true;
  return acc;
}, {}), (): string => // Same implicit parameter as the memoized function
window.location.search);

// returns "foo=bar&fizz=buzz" (eg. no ? symbol)
const getCurrentQueryString = (): string => window.location.search.replace(/^\?/, '');

const queryStringToUrlVars = memoize((queryString: string): Object => Array.from(new URLSearchParams(queryString).entries()) // polyfill.io guarantees URLSearchParams
...
const getUrlVars = (query?: string): Object => queryStringToUrlVars(query || window.location.search);
...
const pushQueryString = (params: Object) => updateQueryString(params, window.history.pushState.bind(window.history));

// equivalent to pushQueryString but uses history.replaceState to
// overwrite history rather than history.pushState
const replaceQueryString = (params: Object) => updateQueryString(params, window.history.replaceState.bind(window.history));
...
const constructQuery = (query: Object): string => Object.keys(query).map(param => {
  const value = query[param];
  const queryValue = Array.isArray(value) ? value.map(v => encodeURIComponent(v)).join(',') : encodeURIComponent(value);
...
export { getUrlVars, getPath, pushUrl, constructQuery, back, supportsPushState, pushQueryString, replaceQueryString, pbTestNameMap };