...
describe('$', () => {
  ...
  test('creates an element and bonzos it', () => {
    expect($.create('<p><span class="test-class"></span></p>').html()).toEqual('<span class="test-class"></span>');
  });
  ...
  test("can find an element's ancestor", () => {
    expect($.ancestor(document.querySelector('.child'), 'no-element-has-this-class')).toBe(null);
    expect($.ancestor(document.querySelector('.child'), 'grandparent')).toEqual(expect.objectContaining({ id: 'ancestorTarget' }));
  });
});