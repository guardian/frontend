...
type yesable = (arg: any) => boolean | void | null | undefined;
type noable = () => boolean | void | null | undefined;

// #? this is very hard to understand, what is it's purpose?
const isit = (isTrue: (string | boolean) | null | undefined, yes: yesable, no: noable, arg: any): boolean => {
  if (isTrue) {
...
const isMatch = (yes: yesable, no: noable): boolean => {
  ...
  const pageTypes = [['minbymin', config.get('page.isLiveBlog')], ['report', config.hasTone('Match reports')], ['preview', config.hasSeries('Match previews')], ['stats', match.id]];
...
const isClockwatch = (yes: yesable): boolean => isit(config.hasSeries('Clockwatch'), yes);

const isLiveClockwatch = (yes: yesable): boolean => isClockwatch(() => isit(!!config.get('page.isLive'), yes));

const isFootballStatsPage = (yes: yesable): boolean => isit(!!config.get('page.footballMatch'), yes);
...
export { isMatch, isCompetition, isClockwatch, isLiveClockwatch, isFootballStatsPage, belowArticleVisible, keywordExists };