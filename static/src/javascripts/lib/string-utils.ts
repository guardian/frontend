...
export const splitAndTrim = (str: string, delim: string): string[] =>
    str
        .split(delim)
        .map((s) => s.trim())
        .filter(Boolean);

export const optionalSplitAndTrim = (str: string, delim: string): string[] =>
    str ? splitAndTrim(str, delim) : [];

export const optionalStringToBoolean = (
    str: string | null | undefined
): boolean =>
    typeof str === 'string' ? str.toLowerCase().trim() === 'true' : false;

export const throwIfEmptyString = (
    name: string,
    str: string | null | undefined
): string => {
