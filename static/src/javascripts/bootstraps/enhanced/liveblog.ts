...
const affixTimeline = (): void => {
  ...
  if (isBreakpoint({
    min: 'desktop'
  }) && !keywordIds.includes('football/football') && !keywordIds.includes('sport/rugby-union')) {
...
const init = (): void => {
  catchErrorsWithContext([['lb-autoupdate', createAutoUpdate], ['lb-timeline', affixTimeline], ['lb-timestamp', keepTimestampsCurrent], ['lb-richlinks', upgradeRichLinks]]);

  initTrails();
  initLiveblogCommon();

  catchErrorsWithContext([['lb-ready', () => {
