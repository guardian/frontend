...
import { isCompetition, isMatch, isFootballStatsPage, isLiveClockwatch } from "lib/page";
import reportError from "lib/report-error";
import { TableDoughnut } from "common/modules/charts/table-doughnut";
import { statsFor, tableFor, matchDayFor } from "common/modules/sport/football/football";
...
const renderExtras = (_extras: Array<Extra | null | undefined>): void => {
  const extras = [..._extras].filter(extra => extra);
  const ready = extras.filter(extra => extra && extra.ready === false).length === 0;
...
const renderTable = (competition: string, extras: Array<Extra | null | undefined>): void => {
  extras[2] = {
    ready: false
  };

  $.create(`<div class="js-football-table"
              data-link-name="football-table-embed"></div>
        `).each(container => {
    tableFor(competition).fetch(container).then(() => {
...
const init = (): void => {
      ...
      renderNav(match, (resp, $nav, endpoint): void => {
        ...
        if (resp.hasStarted && $nav) {
          ...
          $.create(`<div class="match-stats__container js-match-stats"></div>`).each(container => {
            statsFor(statsUrl).fetch(container).then(() => {
        ...
        isCompetition((competition: string) => {
          ...
          $.create(`
                    ...
                    `).each(container => {
            matchDayFor(competition, resp.matchDate).fetch(container).then(() => {
              extras[1] = {
  ...
  isLiveClockwatch(() => {
    const ml = new MatchListLive('match-day', isCompetition() || 'premierleague', config.dateFromSlug());
    const $img = $('.media-primary');
    const $matchListContainer = $.create( // $FlowFixMe
    ...
    ml.fetch($matchListContainer[0]).then(() => {
      handleResponse(true);
    }).catch(() => {
  ...
  // Binding
  bean.on(document.body, 'click', '.js-show-more-football-matches', (e: Event): void => {
    e.preventDefault();
    ...
    if (el && el instanceof HTMLAnchorElement) {
      ...
      if (href && putsMore && newData) {
        fetchJson(`${href}.json`).then(resp => {
          $.create(resp.html).each(html => {
            const htmlContainer = document.querySelector(`[data-show-more-contains="${putsMore}"]`);
  ...
  bean.on(document.body, 'change', $('form.football-leagues')[0], function onChange() {
