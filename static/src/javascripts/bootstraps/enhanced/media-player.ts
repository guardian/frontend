...
const enhanceVideo = (el: HTMLMediaElement, autoplay: boolean): any => {
  ...
  const showEndSlate = dataset.showEndSlate === 'true' && !!config.get('page.section');

  const player = createVideoPlayer(el, videojsOptions({
    plugins: {
      embed: {
        embeddable: !config.get('page.isFront') && config.get('switches.externalVideoEmbeds') && (config.get('page.contentType') === 'Video' || dataset.embeddable === 'true'),
        location: `${config.get('page.externalEmbedHost')}/embed/video/${embedPath || config.get('page.pageId')}`
      }
    }
  }));
    ...
    if (videoInfo.expired) {
      player.ready(() => {
        player.error({
          code: 0,
          type: 'Video Expired',
          message: 'This video has been removed. This could be because it launched early, ' + 'our rights have expired, there was a legal issue, or for another reason.'
        });
      ...
      isGeoBlocked(el).then(isVideoGeoBlocked => {
        if (isVideoGeoBlocked) {
          ...
          playerSetupComplete.then(() => {
            if (autoplay && accessibilityisOn('flashing-elements')) {
              player.play();
            }
          });
        }
...
const initPlayButtons = (root: HTMLElement | null | undefined): void => {
  fastdom.read(() => {
