...
const bootEnhanced = (): void => {
  ...
  catchErrorsWithContext([['ga-user-timing-enhanced-start', () => {
    trackPerformance('Javascript Load', 'enhancedStart', 'Enhanced start parse time');
  }], //
  // A/B tests
  //
  ['ab-tests', () => {
    catchErrorsWithContext([['ab-tests-run', () => {
      runAndTrackAbTests();
    }]]);
  }], ['enrich-acquisition-links', initAcquisitionsLinkEnrichment], ['remote-epics', fetchAndRenderEpic]]);
  ...
  import(
  /* webpackMode: "eager" */
  'bootstraps/enhanced/common').then(({
    init
  }) => {
    ...
    if (config.get('page.contentType') === 'Article' && !config.get('page.isMinuteArticle')) {
      require.ensure([], require => {
        bootstrapContext('article', require('bootstraps/enhanced/article').init);
        bootstrapContext('article : lightbox', require('common/modules/gallery/lightbox').init);
      }, 'article');
    }
    ...
    if (config.get('page.contentType') === 'LiveBlog') {
      require.ensure([], require => {
        ...
        bootstrapContext('liveBlog : lightbox', require('common/modules/gallery/lightbox').init);
      }, 'live-blog');
    }
    ...
    if (config.get('page.contentType') === 'Audio' || config.get('page.contentType') === 'Video' || config.get('page.contentType') === 'Interactive') {
      require.ensure([], require => {
        bootstrapContext('media : trail', require('bootstraps/enhanced/trail').initTrails);
      }, 'trail');
    }

    fastdom.measure(() => qwery('audio')).then(els => {
      if (els.length) {
        require.ensure([], require => {
          bootstrapContext('media-player', require('bootstraps/enhanced/media-player').initMediaPlayer);
        }, 'media-player');
      }
    });

    // Native video player enhancements
    fastdom.measure(() => qwery('video')).then(els => {
      if (els.length) {
        require.ensure([], require => {
          bootstrapContext('video-player', require('bootstraps/enhanced/video-player').initVideoPlayer);
        }, 'video-player');
      }
    });
    ...
    if (config.get('page.contentType') === 'Gallery') {
      require.ensure([], require => {
        ...
        bootstrapContext('gallery : lightbox', require('common/modules/gallery/lightbox').init);
      }, 'gallery');
    }

    if (config.get('page.contentType') === 'ImageContent') {
      require.ensure([], require => {
        bootstrapContext('image-content : lightbox', require('common/modules/gallery/lightbox').init);
        bootstrapContext('image-content : trail', require('bootstraps/enhanced/trail').initTrails);
      }, 'image-content');
    }
    ...
    if (config.get('page.section') === 'identity') {
      require.ensure([], require => {
        bootstrapContext('profile', require('bootstraps/enhanced/profile').initProfile);
      }, 'profile');
    }
    ...
    if ((window.location.protocol === 'https:' && config.get('page.section') !== 'identity') || window.location.hash.indexOf('force-sw') > -1) {
    ...
    if (window.location.hash.includes('experiments')) {
      require.ensure([], require => {
        bootstrapContext('experiments', require('common/modules/experiments').showExperiments);
      }, 'experiments');
    }

    if (config.get('page.contentType') === 'Audio') {
      require.ensure([], require => {
        bootstrapContext('audio', require('common/modules/audio').init);
      }, 'audio');
    }

    // Mark the end of synchronous execution.
    markTime('App End');
    catchErrorsWithContext([['ga-user-timing-enhanced-end', () => {
      trackPerformance('Javascript Load', 'enhancedEnd', 'Enhanced end parse time');
    }]]);
  });
};
