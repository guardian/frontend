...
import { getPopularFiltered, deleteFromSummary, showInMegaNav, showInMegaNavEnable, showInMegaNavEnabled } from "common/modules/onward/history";

class SummaryTagsList extends Component<any, any> {
  ...
  render() {
    const tagElements = this.state.popular.map(tag => <span className="button button--small button--tag button--secondary" key={tag[0]}>
                <button onClick={this.handleRemove.bind(this, tag[0])} data-link-name={`remove | ${tag[1]}`}>
                    X
                </button>
...
class SummaryTagsSettings extends Component<any, any> {
  ...
  render() {
    ...
    return <div data-link-name="suggested links">
                <p>
                ...
                <button onClick={this.handleToggle.bind(this)} className="button button--medium button--primary" data-link-name={toggleAction}>
...
const init = (): void => {
  const placeholder: HTMLElement | null | undefined = document.getElementById('preferences-history-tags');
