...
const cricket = (): void => {
  ...
  if (matchDate && team) {
    ...
    fastdom.read(() => document.querySelector('.js-cricket-score')).then(parentEl => {
...
const rugby = (): void => {
  ...
  if (config.get('page.rugbyMatch') && pageType) {
    ...
    scoreBoard.fetched = (resp: Object): void => {
      fastdom.read(() => document.querySelector('article')).then(liveblog => {
        liveblog.classList.add('content--has-scores');
      });

      fastdom.read(() => document.querySelector('.content--liveblog')).then(liveblog => {
