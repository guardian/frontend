...
const renderExtras = (extras: Array<Extra>): void => {
  if (extras.filter(extra => extra && extra.ready === false).length === 0) {
    belowArticleVisible(() => {
      $('.js-after-article').append($.create('<div class="football-extras"></div>').each(extrasContainer => {
...
const cricket = (): void => {
  ...
  if (matchDate && team) {
    ...
    fastdom.measure(() => document.querySelector('.js-cricket-score')).then(parentEl => {
...
const rugby = (): void => {
  ...
  if (config.get('page.rugbyMatch') && pageType) {
    ...
    scoreBoard.fetched = (resp: Object): void => {
      fastdom.measure(() => document.querySelector('article')).then(liveblog => {
        liveblog.classList.add('content--has-scores');
      });

      fastdom.measure(() => document.querySelector('.content--liveblog')).then(liveblog => {
      ...
      if (resp.matchStat) {
        $.create(`<div class="match-stats__container js-match-stats">${resp.matchStat}</div>`).each(container => {
