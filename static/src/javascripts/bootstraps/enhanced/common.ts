...
import { refresh as refreshUserFeatures, extendContribsCookieExpiry } from "common/modules/commercial/user-features";
...
import { initMembership, membershipBanner } from "common/modules/navigation/membership";
import { logHistory, logSummary, showInMegaNav, incrementDailyArticleCount, incrementWeeklyArticleCount } from "common/modules/onward/history";
...
import { cmpBannerCandidate, addPrivacySettingsLink } from "common/modules/ui/cmp-ui";
...
const initialiseTopNavItems = (): void => {
  const header: HTMLElement | null | undefined = document.getElementById('header');
...
const loadAnalytics = (): void => {
  ...
  if (config.get('switches.ophan')) {
    if (config.get('switches.scrollDepth')) {
      ...
      new ScrollDepth({
        isContent: /Article|LiveBlog/.test(config.get('page.contentType'))
      });
    }
  }
};
...
const cleanupCookies = (): void => {
  cleanUp(['mmcore.pd', 'mmcore.srv', 'mmid', 'GU_ABFACIA', 'GU_FACIA', 'GU_ALPHA', 'GU_ME', 'at', 'gu_join_date']);
};

const cleanupLocalStorage = (): void => {
  const deprecatedKeys = ['gu.subscriber', 'gu.contributor', 'gu.cachedRecommendations', 'gu.recommendationsEnabled', 'gu.abb3.exempt'];
...
const windowEventListeners = (): void => {
  ['orientationchange'].forEach(event => {
    window.addEventListener(event, mediator.emit.bind(mediator, `window:${event}`));
  });
};
...
const initialiseEmail = (): void => {
  ...
  Array.from(document.getElementsByClassName('js-email-sub__iframe')).forEach(el => {
    const iframe: HTMLIFrameElement = (el as any);
  ...
  bean.on(window, 'interactive-loaded', () => {
    Array.from(document.querySelectorAll('.guInteractive .js-email-sub__iframe')).forEach(el => {
      const iframe: HTMLIFrameElement = (el as any);
...
const initialiseBanner = (): void => {
  // ordered by priority
  const bannerList = [cmpBannerCandidate, breakingNews, signInGate, membershipBanner, readerRevenueBanner, subscriptionBanner, smartAppBanner, adFreeBanner, emailSignInBanner, brazeBanner];

  initBannerPicker(bannerList);
};

const initialiseConsentCookieTracking = (): void => trackConsentCookies(getAllAdConsentsWithState());

const init = (): void => {
  catchErrorsWithContext([// Analytics comes at the top. If you think your thing is more important then please think again...
  ['c-analytics', loadAnalytics], ['c-consent-cookie-tracking', initialiseConsentCookieTracking], ['c-identity', initIdentity], ['c-adverts', requestUserSegmentsFromId], ['c-discussion', initDiscussion], ['c-test-cookie', testCookie], ['c-event-listeners', windowEventListeners], ['c-block-link', fauxBlockLink], ['c-iframe', checkIframe], ['c-normalise', normalise], ['c-tabs', showTabs], ['c-top-nav', initialiseTopNavItems], ['c-init-nav', initialiseNavigation], ['c-toggles', showToggles], ['c-dates', showRelativeDates], ['c-clickstream', initialiseClickstream], ['c-history', updateHistory], ['c-id-cookie-refresh', idCookieRefresh], ['c-history-nav', showHistoryInMegaNav], ['c-start-register', startRegister], ['c-cookies', cleanupCookies], ['c-extend-contribs-expiry', extendContribsCookieExpiry], ['c-localStorage', cleanupLocalStorage], ['c-overlay', initOpenOverlayOnClick], ['c-public-api', initPublicApi], ['c-tech-feedback', initTechFeedback], ['c-media-listeners', mediaListener], ['c-accessibility-prefs', initAccessibilityPreferences], ['c-pinterest', startPinterest], ['c-email', initialiseEmail], ['c-user-features', refreshUserFeatures], ['c-membership', initMembership], ['c-banner-picker', initialiseBanner], ['c-increment-article-counts', updateArticleCounts], ['c-reader-revenue-dev-utils', initReaderRevenueDevUtils], ['c-add-privacy-settings-link', addPrivacySettingsLink]]);
};
