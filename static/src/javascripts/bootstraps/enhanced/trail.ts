...
const initMoreInSection = (): void => {
  ...
  if ((config.get('page.contentType') !== 'Audio' && config.get('page.contentType') !== 'Video') || !showRelatedContent || isPaidContent || !section) {
    return;
  }

  const el = $('.js-more-in-section')[0];
  moreInSeriesContainerInit(el, config.get('page.contentType', '').toLowerCase(), section, shortUrl, seriesId);
};
...
const insertOrProximity = (selector, insert) => {
  if (window.location.hash) {
    ...
    fastdom.measure(() => document.querySelector(selector)).then(el => {
      if (el) {
...
const initRelated = () => {
  if (!(config.get('page.seriesId') || config.get('page.blogIds'))) {
    insertOrProximity('.js-related', () => {
      ...
      if ('contentType' in config.get('page') && ['video', 'interactive'].includes(config.get('page.contentType', '').toLowerCase())) {
...
const initOnwardContent = () => {
  insertOrProximity('.js-onward', () => {
    if ((config.get('page.seriesId') || config.get('page.blogIds')) && config.get('page.showRelatedContent')) {
      new OnwardContent(qwery('.js-onward'));
    } else if (config.get('page.tones', '') !== '') {
      fastdom.measure(() => Array.from(document.querySelectorAll('.js-onward'))).then(els => {
        els.forEach(c => {
...
const initDiscussion = () => {
  if (config.get('switches.enableDiscussionSwitch') && config.get('page.commentable')) {
    fastdom.measure(() => document.querySelector('.discussion')).then(el => {
      if (el) {
...
const repositionComments = () => {
  if (!isUserLoggedIn()) {
    fastdom.measure(() => $('.js-comments')).then($comments => fastdom.mutate(() => {
...
const initTrails = () => {
  catchErrorsWithContext([['c-discussion', initDiscussion], ['c-comments', repositionComments], ['c-shares', loadShareCounts], ['c-popular', initPopular], ['c-related', initRelated], ['c-onward', initOnwardContent], ['c-more-in-section', initMoreInSection]]);
};
