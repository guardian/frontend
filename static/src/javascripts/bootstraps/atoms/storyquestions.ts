...
const updateHeight = () => {
  fastdom.read(() => document.documentElement && document.documentElement.getBoundingClientRect().height).then(height => {
    send('resize', { height });
  });
};
...
Promise.all([window.guardian.polyfilled ? Promise.resolve() : new Promise(resolve => {
  ...
  Array.from(document.getElementsByClassName('user__question')).slice(0, 1).forEach((sq: Element) => {
