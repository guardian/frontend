...
const createVideoPlayer = (el: HTMLVideoElement, options: Object): any => videojs(el, options);
...
const initPlayer = (): void => {
  ...
  bonzo(qwery('.js-gu-media--enhance')).each(el => {
    ...
    const player = createVideoPlayer(el, videojsOptions({
      ...
      plugins: {
        embed: {
          embeddable: config.get('switches.externalVideoEmbeds') && config.get('page.embeddable'),
          location: `${config.get('page.externalEmbedHost')}/embed/video/${config.get('page.pageId')}`
        }
      }
    }));
