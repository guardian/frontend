...
import { isUserLoggedIn, init as identityInit } from "common/modules/identity/api";
...
const handleMembershipAccess = (): void => {
  ...
  const redirect = (): void => {
    window.location.assign(`${membershipUrl}/membership-content?referringContent=${contentId}&membershipAccess=${membershipAccess}`);
  };
...
const bootStandard = (): void => {
  ...
  catchErrorsWithContext([['ga-user-timing-standard-start', () => {
    trackPerformance('Javascript Load', 'standardStart', 'Standard start parse time');
  }]]);
  ...
  */
  ...
  if (window.location.hash.match(/[#&]noadsaf(&.*)?$/)) {
    ...
    forcedAdFreeExpiryTime.setTime(forcedAdFreeExpiryTime.getTime() + forcedAdFreeValidSeconds * 1000);
  ...
  if (config.get('switches.blockIas') && config.get('switches.serviceWorkerEnabled') && navigator.serviceWorker) {
  ...
  */
  ...
  catchErrorsWithContext([['ga-user-timing-standard-end', () => {
    trackPerformance('Javascript Load', 'standardEnd', 'Standard end parse time');
  }]]);
};
