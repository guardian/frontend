...
const bootstrapAtom = <A>(atomMaker: AtomMaker<A>, atomType: AtomType) => {
  ...
  Array.from(document.querySelectorAll(`[data-atom-type='${atomType}']`)).forEach(atomDom => {
    const atom = atomBuilder(atomDom).runTry();
    if (typeof atom === 'string') {
      // eslint-disable-next-line no-console
      console.log(`Failed to initialise atom [${atomType}/${atomDom.getAttribute('data-atom-id') || ''}]: ${atom}`);
...
const initCharts = () => {
  const iframes: HTMLIFrameElement[] = ([...document.querySelectorAll('.atom--chart > .atom__iframe')] as any);

  window.addEventListener('message', event => {
    const iframe: HTMLIFrameElement | null | undefined = iframes.find(i => {
      try {
  ...
  iframes.forEach(iframe => {
    const src = (iframe.getAttribute('srcdoc') || '').replace(/<gu-script>/g, '<script>') // eslint-disable-next-line no-useless-concat
