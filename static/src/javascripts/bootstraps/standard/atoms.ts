...
const bootstrapAtom = <A>(atomMaker: AtomMaker<A>, atomType: AtomType) => {
    ...
    Array.from(
        ...
        if (typeof atom === 'string') {
            ...
            console.log(
                `Failed to initialise atom [${atomType}/${
                    atomDom.getAttribute('data-atom-id') || ''
                }]: ${atom}`
            );
...
const initCharts = () => {
    ...
    window.addEventListener('message', (event) => {
        const iframe: HTMLIFrameElement | null | undefined = iframes.find(
            (i) => {
                try {
    ...
    iframes.forEach((iframe) => {
        const src = (iframe.getAttribute('srcdoc') || '')
            .replace(/<gu-script>/g, '<script>') // eslint-disable-next-line no-useless-concat
