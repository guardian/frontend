...
describe('AJAX', function () {
    ...
    beforeEach(function(done){
        ...
        injector.test(['common/utils/ajax'], function () {
            ajax = arguments[0];
            done();
        });
    ...
    it('should proxy calls to reqwest', function () {
        ...
        expect(reqwestSpy.default).toHaveBeenCalledWith({
    ...
    it('should not touch a url that is already absolute', function () {
        ...
        expect(reqwestSpy.default).toHaveBeenCalledWith({url: 'http://apis.guardian.co.uk/endpoint.json'});
    });

    it('should not touch a url that is already absolute (https)', function () {
        ajax({url: 'https://apis.guardian.co.uk/endpoint.json'});

        expect(reqwestSpy.default).toHaveBeenCalledWith({url: 'https://apis.guardian.co.uk/endpoint.json'});
    });

    it('should not touch a protocol-less url', function () {
        ajax({url: '//apis.guardian.co.uk/endpoint.json'});

        expect(reqwestSpy.default).toHaveBeenCalledWith({url: '//apis.guardian.co.uk/endpoint.json'});
    });
