...
//    }
  ...
  private def baseCapiQuery(itemId: String)(implicit request: Request[AnyContent]): ItemQuery =
    contentApiClient
  ...
  def renderHostedPage(campaignName: String, pageName: String): Action[AnyContent] =
    Action.async { implicit request =>
      ...
      (for {
        response <- contentApiClient.getResponse(baseCapiQuery(itemId))
        content = response.content.get
        page = HostedPage.fromContent(content).get
      } yield {
        renderPage(page, itemId)
      }) recover {
        case e: ContentApiError if e.httpStatus == 410 =>
          Cached(cacheDuration)(RevalidatableResult.Ok(commercialExpired(wasAHostedPage = true)))
        case e: ContentApiError if e.httpStatus == 404 =>
          Cached(cacheDuration)(WithoutRevalidationResult(NotFound))
    }
  }
