...
class HostedTrailsTest extends FlatSpec with Matchers {
  ...
  private def mkSponsorship(): Sponsorship =
    new Sponsorship {
  ...
  private def mkTag(
    ...
    new Tag {
      ...
      val internalName = None
    }

  private def mkHostedTag(): Tag =
    mkTag(
  ...
  private def mkToneTag(): Tag =
    mkTag(
      tagType = TagType.Tone,
    )

  private def mkContent(itemId: String, publishedDateTime: Long): Content =
    new Content {
      val id = itemId
