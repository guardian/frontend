...
const installIfNecessary = (...packages) => new Promise(resolve => {
    try {
        packages.forEach(require);
        resolve();
    } catch (e) {
...
installIfNecessary('semver', 'chalk').then(() => {
    ...
    if (!semver.satisfies(nodeVersion, nvmrcVersion)) {
        ...
        console.log(
                chalk.dim(`Frontend requires Node v${nvmrcVersion} or later.\n` +
