...
import model.Cached
import discussion.model.{Comment, Profile, PrivateProfileFields}
...
trait CtaController extends DiscussionController {
  ...
  def cta(): Action[AnyContent] = Action.async {
    ...
    implicit request =>
      ...
      val commentJson = ((componentsJson \\ "components" )(0) \\ "comments")(0)(0)
      val comment = Comment(commentJson)

      Future(
        Cached(60) {
          JsonComponent("html" -> views.html.fragments.commentCta(comment))
        }
      )
