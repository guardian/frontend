...
class GalleryControllerTest extends FlatSpec with ShouldMatchers {
  ...
  "Gallery Controller" should "200 when content type is gallery" in Fake {
    val result = controllers.CommentPageController.render(galleryUrl)(TestRequest())
  ...
  it should "return JSONP when callback is supplied" in Fake {
    ...
    val result = controllers.CommentPageController.render(galleryUrl)(fakeRequest)
    status(result) should be(200)
    header("Content-Type", result).get should be("application/javascript")
  }

  it should "internal redirect when content type is not gallery" in Fake {
    val result = controllers.CommentPageController.render("world/video/2012/feb/10/inside-tibet-heart-protest-video")(TestRequest())
    status(result) should be(200)
  ...
  it should "display an expired message for expired content" in Fake {
    val result = controllers.CommentPageController.render("theobserver/gallery/2012/jul/29/1")(TestRequest())
