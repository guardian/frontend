...
object FrontsController extends Controller with ExecutionContexts with GetPaClient with Logging {
  ...
  private def chooseMatch(competitionId: String, team1IdOpt: Option[String], team2IdOpt: Option[String]) =AuthActions.AuthAction.async { implicit request =>
    for {
  ...
  def bigMatchSpecial(matchId: String) =AuthActions.AuthAction.async { implicit request =>
    for {
