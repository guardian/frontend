...
class FrontPressController(akkaAsync: AkkaAsync, val controllerComponents: ControllerComponents)(implicit context: ApplicationContext) extends BaseController with Logging with ImplicitControllerExecutionContext {

  def press(): Action[AnyContent] = Action { implicit request =>
    Ok(views.html.press())
  }

  def queueAllFrontsForPress(): Action[AnyContent] = Action { implicit request =>
  ...
  def queueHighFrequencyFrontsForPress(): Action[AnyContent] = Action { implicit request =>
    runJob(RefreshFrontsJob.runFrequency(akkaAsync)(HighFrequency), "high frequency")
  }

  def queueStandardFrequencyFrontsForPress(): Action[AnyContent] = Action { implicit request =>
    runJob(RefreshFrontsJob.runFrequency(akkaAsync)(StandardFrequency), "standard frequency")
  }

  def queueLowFrequencyFrontsForPress(): Action[AnyContent] = Action { implicit request =>
