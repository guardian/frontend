...
trait UserFormMapping[T <: UserFormData] extends Mappings {
  ...
  /** Returns a Form filled with existing UserFormData DTO value (as opposed to binding data from request) which is in
    * turn created from User DO from IDAPI.
    *
    * @param userDO
    *   User domain object from IDAPI defined in identity-model library
    * @param messagesProvider
    * @return
    *   Form filled with UserFormData DTO
    */
...
/** Trait to represent form specific Data Transfer Objects used for form data binding. These DTOs are meant to represent
  * parts of User Domain Object form
  */
trait UserFormData {
