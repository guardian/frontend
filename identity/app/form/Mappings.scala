...
trait Mappings {
  ...
  def idEmail(implicit messagesProvider: MessagesProvider): Mapping[String] = text.verifying(
    Messages("error.email"),
    value => value.isEmpty || EmailPattern.findFirstIn(value).isDefined
  )

  def idPassword(implicit messagesProvider: MessagesProvider): Mapping[String] = text verifying(
