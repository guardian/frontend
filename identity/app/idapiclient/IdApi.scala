...
abstract class IdApi(val apiRootUrl: String, http: Http, jsonBodyParser: JsonBodyParser, val clientAuth: Auth)
  ...
  val extractUser = extract(jsonField("user")) _
  ...
  def updateUser(userId: String, auth: Auth, trackingData: TrackingData, path: String, data: JValue): Future[Response[JValue]] = {
    ...
    post(apiUrl(urlJoin("user" :: userId :: pathParts : _*)), Some(auth), Some(trackingData), Some(write(data))) map extract(jsonField(pathParts.head))
  }
  ...
  def post(apiPath: String,
    ...
    http.POST(apiUrl(apiPath), None, buildParams(auth, trackingParameters), buildHeaders(auth))
}
