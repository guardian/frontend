...
class IdApiTest extends path.FreeSpec with ShouldMatchers with MockitoSugar {
  ...
  val api = new SynchronousIdApi(idConfig, http, jsonParser, clientAuth)
  ...
  "the unauth method" - {
    "given a valid response " - {
      ...
      "adds the client access token parameter to the request" in {
        api.unauth(Anonymous, trackingParameters)
        verify(http).POST(Matchers.eq(s"$apiRoot/unauth"), Matchers.any[Option[String]], Matchers.any[Parameters], argThat(new ParamsIncludes(Iterable(("X-GU-ID-Client-Access-Token", "Bearer clientAccessToken")))))
      }
  ...
  "synchronous version" - {
    val syncApi = new SynchronousIdApi(idConfig, http, jsonParser, clientAuth)
