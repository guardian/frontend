...
trait PanDomainAuthActions extends AuthActions with Results {
  ...
  override def invalidUserMessage(claimedAuth: AuthenticatedUser): String = {
    if( (claimedAuth.user.emailDomain == "guardian.co.uk") && !claimedAuth.multiFactor) {
      s"${claimedAuth.user.email} is not valid for use with Fronts as you need to have two factor authentication enabled." +
      ...
      s"${claimedAuth.user.email} is not valid for use with Fronts. You need to use your Guardian Google account to login. Please sign in with your Guardian Google account first, then retry logging in"
    }
  }
