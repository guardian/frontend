...
class WeatherController(weatherApi: WeatherApi, val controllerComponents: ControllerComponents) extends BaseController with ImplicitControllerExecutionContext {
  ...
  def forCity(cityId: String): Action[AnyContent] = Action.async{ implicit request =>
  ...
  def forecastForCityId(cityId: String): Action[AnyContent] = Action.async { implicit request =>
