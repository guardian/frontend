...
class FaciaPressDeduplicationTest extends FlatSpec with Matchers with FaciaPressDeduplicationTestData {
  ...
  it should "curated elements are never removed" in new FaciaPressDeduplicationTestDataScope {
  ...
  it should "remove duplicated backfill'ed content" in new FaciaPressDeduplicationTestDataScope {
    newSequence(1).pressedCollection.backfill.size shouldBe 2
  }

  it should "remove all backfilled contents when possible" in new FaciaPressDeduplicationTestDataScope {
