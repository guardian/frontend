...
class CompetitionsTest extends FreeSpec with Matchers with OptionValues {
  "Competitions" - {
    Seq(
      ...
      (s"isAMatchInProgress returns ${testcase._3} if there's a match that started ${testcase._2} ago") in {
        val matches: Seq[FootballMatch] =
          FootballTestData.matchesWithLiveMatchAtCurrentMinusDuration(testcase._2, testcase._1)
    ...
    Seq(
      ...
      (s"isAMatchInProgress returns ${testcase._3} if there's a match with liveMatch status ${testcase._1} that will start in ${testcase._2}") in {
        val matches: Seq[FootballMatch] =
          FootballTestData.matchesWithLiveMatchAtCurrentPlusDuration(testcase._2, testcase._1)
        val testCompetition = FootballTestData.competitions(1).copy(matches = matches)
        val competitions = Competitions(Seq(testCompetition))
  ...
  object FootballTestData {
    ...
    def matchesWithLiveMatchAtCurrentMinusDuration(duration: Duration, liveMatchStatus: Boolean = false) = {
      Seq(
        result("Bolton", "Derby", 1, 1, today.minusDays(1), Some("Bolton win 4-2 on penalties.")),
        liveMatch(liveMatchStatus, "Cardiff", "Brighton", 2, 0, today.minus(duration)),
        fixture("Wolves", "Burnley", today.plusDays(2)),
      )
    }
    ...
    def matchesWithLiveMatchAtCurrentPlusDuration(duration: Duration, liveMatchStatus: Boolean = false) = {
      Seq(
        result("Bolton", "Derby", 1, 1, today.minusDays(1), Some("Bolton win 4-2 on penalties.")),
        liveMatch(liveMatchStatus, "Cardiff", "Brighton", 2, 0, today.plus(duration)),
        fixture("Wolves", "Burnley", today.plusDays(2)),
      )
    }
  }
