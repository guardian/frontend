...
trait CompetitionTestData {
  ...
  private def fixture(date: DateTime, stage: Stage, round: Round, leg: String, homeTeam: MatchDayTeam = _fixture.homeTeam, awayTeam: MatchDayTeam = _fixture.awayTeam) =
    _fixture.copy(
      date = date,
      stage = stage,
      round = round,
  ...
  private def result(date: DateTime, stage: Stage, round: Round, leg: String, homeTeam: MatchDayTeam = _result.homeTeam, awayTeam: MatchDayTeam = _result.awayTeam) =
    _result.copy(
      date = date,
      stage = stage,
      round = round,
  ...
  def pastKnockoutMatches(stage: Stage) = {
    ...
    List(
      ...
      result(now.minusDays(7), stage, quarterFinals, "1", teams(5), teams(7)),
