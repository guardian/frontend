...
class CapiFeed(contentApiClient: ContentApiClient) extends GuLogging {
  ...
  def getMatchArticles(
      matches: Seq[Match],
  )(implicit executionContext: ExecutionContext): Future[Map[String, MatchNavigation]] = {
    Future
      .sequence(
  ...
  private def loadNavigation(
      rugbyMatch: Match,
  )(implicit executionContext: ExecutionContext): Future[Option[MatchNavigation]] = {
    ...
    contentApiClient
      ...
      .flatMap { response =>
        val navContent = response.results
          .map(Content(_))
          .filter(_.trail.webPublicationDate.toDateTime(DateTimeZone.UTC).toLocalDate.isEqual(matchDate))
        (for {
